...
\newcommand{\errorrow}[1]{%
 #1 & \errorboxed{#1I} & \errorboxed{#1II} & \errorboxed{#1III}
 \write\markdown{#1|\errortext{#1I}|\errortext{#1II}|\errortext{#1III}}
}
...
\item \S7.4 Key Idea 7.4.4\#1 p371: $\cosh^{-1}$ is only defined for positive arguments, so we don't need the absolute value. % todo
\item \S7.5 Example 7.5.1\#3 p377: We need to have a $\lim_{x\to 0}$ in front of the $\frac2{\cos x}$.
%\item \S8.2 p397: ``only by a constant ... $+C$ for some constant $C$.'' Check the spacing between ``C'' and ``for''. % todo 
\item \S8.2 Key Idea 8.2.2\#4 p400: $\tan^{m-2}x$ is missing its $x$.
\item \S8.3 Example 8.3.2 p409: ``Key Idea 8.3.1(b)'' should be ``8.3.1(2)''. % todo
\item \S8.3 Example 8.3.3 p410: ``Key Idea 8.3.1(c)'' should be ``8.3.1(3)''. % todo
\item \S8.3 Example 8.3.5 p412: ``Key Idea 8.3.1(b)'' should be ``8.3.1(2)''. % todo
...
%\item \S9.1 p464: From Larry, at bottom of page: you could put in a sentence that says that theorems on limits at infinity from Calculus I have obvious analogues that apply to sequences.  (But we need to state the theorems on limits at infinity.)
\item \S9.1 p466: We haven't stated a Squeeze Theorem for sequences. % todo (but see previous note from Larry).
\item \S9.2 Theorem 9.2.1 p479: We need to specify that $a\neq 0$.
\item \S9.3 p490: $[n,\infty]$ should be $[n,\infty)$.
\item \S9.4 p497: ``For $n\ge N$, We're now'' should be lowercased. % p496?
...
\item \S9.7 p523 \#6: If the terms contain a factorial, then the divergence test might work.  (The root test is also a possibility, if you believe Knewton.) % todo
\item \S9.9 p546: ``pattern that make their formation'' should be ``makes''.
\item \S9.9 Theorem 9.9.1 p549: The theorem defines $R_n(x)$ and should be stated as such. % todo
...
\item \S9.9 Example 9.9.3\#2 p550: $720/5040\approx1/7$, with subsequent changes. % todo
...
\item \S9.10 Example 9.10.6 p562: The Taylor expansion for $\ln x$ given in Key Idea 9.10.1 is centered at $x=0$.  We can use Theorem 9.10.2 with $h(x)=x-1$. % todo
\item \S10.1 p580: The surface area approximation is not actually a Riemann Sum, because we use two different points in the same interval. % todo: omit or look into the gory details
\item \S10.2 Example 10.2.13 p594: ``on a wheel of radius $r$ as starts rolling" should be ``as it starts rolling''.
%\item \S10.3 p605: There's some bad spacing on $f'$ (twice).
\item \S10.3 p608: ``the parametric equation defining these curves'' should be ``equations''.
\item \S10.4 Figure 10.4.4: A ray for $-\pi/4$ should be in the fourth quadrant. % todo
\item \S10.4 Example 10.4.3 p615: $\tan\pi/4$ should be $\tan(\pi/4)$.
%\item \S10.4 Figure 10.4.7: move this outside of the example. % todo
%\item \S10.4 Example 10.4.6\#2 p617: There's a space in front of ``This function is valid only when''. % todo?
...
\item At the back of the book, integration rule \#2 should enclose its integrand in parentheses, and \#23 should have $a>0$ or use $|a|$.
%\item We should include $\int\sec^3xdx$ in the table of integration rules. % todo (or $\int\sec^{2k+1}xdx$)
\label{2021-06-00II}
\end{enumerate}
