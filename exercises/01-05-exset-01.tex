...
\end{tikzpicture}}{A line segment from (0,1) to a hollow dot at (1,2) then curving to (2,0).  There is also a solid dot at (1,1).}}{No; $\ds \lim_{x\to 1} f(x) = 2$, while $f(1) = 1$.}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A line segment from the origin to a hollow dot at (1,1).  A separate line segment goes from (1,2) to (2,0).}}{No; $\ds \lim_{x\to 1} f(x)$ does not exist.}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A curve from the origin that keeps getting larger as x approaches 1 from the left.  In a mirror image, the curve is large as x approaches 1 from the right and goes down to (2,0).}}{No; $f(1)$ does not exist.}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A line segment from (0,1) to a hollow dot at (1,2), then a solid dot at (1,1), and then a curve from (1,0) to (2,2).}}{No}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A curve from the origin to (1,2) and then a line segment from (1,2) to (2,0).}}{Yes}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A curve from (-4,-4) up to a hollow dot at (0,4), then a solid dot at the origin, and then a separate curve from (0,-4) up to (4,4).}}{Yes}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A line segment from (-4,0) to a hollow dot at (-2,2), then to a solid dot at the origin, then to a hollow dot at (2,2), then to a solid dot at (4,0).  There is also a solid dot at (-2,0).}}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumerate}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A wave that starts at (0,1), goes up to (π/2,2), down to (3π/2,0), and up to (2π,1).}}{Yes; $\ds \lim_{x\to 3\pi/2} \sin x +1 = 0$, and $\sin(3\pi/2)+1 = 0$.}
