\begin{exerciseset}{In Exercises}{, evaluate the given limits of the piecewise defined functions $f$.}

\exercise{$\ds f(x) = \begin{cases}
	x+1 & x\leq 1 \\
	x^2-5 & x>1
	\end{cases}$
\begin{multicols}{2}
\begin{enumerate}
\item		$\ds \lim_{x\to 1^-} f(x)$
\item		$\ds \lim_{x\to 1^+} f(x)$
\item		$\ds \lim_{x\to 1} f(x)$
\item		$f(1)$
\end{enumerate}
\end{multicols}}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumerate}
\item		2
\item		$-4$
\item		Does not exist.
\item		2
\end{enumerate}}}

\exercise{$f(x) = \begin{cases}
	2x^2+5x-1 & x<0 \\
	\sin x & x\geq 0
	\end{cases}$
\begin{multicols}{2}
\begin{enumerate}
\item		$\ds \lim_{x\to 0^-} f(x)$
\item		$\ds \lim_{x\to 0^+} f(x)$
\item		$\ds \lim_{x\to 0} f(x)$
\item		$f(0)$
\end{enumerate}
\end{multicols}}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumerate}
\item		$-1$
\item		0
\item		Does not exist.
\item		0
\end{enumerate}}}

\exercise{$\ds f(x) = \begin{cases}
	x^2-1 & x<-1 \\
	x^3+1 & -1\leq x\leq 1\\
	x^2+1 & x>1
	\end{cases}$
\begin{multicols}{2}
\begin{enumerate}
\item		$\ds \lim_{x\to -1^-} f(x)$
\item		$\ds \lim_{x\to -1^+} f(x)$
\item		$\ds \lim_{x\to -1} f(x)$
\item		$f(-1)$
\item		$\ds \lim_{x\to 1^-} f(x)$
\item		$\ds \lim_{x\to 1^+} f(x)$
\item		$\ds \lim_{x\to 1} f(x)$
\item		$f(1)$
\end{enumerate}
\end{multicols}}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumerate}
\item		0
\item		0
\item	  0
\item		0
\item		2
\item		2
\item		2
\item		2
\end{enumerate}}}

\exercise{$\ds f(x) = \begin{cases}
	\cos x & x<\pi \\
	\sin x & x\geq \pi
	\end{cases}$
\begin{multicols}{2}
\begin{enumerate}
\item		$\ds \lim_{x\to \pi^-} f(x)$
\item		$\ds \lim_{x\to \pi^+} f(x)$
%\item		$\ds \lim_{x\to -1} f(x)$
%\item		$f(-1)$
%\item		$\ds \lim_{x\to \pi} f(x)$
%\item		$\ds \lim_{x\to 1^+} f(x)$
\item		$\ds \lim_{x\to \pi} f(x)$
\item		$f(\pi)$
\end{enumerate}
\end{multicols}}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumerate}
\item		$-1$
\item		0
\item		Does not exist.
\item		0
\end{enumerate}}}

\exercise{$\ds f(x) = \begin{cases}
	1-\cos^2 x & x<a \\
	\sin^2 x & x\geq a
	\end{cases},$\\
where $a$ is a real number.
\begin{multicols}{2}
\begin{enumerate}
\item		$\ds \lim_{x\to a^-} f(x)$
\item		$\ds \lim_{x\to a^+} f(x)$
%\item		$\ds \lim_{x\to -1} f(x)$
%\item		$f(-1)$
%\item		$\ds \lim_{x\to \pi} f(x)$
%\item		$\ds \lim_{x\to 1^+} f(x)$
\item		$\ds \lim_{x\to a} f(x)$
\item		$f(a)$
\end{enumerate}
\end{multicols}}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumerate}
\item		$1-\cos^2 a = \sin^2 a$
\item		$\sin^2 a$
\item		$\sin^2 a$
\item		$\sin ^2 a$
\end{enumerate}}}

\exercise{$\ds f(x) = \begin{cases}
	x+1 & x<1 \\
	1  & x=1\\
	x-1 & x>1
	\end{cases}$
\begin{multicols}{2}
\begin{enumerate}
\item		$\ds \lim_{x\to 1^-} f(x)$
\item		$\ds \lim_{x\to 1^+} f(x)$
%\item		$\ds \lim_{x\to -1} f(x)$
%\item		$f(-1)$
%\item		$\ds \lim_{x\to \pi} f(x)$
%\item		$\ds \lim_{x\to 1^+} f(x)$
\item		$\ds \lim_{x\to 1} f(x)$
\item		$f(1)$
\end{enumerate}
\end{multicols}}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumerate}
\item		2
\item		0
\item		Does not exist
\item	  1
\end{enumerate}}}

\exercise{$\ds f(x) = \begin{cases}
	x^2 & x<2 \\
	x+1  & x=2\\
	-x^2+2x+4 & x>2
	\end{cases}$
\begin{multicols}{2}
\begin{enumerate}
\item		$\ds \lim_{x\to 2^-} f(x)$
\item		$\ds \lim_{x\to 2^+} f(x)$
%\item		$\ds \lim_{x\to -1} f(x)$
%\item		$f(-1)$
%\item		$\ds \lim_{x\to \pi} f(x)$
%\item		$\ds \lim_{x\to 1^+} f(x)$
\item		$\ds \lim_{x\to 2} f(x)$
\item		$f(2)$
\end{enumerate}
\end{multicols}}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumerate}
\item		4
\item		4
\item		4
\item	  3
\end{enumerate}}}

\exercise{$\ds f(x) = \begin{cases}
	a(x-b)^2+c & x<b \\
	a(x-b)+c & x\geq b
	\end{cases},$
where $a$, $b$ and $c$ are real numbers.
\begin{multicols}{2}
\begin{enumerate}
\item		$\ds \lim_{x\to b^-} f(x)$
\item		$\ds \lim_{x\to b^+} f(x)$
%\item		$\ds \lim_{x\to -1} f(x)$
%\item		$f(-1)$
%\item		$\ds \lim_{x\to \pi} f(x)$
%\item		$\ds \lim_{x\to 1^+} f(x)$
\item		$\ds \lim_{x\to b} f(x)$
\item		$f(b)$
\end{enumerate}
\end{multicols}}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumerate}
\item		$c$
\item		$c$
\item		$c$
\item	  $c$
\end{enumerate}}}

\exercise{$\ds f(x) = \begin{cases}
	\frac{\abs x}{x} & x\neq 0 \\
	0 & x=0
	\end{cases}$
\begin{multicols}{2}
\begin{enumerate}
\item		$\ds \lim_{x\to 0^-} f(x)$
\item		$\ds \lim_{x\to 0^+} f(x)$
%\item		$\ds \lim_{x\to -1} f(x)$
%\item		$f(-1)$
%\item		$\ds \lim_{x\to \pi} f(x)$
%\item		$\ds \lim_{x\to 1^+} f(x)$
\item		$\ds \lim_{x\to 0} f(x)$
\item		$f(0)$
\end{enumerate}
\end{multicols}}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumerate}
\item		$-1$
\item		$1$
\item		Does not exist
\item	  $0$
\end{enumerate}}}

\exercise{$\ds f(x)=\frac{\abs{4-x}}{x-4}$
\begin{multicols}{2}
\begin{enumerate}
\item		$\ds \lim_{x\to 4^-} f(x)$
\item		$\ds \lim_{x\to 4^+} f(x)$
\item		$\ds \lim_{x\to 4} f(x)$
\item		$f(4)$
\end{enumerate}
\end{multicols}}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumerate}
\item		$-1$
\item		$1$
\item		Does not exist.
\item		Undefined
\end{enumerate}}}

%\exercise{$\ds f(x)=\frac{x+2}{\abs{x+2}}$
%\begin{multicols}{2}
%\begin{enumerate}
%\item		$\ds \lim_{x\to -2^-} f(x)$
%\item		$\ds \lim_{x\to -2^+} f(x)$
%\item		$\ds \lim_{x\to -2} f(x)$
%\item		$f(-2)$
%\end{enumerate}
%\end{multicols}}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumerate}
%\item		$-1$
%\item		$1$
%\item		Does not exist.
%\item		undefined
%\end{enumerate}}}

\end{exerciseset}
