...
\exercise{What is the name of the rule which states that $\ds \frac{\dd}{\dd x}\big(x^n\big) = nx^{n-1}$, where $n>0$ is an integer?}{Power Rule.}

\exercise{What is $\ds \frac{\dd}{\dd x}\bigl(\ln x\bigr)$?}{$1/x$}
...
\item	$m(x) = \sqrt{x}$
\end{itemize}
\end{multicols}}{$f(x)$, $g(x)$, $h(x)$, and $m(x)$}
...
\input{exercises/02-03-exset-01}
...
\exercise{Prove the Constant Rule: $\dfrac {\dd}{\dd x} (c)=0$, where $c$ is constant.}{$\displaystyle\frac {\dd}{\dd x}(c) = \lim_{h\to 0} \frac{c - c}{h}=\lim_{h\to 0}0=0$}
...
\end{tikzpicture}}{The curve a starts near (-0.5,5), drops quickly to cross the x axis near x=-0.2, crosses the y axis near y=-1, then turns upward near (0.5,-1.5), crosses the x axis near x=0.8, and increases at a relatively constant rate to finish near (4,4).  The curve b starts near (-0.2,-6), crosses the y axis near y=-2 and the x axis near x=0.5, peaks near (2,1.2), and decreases slowly from there.  The curve c starts near (0.4,4), drops quickly to cross the x axis near x=2, and remains close to the x axis (but negative) after that.}}{$a$ is $f$, $b$ is $\fp$, $c$ is $\fpp$}

\exercise{The figure shows the graphs of $f$, $\fp$, $\fpp$ and $\fpp'$. Identify each curve and explain your choices.\\
\pdftooltip{\begin{tikzpicture}
 \begin{axis}[xtick=\empty, ytick=\empty, axis y line=middle,axis x line=middle,
   ymin=-3,ymax=3.5, xmin=-2,xmax=2, name=myplot, xscale=.9/1,]
 ...
 \node [right] at (myplot.right of origin) {\scriptsize $x$};
 \node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{The curve a is U shaped, with the bottom of the U touching the origin.  The curve b starts low in the third quadrant, increases and then flattens out as it passes through the origin before turning to increase again.  The curve c is U shaped as well, but with a flatter bottom than curve a, and the bottom of the curve is a bit below the x axis but on the y axis.  The curve d starts low in the third quadrant, increases quickly to the second quadrant where it turns to pass through the origin down to the fourth quadrant, where it turns again to increase quickly into the first quadrant.}}{$d$ is $f$, $c$ is $\fp$, $b$ is $\fpp$, and $a$ is $\fpp'$}

\input{exercises/02-03-exset-02}
...
\exercise{The position of an object is described by $s(t)=5e^t-5t$, where $s$ is in inches and $t$ is in seconds. Find
\begin{enumerate}
\item the velocity and acceleration functions for the object,
