\exercisesetinstructions{, find the equation of the tangent line to  the graph of the  implicitly defined function at the indicated points. As a visual aid, each function is graphed.}
...
\end{enumerate}
\begin{tikzpicture}[alt={A diamond shape with the sides pushed far in, so that the corners are pointed.},baseline=10pt]
...
\item	At $(0,1)$.
\end{enumerate}
\begin{tikzpicture}[alt={A squarish shape, with corners that have been rounded off.},baseline=10pt]
...
\end{enumerate}
\begin{tikzpicture}[alt={An oval oriented along the y-axis, with its sides pinched in toward the origin.},baseline=10pt]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis equal,
...
\end{enumerate}
\begin{tikzpicture}[alt={A round shape mostly to the left of the y-axis.  To the right of the y-axis, the roundness is interrupted by the curve pinching in to a cusp at the origin.},baseline=10pt]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis equal,
...
\end{enumerate}
\begin{tikzpicture}[alt={A circle centered at (2,3) with radius 3.},baseline=10pt]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis equal,
...
\item	At $(1,2)$.
\end{enumerate}
\begin{tikzpicture}[alt={A curve starting in the second quadrant near the x-axis, crossing the x-axis at x=-2, turning downward and then fading away from the y-axis as it continues downward.  A second curve mirrors the first with 180 degree symmetry.}]%[baseline=10pt]
