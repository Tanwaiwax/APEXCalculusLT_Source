...
\exercise{$f(x) =\sin (\sin^{-1}x)$}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumext}[start=1]
\item	$f(x) = x$, so $\fp(x) = 1$
\item	$\fp(x) = \cos(\sin^{-1}x)\frac{1}{\sqrt{1-x^2}} = 1$.
\end{enumext}}}

\exercise{$f(x) =\tan^{-1}(\tan x)$}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumext}[start=1]
\item	$f(x) = x$, so $\fp(x) = 1$
\item	$\fp(x) = \frac{1}{1+\tan^2x}\sec^2 x = 1$
\end{enumext}}}

\exercise{$f(x) =\sin(\cos^{-1} x)$}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumext}[start=1]
\item	$f(x) = \sqrt{1-x^2}$, so $\fp(x) = \frac{-x}{\sqrt{1-x^2}}$
\item	$\fp(x) = \cos (\cos^{-1}x)(\frac{1}{\sqrt{1-x^2}}) =\frac{-x}{\sqrt{1-x^2}}$
\end{enumext}}}

\exercise{$f(x) =\sin(\tan^{-1} x)$}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumext}[start=1]
