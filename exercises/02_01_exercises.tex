...
\exercise{In your own words, explain the difference between Definitions \ref{def:derivative_at_a_point} and \ref{def:the_derivative}.
}{Answers will vary.}

\exercise{Let $y=f(x)$. Give three different notations equivalent to ``$\fp(x)$.''
}{Answers will vary.}

\printproblems
\input{exercises/02_01_exset_02}
\input{exercises/02_01_exset_03}
\input{exercises/02_01_exset_01}
...
\input{exercises/02_01_exset_04}
...
\myincludegraphics[scale=.8]{figures/fig02_01_ex_30}}{\begin{enumerate}
...
%\ifthenelse{\boolean{printquestions}}{\columnbreak}{}
\printreview
\exercise{Approximate $\ds \lim_{x\to 5}\frac{x^2+2 x-35}{x^2-10.5 x+27.5}$.
}{Approximately 24.
}

\exercise{Use the Bisection Method to approximate, accurate to two decimal places, the root of $g(x) = x^3+x^2+x-1$ on $[0.5,0.6]$.
}{Approximately $0.54$.
}
...
\myincludegraphics[scale=.8]{figures/fig02_01_ex_34}}{\begin{enumerate}
\item		1
\item		3
\item		Does not exist
