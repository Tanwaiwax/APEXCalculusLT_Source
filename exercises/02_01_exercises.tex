...
\exerciseset{In Exercises}{, a graph of $g(x)$ is given.  Using the graph, answer the following questions.\\
...
\end{minipage}%
\begin{minipage}[t]{.5\linewidth}
	\begin{enumerate}[label=\alpha*]\addtocounter{enumi}{3}
		\item	Where is $g'(x) > 0$?
		\item	Where is $g'(x) < 0$?
		\item	Where is $g'(x) = 0$?
\end{enumerate}
...
\exercise{\mbox{}\\[-2\baselineskip]\begin{tikzpicture}
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
...
\exercise{\mbox{}\\[-2\baselineskip]\begin{tikzpicture}[scale=.8]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,xtick={-2,-1,1,2},
