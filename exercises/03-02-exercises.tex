...
\input{exercises/03-02-exset-01}

\input{exercises/03-02-exset-02}
...
\end{enumext}}}{(a) is Rolle's Theorem.  For (b), applying Rolle's Theorem to roots 1 and 2 and roots 2 and 3 shows that $\fp$ has two roots, and we can then apply (a).}
...
\exercise{Let $p$ and $q$ be constants, and define $f(x)=\frac px+q$.  Show that the Mean Value Theorem applied to $f$ for the interval $[a,b]$ is always satisfied at the geometric average of the endpoints.}{$-\frac p{c^2}=\fp(c)=\frac{f(b)-f(a)}{b-a}=\frac{p/b+q-p/a-q}{b-a}=\frac{p(a-b)}{ab(b-a)}=-\frac p{ab}$ implies that $c=\sqrt{ab}$.}
...
\exercise{Describe the critical points of $f(x) = \tan x$.}{They are the odd, integer valued multiples of $\pi/2$ (such as $\pm\pi/2, \pm 3\pi/2, \pm5\pi/2$, etc.)}
