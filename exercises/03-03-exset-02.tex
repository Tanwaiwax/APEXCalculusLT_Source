\begin{exerciseset}{In Exercises}{, a function $f(x)$ is given.
	...
	\item	Create a number line to determine the intervals on which $f$ is increasing and decreasing.
	\item	Use the First Derivative Test to determine whether each critical point corresponds to a relative maximum, minimum, or neither.
	\end{enumerate}}
...
\exercise{$\ds f(x) =\sin x\cos x$ on $(-\pi,\pi)$.}{domain=$(-\pi,\pi)$;\\
...
\exercise{$\ds f(x) =x-2 \sin x$ on $0\le x\le3\pi$}{domain=$[0,3\pi]$;\\
...
\exercise{$\ds f(x) =\cos^2 x-2 \sin x$ on $0\le x\le2\pi$}{domain=$[0,2\pi]$;\\
