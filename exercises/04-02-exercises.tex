...
\exercise{Water flows onto a flat surface at a rate of 5cm\textsuperscript{3}/s forming a circular puddle 10mm deep. How fast is the radius growing when the radius is:
	\begin{enumext}
	\item		1 cm?
	\item		10 cm?
	\item		100 cm?
...
\centering
\begin{tikzpicture}[alt={The plane is distance x horizontally from the gun.  The gun has an angle of elevation θ to the plane.},>=latex]
 \begin{scope}
...
\end{enumext}}{Due to the height of the plane, the gun does not have to rotate very fast.
\begin{enumext}
\item  $0.0573$ rad/s
\item	$0.0725$ rad/s
...
\noindent\begin{minipage}{\linewidth}
\centering
\begin{tikzpicture}[alt={Picture of the described situation.},>=latex]
...
\item	19 feet?
\end{enumext}
How long will the tank take to fill when starting at empty?}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumext}
\item  $50.92$ ft/min
\item	 $0.509$ ft/min
\item  $0.141$ ft/min
\end{enumext}}
...
\noindent\begin{minipage}{\linewidth}
\centering
\begin{tikzpicture}[alt={The connection point is 30 ft below the pulley.  The man moves to the right at 2 ft/s.},>=latex,x=1pt,y=1pt,scale=1.25]
...
\item	40 feet?
\end{enumext}
How far must the man walk to raise the weight all the way to the pulley?}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumext}
\item  $0.63$ ft/sec
\item	 $1.6$ ft/sec
\end{enumext}}
About 52 ft.}
