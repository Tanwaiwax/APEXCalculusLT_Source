...
\input{exercises/05_03_exset_02}

\input{exercises/05_03_exset_03}

\input{exercises/05_03_exset_04}

\exerciseset{In Exercises}{, express the limit as a definite integral.}{

\exercise{$\ds \lim_{n\to\infty} \frac{\pi}{n} \sum_{i=1}^n \frac{\sin \frac{\pi i}{n}}{1+\frac{\pi i}{n}}$}{$\ds \int_0^{\pi} \frac{\sin x}{1+x}\ dx$}

\exercise{$\ds \lim_{n\to\infty} \sum_{i=1}^n \frac{3}{n} \biggl(2+\frac{3i}{n}\biggr)\sqrt{1+\biggl(2+\frac{3i}{n}\biggr)^3}$}{$\ds \int_2^5 x\sqrt{1+x^3}\ dx$}

\exercise{$\ds \lim_{n\to\infty} \frac{5}{n} \sum_{i=1}^n \biggl(5\biggl(2+\frac{5i}{n}\biggr)^3-4\biggl(2+\frac{5i}{n}\biggr)+7\biggr)$}{$\ds \int_2^7 5x^3-4x+7\ dx$}

\exercise{$\ds  \lim_{n\to\infty} \frac{2}{n} \sum_{i=1}^n \frac{1+\frac{2i}{n}}{\biggl(1+\frac{2i}{n}\biggr)^2+4}$}{$\ds \int_1^3 \frac{x}{x^2+4}\ dx$}

}

\exerciseset{In Exercises}{, express the definite integral as a limit of a sum.}{

\exercise{$\ds \int_2^5 4-2x\ dx$}{$\ds \lim_{n\to\infty}\left[\frac{3}{n} \sum_{i=1}^n 4-2\biggl(2+\frac{3i}{n}\biggr)\right]$}

\exercise{$\ds \int_{-2}^0 x^2+3x\ dx$}{$\ds \lim_{n\to\infty} \frac{2}{n} \sum_{i=1}^n \biggl[\biggl(-2+\frac{2i}{n}\biggr)^2+3\biggl(-2+\frac{2i}{n}\biggr)\biggr]$}

\exercise{$\ds \int_{-\pi/2}^{\pi/2} \frac{\sin^3 x}{2+\cos x}\ dx$}{$\ds \lim_{n\to\infty} \frac{\pi}{n} \sum_{i=1}^n \frac{\sin^3(-\pi/2+\pi i/n)}{2+\cos (-\pi/2+\pi i/n)}$}

\exercise{$\ds\int_0^2 e^x\ dx$}{$\ds\lim_{n\to\infty}\frac2n\sum_{i=1}^n e^{2i/n}$}

}

\input{exercises/05_03_exset_05}

\input{exercises/05_03_exset_06}
...
\end{tabular}
\end{center}}{\mbox{}\\[-2\baselineskip]\begin{enumerate}
...
\exercise{Use Theorems \ref{thm:summation} and \ref{thm:riemannSum} to justify the remaining property in \autoref{thm:defintprop}: \[\int_a^b k\cdot f(x)\,dx =k\int_a^b f(x)\,dx\]}{\mbox{}\\[-2\baselineskip]\begin{align*}
...
\exercise{Use Theorems \ref{thm:summation} and \ref{thm:riemannSum} to justify the remaining property in \autoref{thm:further_def_int_props}: If $f(x)\leq M$ for all $x$ in $[a,b]$, then \[\int_a^b f(x)\,dx\leq M(b-a).\]}{Let $f$ and $M$ be as given.
\begin{align*}
\int_a^b f(x)\,dx
