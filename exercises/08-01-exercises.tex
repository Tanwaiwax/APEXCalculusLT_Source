...
\input{exercises/08-01-exset-01}

\input{exercises/08-01-exset-02}

\input{exercises/08-01-exset-06}

\input{exercises/08-01-exset-03}

\input{exercises/08-01-exset-04}

%\questioncolumnbreak

\input{exercises/08-01-exset-05}
...
%\end{enumext}}{A sketch of one proof method:
%\begin{enumext}[start=1]
...
\item Many recurrence relations yield sequences whose terms cannot be expressed by a simple function $f(n)$, and in fact such sequences can exhibit very unusual behavior. Read the Wikipedia article on the \href{https://en.wikipedia.org/wiki/Logistic_map}{Logistic Map} for a simple example that can exhibit a wide variety of behavior. (There is nothing to turn in for this part, unless your instructor gives you further directions.)
\end{enumext}}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumext}[start=1]\item $f(n)=Ar^n$\end{enumext}}}
...
\exercise{Define a sequence $\{a_n\}$ by $a_1=1$ and $a_n=\sqrt{2+a_{n-1}}$ for all $n > 1$. So $a_1=1$, $a_2=\sqrt3$, $a_3=\sqrt{2+\sqrt3}$, $a_3=\sqrt{2+\sqrt{2+\sqrt3}}$,\dots.
 ...
 \end{enumext}}{\mbox{}\\[-2\baselineskip]\parbox[t]{\linewidth}{\begin{enumext}[start=1]\setcounter{enumii}{3}\item 2\end{enumext}}}
% x = sqrt(2+x)
% x^2 = 2+x
% x^2 - x - 2 = 0
% (x-2)(x+1) = 0
