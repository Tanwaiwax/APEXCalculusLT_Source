\exerciseset{In Exercises}{, show that the Taylor series for $f(x)$, as given in \autoref{idea:common_taylor}, is equal to $f(x)$ by applying \autoref{thm:function_series_equality}; that is, show $\ds \lim_{n\to\infty}R_n(x) =0$.
}{
...
\exercise{$f(x) = \sin x$
}{The following argument is essentially the same as that given for $f(x) = \cos x$ in \autoref{ex_ts3}.
