...
\begin{enumext}
	...
	\item $D(1,\pi/4)$
\end{enumext}
\end{multicols}}{\mbox{}\\\begin{minipage}[m]{\linewidth}
\begin{tikzpicture}[alt={Four points: A at (2,0), B at (-1,0), C at (0,-2), and D at (1/√2,1/√2).},scale=.5]
\foreach \x in {1,2} {
	\draw (0,0) circle (\x);
...
\end{enumext}
\end{multicols}}{\mbox{}\\\begin{minipage}[m]{\linewidth}
\begin{tikzpicture}[alt={Four points: A at (-2,0), B at (-1,0), C at (-0.4,0.9), and D at (0.43,0.25).},scale=.5]
\foreach \x in {1,2} {
 \draw (0,0) circle (\x);
 \draw (\x,0) node [below right] {\scriptsize $\x$};
}
...
\begin{tikzpicture}[alt={Four points: A at distance 2.5 and angle π/4, B at distance 1 and angle -π/6, C at distance 3 and angle -2π/3, and D at distance 1.5 and angle 2π/3.},scale=.5]
...
\begin{tikzpicture}[alt={Four points: A at distance 2 and angle π/6, B at distance 1 and angle -π/3, C at distance 2 and angle 3π/4, and D at (-2.5,0).},scale=.5]
	\draw [dashed,gray] (-3.1,0) -- (0,0);
	\draw[thick,->,>=stealth] (0,0) node [below] {$O$} -- (3.5,0) ;
	\filldraw (0,0) circle (1.5pt);
	\foreach \x in {1,2,3}
...
\input{exercises/09-04-exset-03}
