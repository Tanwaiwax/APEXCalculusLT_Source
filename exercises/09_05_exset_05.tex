...
\exercise{Use the arc length formula to compute the arc length of the circle $r=2$.
}{$4\pi$
}

\exercise{Use the arc length formula to compute the arc length of the circle $r=4\sin\theta$.
}{$4\pi$
}

\exercise{Approximate the arc length of one petal of the rose curve $r=\sin(3\theta)$ with Simpson's Rule and $n=4.$
}{$L\approx 2.2592$; (actual value $L=2.22748$)
}

\exercise{Approximate the arc length of the cardioid $r=1+\cos\theta$ with Simpson's Rule and $n=6.$
