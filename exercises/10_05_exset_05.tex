\exerciseset{Exercises}{ explore special cases of the distance formulas found in Key Idea \ref{idea:line_distance}.
}{

\exercise{Let $Q$ be a point on the line $\ell(t)$. Show why the distance formula correctly gives the distance from the point to the line as $0$.
}{Since both $P$ and $Q$ are on the line, $\vv{PQ}$ is parallel to $\vec d$. Thus $\vv{PQ}\times \vec d = \vec 0$, giving a distance of $0$.
}

\exercise{\label{int_lines_dist}Let lines $\ell_1(t)$ and $\ell_2(t)$ be intersecting lines. Show why the distance formula correctly gives the distance between these lines as $0$.\label{10_05_ex_30}
