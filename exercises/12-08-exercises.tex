...
\exercise{Let a level curve of $z=f(x,y)$ be described by $x=g(t)$, $y = h(t)$. Explain why $\frac{\dd z}{\dd t}=0$.}{Because the parametric equations describe a level curve, $z$ is constant for all $t$. Therefore $\frac{\dd z}{\dd t}=0$.}

\exercise{Fill in the blank: The single variable Chain Rule states $\ds\frac{\dd}{\dd x}\Big(f\big(g(x)\big)\Big) = \fp\big(g(x)\big)\cdot$\underline{\hskip.5in}.}{$g'(x)$}

\exercise{Fill in the blank: The Multivariable Chain Rule states that if $f$ is a function of $x$ and $y$ which are each a function of $t$, then\\[5pt]
$\ds\frac{\dd f}{\dd t}=\frac{\partial f}{\partial x}\cdot\text{\underline{\hskip.5in}}+\text{\underline{\hskip.5in}}\cdot\frac{\dd y}{\dd t}$.}{$\frac{\dd x}{\dd t}$, and $\frac{\partial f}{\partial y}$}

\exercise{If $z=f(x,y)$, where $x=g(t)$ and $y=h(t)$, we can substitute and write $z$ as an explicit function of $t$.\\
T/F: Using the Multivariable Chain Rule to find $\frac{\dd z}{\dd t}$ is  sometimes easier than  first substituting and then taking the derivative.}{T}
...
\input{exercises/12-08-exset-01}

\input{exercises/12-08-exset-02}

\input{exercises/12-08-exset-03}

\input{exercises/12-08-exset-04}

\input{exercises/12-08-exset-05}
