\begin{exerciseset}{In Exercises}{,  functions $z=f(x,y)$, $x=g(t)$ and $y=h(t)$ are given. Find the values of $t$ where $\frac{\dd z}{\dd t}=0$. Note: these are the same surfaces/curves as found in Exercises~\ref{12_08_ex_07}--\ref{12_08_ex_08}.}
...
\[\frac{\dd z}{\dd t} = -\frac{2\cos^2t\sin t}{(1+\sin^2t)^2}-\frac{\sin t}{1+\sin^2t}.\]
...
\[\frac{\dd z}{\dd t} = 38\cos t\sin t.\]
Thus $\frac{\dd z}{\dd t} = 0$ when $t=\pi n$ or $\pi n+\pi/2$, where $n$ is any integer.}

\exercise{$\ds z=\cos x\sin y$,\qquad $x=\pi t$,\qquad $y=2\pi t+\pi/2$}{We find that $\frac{\dd z}{\dd t} =$
\[-\pi\sin(\pi t)\sin(2\pi t+\pi/2)+2\pi\cos(\pi t)\cos(2\pi t+\pi/2).\]

One can ``easily'' see that when $t$ is an integer, $\sin(\pi t) =0$ and $\cos(2\pi t+\pi/2)=0$, hence $\frac{\dd z}{\dd t}=0$ when $t$ is an integer. There are other places where $z$ has a relative max/min that require more work. First, verify that $\sin(2\pi t+\pi/2) = \cos(2\pi t)$, and $\cos(2\pi t+\pi/2) = -\sin(2\pi t)$. This lets us rewrite $\frac{\dd z}{\dd t} = 0$ as
