...
\exercise{Integrating an integral is called \underline{\hskip .5in} \underline{\hskip .5in}.
}{iterated integration
}

\exercise{When evaluating an iterated integral, we integrate from \underline{\hskip .5in} to \underline{\hskip .5in}, then from \underline{\hskip .5in} to \underline{\hskip .5in}.
}{curve to curve, then from point to point
}

\exercise{One understanding of an iterated integral is that $\ds \int_a^b\int_{g_1(x)}^{g_2(x)} \ dy\ dx$ gives the \underline{\hskip .5in} of a plane region.
}{area
}
