...
\exercise{Why does $M_x$ use $\ds\iint_R y\delta(x,y)\ dA$ instead of $\ds\iint_R x\delta(x,y)\ dA$; that is, why do we use ``$y$'' and not ``$x$''?}{$M_x$ measures the moment about the $x$-axis, meaning we need to measure distance from the $x$-axis. Such measurements are measures in the $y$-direction.}
