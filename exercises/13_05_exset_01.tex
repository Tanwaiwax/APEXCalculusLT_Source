\exerciseset{In Exercises} 
{,  \textit{set up} the iterated integral that computes the surface area of the given surface over the region $R$.
}{

\exercise{$f(x,y) = \sin x\cos y$;\quad $R$ is the rectangle with bounds $0\leq x\leq 2\pi$, \quad $0\leq y\leq2\pi$.

\myincludegraphicsthree{width=150pt,3Dmenu,activate=onclick,deactivate=onclick,
3Droll=0,
3Dortho=0.0049752527847886086,
3Dc2c=0.40672242641448975 0.6828556656837463 0.6068649291992188,
3Dcoo=30.6619873046875 3.983891248703003 -25.03248405456543,
3Droo=150.00000343149,
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.2,trim=0mm 5mm 0mm 0mm,clip}{figures/fig13_05_ex_05}}{$\ds SA = \int_0^{2\pi}\int_0^{2\pi} \sqrt{1+ \cos^2x\cos^2y+\sin^2x\sin^2y}\ dx\ dy$
}
\exercise{$\ds f(x,y) = \frac{1}{x^2+y^2+1}$;\quad $R$ is the circle $x^2+y^2=9$.

\myincludegraphicsthree{width=150pt,3Dmenu,activate=onclick,deactivate=onclick,
3Droll=0,
3Dortho=0.005094848107546568,
3Dc2c=0.5450001358985901 0.6626530885696411 0.5136786103248596,
3Dcoo=-12.860339164733887 -20.208675384521484 36.5958251953125,
3Droo=150.00000281692286,
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.2,trim=0mm 5mm 0mm 10mm,clip}{figures/fig13_05_ex_06}
}{$\ds SA = \int_{-3}^{3}\int_{-\sqrt{9-x^2}}^{\sqrt{9-x^2}} \sqrt{1+ \frac{4x^2+4y^2}{(1+x^2+y^2)^4}}\ dx\ dy$

Polar offers simpler bounds:

$\ds SA = \int_0^{2\pi}\int_0^3 r\sqrt{1+\frac{4r^2}{(1+r^2)^4}}\ dr\ d\theta$
}
\exercise{$\ds f(x,y) = x^2-y^2$;\quad $R$ is the rectangle with opposite corners $(-1,-1)$ and $(1,1)$.

\myincludegraphicsthree{width=150pt,3Dmenu,activate=onclick,deactivate=onclick,
3Droll=0,
3Dortho=0.004975249990820885,
3Dc2c=0.4553585350513458 0.7132656574249268 0.532823383808136,
3Dcoo=-0.20323395729064941 3.399019956588745 -5.396617889404297,
3Droo=150.00000127208068,
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.2,trim=0mm 5mm 0mm 10mm,clip}{figures/fig13_05_ex_07}
}{$\ds SA = \int_{-1}^{1}\int_{-1}^{1} \sqrt{1+ 4x^2+4y^2}\ dx\ dy$
}
\exercise{$\ds f(x,y) = \frac{1}{e^{x^2}+1}$;\quad $R$ is the rectangle bounded by\\[5pt]
 $-5\leq x\leq 5$ and $0\leq y\leq 1$.

\myincludegraphicsthree{width=150pt,3Dmenu,activate=onclick,deactivate=onclick,
3Droll=0,
3Dortho=0.004975249990820885,
3Dc2c=0.3090496361255646 0.6646339893341064 0.680257260799408,
3Dcoo=-18.87652587890625 -2.228297710418701 8.584968566894531,
3Droo=149.99999722540937,
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.2,trim=0mm 5mm 0mm 10mm,clip}{figures/fig13_05_ex_08}}{$\ds SA = \int_{-5}^{5}\int_{0}^{1} \sqrt{1+ \frac{4x^2e^{2x^2}}{\big(1+e^{x^2}\big)^4}}\ dy\ dx$
}

}
