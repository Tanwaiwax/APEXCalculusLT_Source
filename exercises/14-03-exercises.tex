...
\exercise{Let $\vec F$ be a conservative field and let $C$ be a closed curve. Why are we able to conclude that $\oint _C \vec F\cdot\dd\vec r = 0$?}{By the Fundamental Theorem of Line Integrals, since $\vec F$ is conservative, $\oint_C \vec F\cdot\dd\vec r = f(B) - f(A)$, where $f$ is a potential function for $\vec F$ and $A$ and $B$ are the initial and terminal points of $C$, respectively. Since $C$ is a closed curve, $A = B$, and hence $f(B) - f(A) = 0$.}

\printproblems

\input{exercises/14-03-exset-01}
\input{exercises/14-03-exset-02}

% Mecmath

\exercisesetinstructions{, determine if the given vector field has a potential.  If so, find one.}

\exercise{$\vec F(x,y) = y\,\veci - x\,\vecj$}{No.}
...
\input{exercises/14-03-exset-03}
...
\exercise{Show that if $\vecf\perp\vecr\primeskip'(t)$ at each point $\vecr(t)$ along a smooth curve $C$, then $\int_C\vecf\cdot\dd\vecr=0$.}{}

\exercise{Show that if $\vecf$ points in the same direction as $\vecr\primeskip'(t)$ at each point $\vecr(t)$ along a smooth curve $C$, then $\int_C \vecf\cdot\dd\vecr=\int_C\norm{\vecf}\dd s$.}{}
...
$\abs{\int_C\vecf\cdot\dd\vecr} \le ML$. (\emph{Hint: Recall that $\abs{\int_a^b g(x)\dd x} \le \int_a^b \abs{g(x)}\dd x$ for Riemann integrals.})}{}
...
\exercise{Prove \autoref{thm:line_int_properties_vector} part 1.  Let $\vec F$ and $\vec G$ be vector fields, let $k_1$ and $k_2$ be constants, and let $C$. Show that
  \[
   \int_C(k_1\,\vec F\pm k_2\,\vec G)\cdot\dd\vecr
   =k_1\int_C\vec F\cdot\dd\vecr\pm k_2\int_C\vec G\cdot\dd\vecr.
  \]}{}

\exercise{Let $f(x,y)$ and $g(x,y)$ be continuously differentiable real-valued functions in a region $R$. Show that
  \[
   \oint_{C}(f\,\nabla g)\cdot\dd\vecr = -\oint_{C}(g\,\nabla f)\cdot\dd\vecr
  \]
...
\exercise{Let\label{contra_green} $\vec F(x,y)=\frac{-y}{x^2 + y^2}\,\veci + \frac{x}{x^2 + y^2}\,\vecj$ for all $(x,y)\ne(0,0)$, and $C:$ $x=\cos t$, $y=\sin t$, $0\le t\le 2\pi$.
  \begin{enumext}[start=1]
   ...
   \item Show that $\ds\oint_{C}\vec F\cdot\dd\vecr = 2\pi$. Does this contradict \autoref{thm:FTofLineIntegrals}? Explain.%{cor:lineintsuffpath}? Explain.
  \end{enumext}}{(b) No. \emph{Hint:} Think of how $f$ is defined.}
...
% Bevelaqua problems
\exercise{For $\vec F=\bracket{xy,y}$, evaluate $\ds\oint_C\vec F\cdot\dd\vecr$ where $C$ is $x=\cos t$, $y=\sin t$ for $0\le t\le2\pi$. Is $\vec F$ conservative?}{$0$; no}

\exercise{For $\vec F=\bracket{y^2,x^2}$, evaluate $\ds\int_C\vec F\cdot\dd\vecr$ for:
\begin{enumext}[start=1]
\item $C$ given by $x=t$, $y=t^2$ for $0\le t\le1$.
\item $C$ given by $x=t^2$, $y=t$ for $0\le t\le1$.
\item Is $\vec F$ conservative?
\end{enumext}}{$7/10$; $7/10$; no}
