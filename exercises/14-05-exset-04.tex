\begin{exerciseset}{In Exercises}{, find the surface area $S$ of the given surface \surfaceS. (The associated integrals are computable without the assistance of technology.)}
...
% Mecmath
\exercise{\surfaceS\ is a sphere of radius $r$. (\emph{Hint: Use spherical coordinates to parameterize the sphere.})}{$4\pi r^2$}

\exercise{\surfaceS\ is a right circular cone of radius $R$ and height $h$. (\emph{Hint: Use the parameterization $x=r\cos\theta$, $y=r\sin\theta$, $z=\frac{h}{R}r$, for $0 \le r \le R$ and $0 \le \theta \le 2\pi$.})}{$\pi R \sqrt{h^2 + R^2}$}
...
\exercise{The ellipsoid\index{ellipsoid} $\frac{x^2}{a^2}+\frac{y^2}{b^2}+\frac{z^2}{c^2}=1$ can be parameterized using \emph{ellipsoidal coordinates}\index{coordinates!ellipsoidal}
 \[
 ...
 \begin{multline*}
  \int_0^{\pi} \int_0^{2\pi} \sin\phi\cdot\\
  \sqrt{a^2 b^2 \cos^2 \phi + c^2 (a^2 \sin^2 \theta + b^2 \cos^2 \theta )
  \sin^2 \phi} \dd\theta\dd\phi.
 \end{multline*}
...
\exercise{Use \autoref{thm:par_surface_area} to prove that the surface area $S$ over a region $R$ in $\mathbb{R}^{2}$ of a surface $z=f(x,y)$ is given by the formula
  \[
   ...
   \left( \tfrac{\partial f}{\partial y} \right)^2} \dd A .
  \]
  (\emph{Hint: Think of the parameterization of the surface.})}{}

% trying to avoid underfull hbox
