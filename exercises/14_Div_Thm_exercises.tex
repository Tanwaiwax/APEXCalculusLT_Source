...
\exerciseset{In Exercises}{, use the Divergence Theorem to evaluate the surface integral $\iint_{\Sigma} \Dotprod{\vecf}{d\vec\sigma}$ of the given vector field $\vecf(x,y,z)$ over the surface $\Sigma$.}{

\exercise{$\vecf(x,y,z) = x\veci + 2y\vecj + 3z\veck$, $\Sigma: x^2 + y^2 + z^2 = 9$}{$216\pi$}

\exercise{$\vecf(x,y,z) = x\veci + y\vecj + z\veck$, $\Sigma:$ boundary of the solid cube $S = \{\, (x,y,z): 0 \le x,y,z \le 1 \,\}$}{3}
...
\exercise{Evaluate the surface integral $\iint_{\Sigma} \Dotprod{\vecf}{d\vec\sigma}$, where $\vecf(x,y,z) = x^2 \veci + xy\vecj + z\veck$ and $\Sigma$ is the part of the plane $6x+3y+2z=6$ with $x \ge 0$, $y \ge 0$, and $z \ge 0$, with the outward unit normal $\vecn$ pointing in the positive $z$ direction.}{$7/4$}
