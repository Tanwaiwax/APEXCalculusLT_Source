...
\exerciseset{For Exercises}{, use the Divergence Theorem to evaluate the surface integral $\iint_{\Sigma} \Dotprod{\textbf{f}}{d\sigma}$ of the given vector field $\textbf{f}(x,y,z)$ over the surface $\Sigma$.}{

\exercise{$\textbf{f}(x,y,z) = x\textbf{i} + 2y\textbf{j} + 3z\textbf{k}$, $\Sigma: x^2 + y^2 + z^2 = 9$}{$216\pi$}

\exercise{$\textbf{f}(x,y,z) = x\textbf{i} + y\textbf{j} + z\textbf{k}$, $\Sigma:$ boundary of the solid cube $S = \lbrace\, (x,y,z): 0 \le x,y,z \le 1 \,\rbrace$}{3}

\exercise{$\textbf{f}(x,y,z) = x^3\textbf{i} + y^3\textbf{j} + z^3\textbf{k}$, $\Sigma: x^2 + y^2 + z^2 = 1$}{$12\pi/5$}

\exercise{$\textbf{f}(x,y,z) = 2\textbf{i} + 3\textbf{j} + 5\textbf{k}$, $\Sigma: x^2 + y^2 + z^2 = 1$}{}
...
\exercise{Evaluate the surface integral $\iint_{\Sigma} \Dotprod{\textbf{f}}{d\sigma}$, where $\textbf{f}(x,y,z) = x^2 \textbf{i} + xy\textbf{j} + z\textbf{k}$ and $\Sigma$ is the part of the plane $6x+3y+2z=6$ with $x \ge 0$, $y \ge 0$, and $z \ge 0$, with the outward unit normal $\textbf{n}$ pointing in the positive $z$ direction.}{$15/4$}
...
\exercise{Use \autoref{defn:surfintreal} to prove that the surface area $S$ over a region $R$ in $\Real{2}$ of a surface $z=f(x,y)$ is given by the formula
  \[
   S = \iint_R \sqrt{1 + \left( \tfrac{\partial f}{\partial x} \right)^2 +
