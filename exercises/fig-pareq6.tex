% who is this file for?

\begin{tikzpicture}[alt={A parabola from the origin to (96,144) back to (192,0), with incorrect x and y equations.}]
\begin{axis}[width=1.16\marginparwidth,%
tick label style={font=\scriptsize},axis y line=middle,axis x line=middle,name=myplot,%
			%x=.37\marginparwidth,
			%y=.37\marginparwidth,
			%xtick={2,4,6,8,10},
			%minor x tick num=4,% 
%			extra x ticks={.33},
%			extra x tick labels={$1/3$},
			%ytick={-10,-5,5,10},
			%minor y tick num=4,%extra y ticks={-5,-3,...,7},%
			ymin=0,ymax=200,%
			xmin=0,xmax=200%
]
%\draw (axis cs:4,-1) node (A) {}
			%(axis cs:4,3) node (B) {} ;
%\draw [->,>=latex] (A) parabola bend (axis cs:0,1) (B);
\addplot [thick, smooth,domain=0:6] ({32*x},{-16*x^2+96*x});
\draw [->,>=latex] (axis cs:-4.62287, 7.5225) -- (axis cs:-4.4041, 7.4756);
\draw [->,>=latex] (axis cs:7,11) -- (axis cs:7.1107, 11.0601);
\draw (axis cs: 7,14) node [above right,align=left] {\scriptsize $x=t^3-5t^2+3t+11$\\ \scriptsize $y=t^2-2t+3$};
\end{axis}
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}
