...
//ASY file for fig:lines_dist2 in Chapter 10 section sec:lines
...
// line 1
triple f(real t) {return (p1+t*d1);}
	
	// line 2
triple g(real t) {return (p2+t*d2);}
...
dot(p1);
label("$P_1$",p1,S);
draw(f(0)--f(.6),colortwo+.7mm,Arrow3(size=2mm));
...
dot(p2);
label("$P_2$",p2,N);
draw(g(0)--g(.6),colortwo+.7mm,Arrow3(size=2mm));
...
//draw the lines
draw(f(-1)--f(0),colorone);//L1
draw(f(.6)--f(2),colorone);//L1
draw(g(-1)--g(0),colorone);//L2
draw(g(.6)--g(2),colorone);//L2
...
// draw perp vector
draw(f(.9)--f(.9)+c*abs(dot(p12,c))*.4,colortwo,Arrow3(size=2mm));
