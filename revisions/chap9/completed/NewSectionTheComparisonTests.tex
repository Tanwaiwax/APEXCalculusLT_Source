...
%%%%%  ADD this PROOF HERE: %%%%%%%%
Proof :  First consider the partial sums of each series. $$S_n=\sum_{i=1}^n a_i \quad \text{and} \quad T_n=\sum_{i=1}^n b_i$$ Since both series have positive terms we know that $$S_n\leq S_n+a_{n+1}=\sum_{i=1}^n a_i+a_{n+1}=\sum_{i=1}^{n+1} a_i=S_{n+1}$$ and  $$T_n\leq T_n+b_{n+1}=\sum_{i=1}^n b_i+b_{n+1}=\sum_{i=1}^{n+1} b_i=T_{n+1}$$ Therefore, both of the sequences of partial sums,$\{S_n\}$ and $\{T_n\}$, are increasing. We also know that because $a_n\leq b_n$ for all $n\geq N$ that we must have $S_n\leq T_n$ for all $n\geq N$. \\
...
$$S_n\leq \sum_{i=1}^\infty b_i$$ Because $\ds \sum_{i=1}^\infty b_i$ converges it must have a finite value and $\{S_n\}$ is bounded above. We also showed that $\{S_n\}$ is increasing so by Theorem \ref{thm:monotonic_converge} we know $\{S_n\}$ converges and so $\ds \sum_{n=1}^\infty a_n$ converges.   \\
...
{
Determine the convergence of $\ds\sum_{n=1}^\infty \frac{n^3}{n^4-1}$.}
...
Proof of 2. : Since $\ds \lim_{n\to\infty} a_n =0$, there is a number $N>0$ such that 
\begin{align*}
...
Proof of 3. : Since $\ds \lim_{n\to\infty} a_n =\infty$, there is a number $N>0$ such that 
\begin{align*}
\frac{a_n}{b_n}&>1~ \text{for all}~ n>N \\
a_n&>b_n ~ \text{for all}~ n>N 
\end{align*}
Now since $\ds \sum_{n=1}^\infty b_n$ diverges, $\ds \sum_{n=1}^\infty a_n$ diverges by the Comparison Test.
...
\begin{align*}
\lim_{n\to\infty}\frac{{n^3}/(n^4+1)}{1/n} &= \lim_{n\to\infty} \frac{n^4}{n^4+1}= \lim_{n\to\infty} \frac{1}{1+1/n^4} \\
			&= 1.
\end{align*}
...
\begin{align*}
\lim_{n\to\infty}\frac{1/(3^n-n^2)}{1/3^n} &= \lim_{n\to\infty}\frac{3^n}{3^n-n^2} \stackrel{\ \text{ L'H \rule[-5pt]{0pt}{3pt}} \ }{=} \lim_{n\to\infty} \frac{\ln 3 \cdot 3^n}{\ln 3 \cdot 3^n -2n}\\
...
For the exercises, take problem 4  from the terms and concepts from Section 8.3

Take 13-17 from 8.3 with their instructions. \\
Take 23-27 from 8.3 with their instructions.\\

Add a new grouping of problems with the instructions:\\

In Exercises ???-???, use the Comparison Test or the Limit Comparison Test to determine the convergence of the given series. State what series is used for comparison.\\
		For this block use problems 18-22 and 28-32 from Section 8.3 and also add the following problems \\


$\ds \sum_{n=1}^\infty \frac{n}{n^4+1}$		Solution: Converges by Comparison Test with $\sum \frac{1}{n^3}$\\

$\ds \sum_{n=1}^\infty \frac{n-1}{n4^n}$		Solution: Converges by Comparison Test with $\sum \frac{1}{4^n}$ \\

$\ds \sum_{n=1}^\infty \frac{1+\sin n}{10^n}$	Solution: Converges by Comparison Test with $\sum \frac{2}{10^n}$\\
