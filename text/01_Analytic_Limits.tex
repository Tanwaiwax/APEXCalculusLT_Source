...
\item	\parbox{80pt}{Compositions:} \parbox[t]{200pt}{Adjust our previously given limit situation to: $$\lim_{x\to c}f(x) = L \text{\ and\ } \lim_{x\to L} g(x) = g(L) = K.$$ Then $\ds \lim_{x\to c}g(f(x)) = K$.}
\end{enumerate}
}
