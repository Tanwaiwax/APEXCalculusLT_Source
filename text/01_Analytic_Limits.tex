...
\mtable[-1.5in]{The situation of the squeeze theorem}{fig:squeeze_thm}{\pdftooltip{\begin{tikzpicture}
...
\end{tikzpicture}}{A blue h(x) above a dashed g(x) above a red f(x).  All three come together when x=c.}}
...
\pdftooltip{\begin{tikzpicture}[x=.4\marginparwidth,y=.4\marginparwidth,>=stealth]
...
\end{tikzpicture}}{The unit circle with a ray at angle 𝜃 intersecting the circle at (cos𝜃,sin𝜃) and the line x=1 at (1,tan𝜃).  The point (cos𝜃,sin𝜃) is also connected to (1,0).}}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A blue line segment connecting (0,1) to (2,3).  There is a red circle around a hole in the line at (1,2).}}
