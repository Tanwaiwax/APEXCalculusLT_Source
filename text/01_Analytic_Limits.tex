...
\example{ex_limit_rat}{Finding a limit of a rational function}{Using \autoref{thm:poly_rat}, find
...
\example{ex_limit_sinx_prove}{Using the Squeeze Theorem}{Use the Squeeze Theorem to show that
...
\theorem{thm:special_limits}{Special Limits}{\mbox{}\\[-2.5\baselineskip]
\begin{multicols}{2}
\begin{enumerate}
...
\example{ex_limit_onept}{Using algebra to evaluate a limit}{Evaluate the following limit:
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}}{We would like to apply Theorems \ref{thm:limit_algebra} and \ref{thm:poly_rat} and substitute 1 for $x$ in the quotient. This gives:
