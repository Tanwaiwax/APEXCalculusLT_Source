...
\item	\parbox{80pt}{Roots:} %\parbox[t]{185pt}{
$\ds\lim_{x\to c} \sqrt[n]{f(x)} = \sqrt[n]{L}$ \qquad {\small (when $n$ is odd or $L\ge0$)}%}
%\item	\parbox{80pt}{Compositions:} \parbox[t]{200pt}{Adjust our previously given limit situation to: \[\lim_{x\to c}f(x) = L \text{\ and\ } \lim_{x\to L} g(x) = g(L) = K\text{.}\] Then $\ds \lim_{x\to c}g(f(x)) = K$.}
\end{enumerate}}

%We make a note about Property \#8: when $n$ is even, $L$ must be greater than 0. If $n$ is odd, then the statement is true for all $L$.

We will now prove the Sum Property using the formal definition of a limit from the previous section. We know that $\ds \lim_{x\to c}f(x) = L$ and  $\lim_{x\to c} g(x) = K$. We want to show that $\ds \lim_{x\to c}f(x)+g(x)=L+K$.
...
We know $\ds \lim_{x\to c}f(x) = L$. So for any $\epsilon_1 >0$, we can find $\delta_1>0$ such that if $0<\abs{x-c}<\delta_1$, then $\abs{f(x)-L}<\epsilon_1$. Similarly we know $\lim_{x\to c} g(x) = K$ so for any $\epsilon_2>0$, we can find $\delta_2>0$ such that if  $0<\abs{x-c}<\delta_2$, then $\abs{g(x)-K}<\epsilon_2$. We will let both $\epsilon_1$ and $\epsilon_2$ be $\frac{\epsilon}{2}$. Now, we have a $\delta_1>0$ and a $\delta_2>0$ such that:
\begin{gather*}
...
\youtubeVideo{Nz6UUQ4HQ}{Limit Laws to Evaluate a Limit, Example 1}
...
\noindent
\ifthenelse{\isodd{\pageref{thm:lim_continuous}}}{}{\hskip -100pt}%
...
We can also use Theorem \ref{thm:limit_composition}. Using Theorem \ref{thm:lim_continuous}, we have $\ds \lim_{x\to 1}\ln x = \ln 1 = 0$. Applying the Composition rule, $$\ds \lim_{x\to 1} e^{\ln x} = \lim_{x\to 0} e^x = e^0 = 1.$$ Both approaches are valid, giving the same result.
...
\mtable{The situation of the squeeze theorem}{fig:squeeze_thm}{\begin{tikzpicture}
...
\mfigure{0in}{The unit circle and related triangles.}{fig:squeeze_sinx}{figures/figSqueeze1}
% todo make this in color
...
$\ds \frac{\tan \theta}{2}$
 & $\geq$ & $\ds \frac{\theta}{2}$ & $\geq$ & $\ds \frac{\sin \theta}{2}$
\end{tabular}
\end{center}
...
{We would like to apply Theorems \ref{thm:limit_algebra} and \ref{thm:lim_continuous} and substitute 1 for $x$ in the quotient. This gives:
		$$\lim_{x\to 1}\frac{x^2-1}{x-1} = \frac{1^2-1}{1-1} = \raisebox{8pt}{\text{``\ }}\frac{0}{0}\raisebox{8pt}{\text{\ ''}},$$ an indeterminate form. We cannot apply the Theorem \ref{thm:limit_algebra} because the denominator is 0.

\mfigure{0in}{Graphing $f$ in Example \ref{ex_limit_onept} to understand a limit.}{fig:limitxplus1}{figures/fig_LimitXplus1}
...
then $(x-c)$ is a factor of both $g(x)$ and $f(x)$. One can then use algebra to factor this term out, divide, then apply Theorem \ref{thm:limit_allbut1}. Some useful algebraic techniques to rewrite functions that return an indeterminate form when evaluating a limit are:
\begin{itemize}
...
Notice that we didn't distribute the denominator in the second line.  Generally speaking, when we are hoping to cancel a factor in a fraction we will need to undo any distributing that we may have prematurely done.
...
\begin{align*}
	...
	&= \lim_{h\to 0}(-1.5h+8.5) \qquad (\text{\small using Theorem \ref{thm:limit_allbut1}, as $h\neq 0$}) \\
	&= 	8.5 \qquad (\text{\small using Theorem \ref{thm:lim_continuous}})
\end{align*}	
