...
\end{tikzpicture}}{A line segment from (-3,-1) to (3,2) with a hollow dot at (1,1).  There is also a solid dot at (1,2).}
&
\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=.8\marginparwidth, axis y line=middle,axis x line=middle, ymin=-3, ymax=3, xmin=-3, xmax=3, name=myplot]
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A curve starting near (-3,0) and then going upward as x approaches 1 from the left.  A separate line segment connects a solid dot at (1,2) to (3,0).}
&
\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=.8\marginparwidth, axis y line=middle,axis x line=middle, ymin=-3, ymax=3, xmin=-3, xmax=3, name=myplot]
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A line segment connecting (-3,-1) to a hollow dot at (1,1).  A separate line segment connects a solid dot at (1,2) to (3,0).}
\\
...
%
\mtable{A graph of $f$ in \autoref{ex_contint1}.}{fig_continuous1}{\pdftooltip{\begin{tikzpicture}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A curve from a solid dot at (0,1.5) to a hollow dot at (1,1), then a horizontal line to (2,1), then a curve to (3,1).}}
%
\solution
...
\mtable{A graph of the step function in \autoref{ex_contint2}.}{fig:continuous2}{\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
minor x tick num=1,axis y line=middle,axis x line=middle,
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{For each integer from -2 to 2 inclusive, a horizontal line segment from a solid dot at (n,n) to a hollow dot at (n+1,n).}}
...
\mtable{A graph of $f$ in\\\autoref{ex_cont_funct}(1).}{fig_continuous3}{\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
minor x tick num=1,axis y line=middle,axis x line=middle,
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{An upside down U shape, connecting solid dots at (1,2) and (5,2).}}
...
\end{tikzpicture}}{A curve that goes through the points (a,f(a), (c,L), and then (b,f(b))}
\\ (a) \\
\pdftooltip{\begin{tikzpicture}
...
\end{tikzpicture}}{A line segment connecting (a,f(a)) to a point that is below the line y=L.  A separate line segment starts above the line y=L (but at the same x coordinate where the previous line ended) and ends at the point (b,f(b)).}
\\ (b)}
...
$f(x) = x-\cos x$.}{fig:xminuscosx}{\pdftooltip{\begin{tikzpicture}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A curve starting at (0,-1) and going to (1,0.5).}}
