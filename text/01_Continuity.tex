...
\mtable{A graph of $f$ in \autoref{ex_contint1}.}{fig_continuous1}{\begin{tikzpicture}
...
\mtable{A graph of the step function in \autoref{ex_contint2}.}{fig:continuous2}{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
minor x tick num=1,axis y line=middle,axis x line=middle,
...
\definition{def:left_right_cont}{Right and Left Continuity}{%
\textbf{Right Continuous}

Let $f$ be defined on a closed interval with left endpoint $a$. We say that $f$ is \textbf{continuous from the right at $a$} (or \textbf{right continuous at $a$}) if  \[\lim_{x\to a^+} f(x)=f(a).\vspace{-\baselineskip}\]\index{continuity!right}
...
\mtable{A graph of $f$ in \autoref{ex_cont_funct}(1).}{fig_continuous3}{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
minor x tick num=1,axis y line=middle,axis x line=middle,
...
\mtable[-1in]{A situation where the Intermediate Value Theorem applies (top) and does not (bottom).}{fig_ivt}{%
\begin{tikzpicture}
...
\theorem{thm:IVT}{Intermediate Value Theorem}{Let $f$ be a continuous function on $[a,b]$ and, without loss of generality, let $f(a) < f(b)$. Then for every value $y$, where $f(a) < y < f(b)$, there exists at least one value $c$ in $(a,b)$ such that $f(c) = y$\index{Intermediate Value Theorem}}
...
$f(x) = x-\cos x$.}{fig:xminuscosx}{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
