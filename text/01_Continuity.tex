...
\definition{def:left_right_cont}{Left and Right Continuity}{Let $f$ be defined on an open interval that contains $a$.  We say that $f$ is \textbf{continuous from the right at $a$} (or \textbf{right continuous at $a$}) if $\ds \lim_{x\to a^+} f(x)=f(a)$.  We say that $f$ is \textbf{continuous from the left at $a$} (or \textbf{left continuous at $a$}) if  $\ds \lim_{x\to a^-} f(x)=f(a)$.}
