...
%{\mtable{Illustrating the $\epsilon-\delta$ process.}{fig:choose_e_d}{%
		...
		%}
%}
\mtable[2in]{Illustrating the $\epsilon-\delta$ process.}{fig:choose_e_d}{%
%		\begin{tabular}{c} 
		\myincludegraphics{figures/figLimitProof1a}\\
		\myincludegraphics{figures/figLimitProof1b}\\
		\noindent
...
\example{ex_compute_limNew8}{Evaluating a limit using the definition}{Show that $\ds \lim_{x\to 2} 4-\frac{3}{2}x=1$.}{Scratch-Work:
...
which is what we wanted to show. Thus  $\ds \lim_{x\to 2} 4-\frac{3}{2}x=1$.\qedhere
\end{proof}}
...
\mfigure{0in}{Choosing $\delta = \epsilon/5$ in Example \ref{ex_compute_lim2}.}{fig:limit_eover5}{figures/figlimit_proof2a}
...
\ifthenelse{\boolean{latexml}}{The portions of the graph outside of the $\epsilon$ tolerance are highlighted in red.  If you get everything within $\epsilon$, then the graph turns green.  You can also put in a piecewise defined function using \texttt{If[~condition~,~then~,~else~]}, such as \texttt{If[~x>0~,~x\^{}2~,~-x~]}.  (If you have trouble moving $L$ around, you may need to use the up and down arrows on your keyboard.)}{\mnote{\hfill\qrcode{http://ggbm.at/RtY27ybS}\hfill}}
