...
%{\mtable{.4}{Illustrating the $\epsilon-\delta$ process.}{fig:choose_e_d}{%
		...
		%}
%}
\mtable{.6}{Illustrating the $\epsilon-\delta$ process.}{fig:choose_e_d}{%
%		\begin{tabular}{c} 
		\myincludegraphics{figures/figLimitProof1a}\\
		\myincludegraphics{figures/figLimitProof1b}\\
		\noindent
...
\begin{equation} |x^2-4| < \epsilon \longrightarrow |x-2|\cdot|x+2| < \epsilon \longrightarrow |x-2| < \frac{\epsilon}{|x+2|}\text{.}\label{eq:limit1}\end{equation} 
...
\frac\epsilon5<&\frac{\epsilon}{|x+2|}\text{.}\label{eq:limit2}
\end{align}
...
\text{\small(Using \eqref{eq:limit2} as long as $\delta <1$)}
