...
\begin{tikzpicture}
 \begin{axis}[width=\marginparwidth+25pt,tick label style={font=\scriptsize },
  ...
  \addplot [{\colorone},smooth,thick,domain=-1:-.02] {sin(deg(x))/x};
  \addplot [{\colorone},smooth,thick,domain=.02:1] {sin(deg(x))/x};
...
\mtable{Values of $\sin(x)/x$ with $x$ near 1 and near 0.}{table:sinx_1}{\begin{tabular}{c}
\input{figures/sinx_over_x_table_1} \\
(a) \\\addlinespace
\input{figures/sinx_over_x_table_2} \\
(b)
\end{tabular}}
...
{To graphically approximate the limit, graph \[y = (x^2-x-6)/(6x^2-19x+3)\] on a small interval that contains 3. To numerically approximate the limit, create a table of values where the $x$ values are near 3. This is done in \autoref{fig:limit1}.

\mtable{Graphically and numerically approximating a limit in \autoref{ex_limit1}.}{fig:limit1}{\begin{tabular}{c}
...
\mtable{Graphically and numerically approximating a limit in \autoref{ex_limit2}.}{fig:limit2}{\begin{tabular}{c}
...
%
\mtable[-3in]{Graphically and numerically observing no limit as $x\to 1$ in \autoref{ex_no_limit1}.}{fig:nolimit1}{\begin{tabular}{c}
...
\mtable{Graphically and numerically observing no limit as $x\to 1$ in \autoref{ex_no_limit2}.}{fig:nolimit2}{\begin{tabular}{c}
...
(a) & (b) & (c)
\end{tabular}}%
\captionof{figure}{Observing that $f(x) = \sin(1/x)$ has no limit as $x\to 0$ in \autoref{ex_no_limit3}.}%
\label{fig:nolimit3}%
}%
\end{lxfigure}
...
\mfigure{-.75in}{Interpreting a difference quotient as the slope of a secant line.}{fig:diffquot1}{figures/figDiffQuot1}
...
{% is longpage
	...
	\end{tabular}
	\begin{tabular}{c}
		(c)\myincludegraphics{figures/figDiffQuotSmallhc}
	\end{tabular}
...
{% isn't longpage
	\mtable[-1.5in]{Secant lines of $f(x)$ at $x=1$ and $x=1+h$, for shrinking values of $h$ (i.e., $h\rightarrow 0$).}{fig:diff_quot_small_h}{\begin{tabular}{c}
		\myincludegraphics{figures/figDiffQuotSmallha}\\ (a)\\ \myincludegraphics{figures/figDiffQuotSmallhb} 
		\\ (b)\\
		\myincludegraphics{figures/figDiffQuotSmallhc}\\(c)\end{tabular}%
	}
