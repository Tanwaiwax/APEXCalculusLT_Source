...
While our question is not precisely formed (what constitutes ``near the value 1''?), the answer does not seem difficult to find. One might think first to look at a graph of this function to approximate the appropriate $y$ values. Consider Figure \ref{fig:sinx_over_x}(a), where $y = \frac{\sin x}{x}$ is graphed. For values of $x$ near 1, it seems that $y$ takes on values near $0.85$. In fact, when $x=1$, then $y=\frac{\sin 1}{1} \approx 0.84$, so it makes sense that when $x$ is ``near'' 1, $y$ will be ``near'' $0.84$.

\mtable[1.5in]{$\sin(x)/x$ near $x=1$ (top) and $x=0$ (bottom).}{fig:sinx_over_x}%
{\begin{tabular}{c}
\myincludegraphics{figures/figZoomSinXOverX}\\ (a)\\
\begin{tikzpicture}
...
\addplot [{\colorone},smooth,thick,domain=-1:-.03] {sin(deg(x))/x};
\addplot [{\colorone},smooth,thick,domain=.03:1] {sin(deg(x))/x};
...
Consider this again at a different value for $x$. When $x$ is near 0, what value (if any) is $y$ near? By considering Figure \ref{fig:sinx_over_x}(b), one can see that it seems that $y$ takes on values near $1$. But what happens when $x=0$? We have $$ y \rightarrow \frac{\sin 0}{0} \rightarrow \raisebox{8pt}{\text{``\ }}\frac{0}{0}\raisebox{8pt}{\text{\ ''}}.$$ 
...
\mtable{Values of $\sin(x)/x$ with $x$ near 1.}{table:sinx_1}{\input{figures/sinx_over_x_table_1}}

%\vskip \baselineskip

Now approximate $\lim_{x\to 0} \sin(x)/x$ numerically. We already approximated the value of this limit as 1 graphically in Figure \ref{fig:sinx_over_x}(b). The table in Figure \ref{table:sinx_2} shows the value of $\sin(x)/x$ for values of $x$ near 0. Ten places after the decimal point are shown to highlight how close to 1 the value of $\sin(x)/x$ gets as $x$ takes on values very near 0. We include the $x=0$ row in bold again to stress that we are not concerned with the value of our function at $x=0$, only on the behavior of the function \textit{near} 0. 

\mtable{Values of $\sin(x)/x$ with $x$ near 0.}{table:sinx_2}{\input{figures/sinx_over_x_table_2}}
...
\khanVideo{riXcZT2ICjA}{introduction-to-limits-hd}{\textbf{Intro to limits:} What are limits?}

We now consider several examples that allow us to explore different aspects of the limit concept.\\

\mtable[2in]{Graphically and numerically approximating a limit in Example \ref{ex_limit1}.}{fig:limit1}{\begin{tabular}{c}
\myincludegraphics{figures/figlimit1} \\ (a) \\
\input{figures/table_limit1} \\ (b)
\end{tabular}}
%\mfigure{0in}{Graphically approximating a limit in Example \ref{ex_limit1}.}{fig:limit1}{figures/figlimit1}
%\mtable[1in]{Numerically approximating a limit in Example \ref{ex_limit1}.}{table:limit1}{\input{figures/table_limit1}}
...
{To graphically approximate the limit, graph \[y = (x^2-x-6)/(6x^2-19x+3)\] on a small interval that contains 3. To numerically approximate the limit, create a table of values where the $x$ values are near 3. This is done in Figure \ref{fig:limit1}. % and \ref{table:limit1}, respectively.
...
Graphically and numerically approximate the limit of $f(x)$ as $x$ approaches 0, where $$f(x) = \begin{cases} x+1 & x< 0 \\ -x^2+1 & x > 0 \end{cases}.$$}{Again we graph $f(x)$ and create a table of its values near $x=0$ to approximate the limit. Note that this is a piecewise defined function, so it behaves differently on either side of 0. Figure \ref{fig:limit2}(a) shows a graph of $f(x)$, and on either side of 0 it seems the $y$ values approach 1. Note that $f(0)$ is not actually defined, as indicated in the graph with the open circle.

\mtable{Graphically and numerically approximating a limit in Example \ref{ex_limit2}.}{fig:limit2}{\begin{tabular}{c}
\myincludegraphics{figures/figlimit2} \\ (a) \\
\input{figures/table_limit2} \\ (b)
\end{tabular}}
%\mfigure{0in}{Graphically approximating a limit in Example \ref{ex_limit2}.}{fig:limit2}{figures/figlimit2}
%\mtable[1in]{Numerically approximating a limit in Example \ref{ex_limit2}.}{table:limit2}{\input{figures/table_limit2}}

The table shown in Figure \ref{table:limit2}(b) shows values of $f(x)$ for values of $x$ near 0. It is clear that as $x$ takes on values very near 0, $f(x)$ takes on values very near 1. It turns out that if we let $x=0$ for either ``piece'' of $f(x)$, 1 is returned; this is significant and we'll return to this idea later.
...
A function may not have a limit for all values of $x$. That is, we cannot say $\ds\lim_{x\to c}f(x)=L$ for some numbers $L$ for all values of $c$, for there may not be a number that $f(x)$ is approaching. There are three ways in which a limit may fail to exist. \index{limit!does not exist}
\begin{enumerate}
...
{A graph of $f(x)$ around $x=1$ and a table are given in Figure \ref{fig:nolimit1}. % and \ref{table:nolimit1}, respectively. 
...
\mtable[-2in]{Graphically and numerically observing no limit as $x\to 1$ in Example \ref{ex_no_limit1}.}{fig:nolimit1}{\begin{tabular}{c}
\myincludegraphics{figures/fignolimit1} \\ (a) \\
\input{figures/table_nolimit1} \\ (b)
\end{tabular}}
%\mfigure{0in}{Observing no limit as $x\to 1$ in Example \ref{ex_no_limit1}.}{fig:nolimit1}{figures/fignolimit1}
%\mtable[1in]{Values of $f(x)$ near $x=1$ in Example \ref{ex_no_limit1}.}{table:nolimit1}{\input{figures/table_nolimit1}}
...
{A graph and table of $f(x) = 1/(x-1)^2$ are given in Figure \ref{fig:nolimit2}. % and \ref{table:nolimit2}, respectively. 
Both show that as $x$ approaches 1, $f(x)$ grows larger and larger.
\mtable[1in]{Graphically and numerically observing no limit as $x\to 1$ in Example \ref{ex_no_limit2}.}{fig:nolimit2}{\begin{tabular}{c}
\myincludegraphics{figures/fignolimit2} \\ (a) \\
\input{figures/table_nolimit2} \\ (b)
\end{tabular}}
%\mfigure{1in}{Observing no limit as $x\to 1$ in Example \ref{ex_no_limit2}.}{fig:nolimit2}{figures/fignolimit2}
%\mtable[2in]{Values of $f(x)$ near $x=1$ in Example \ref{ex_no_limit2}.}{table:nolimit2}{\input{figures/table_nolimit2}}
...
{%\mfigure{0in}{Observing no limit as $x\to 0$ in Example \ref{ex_no_limit3}.}{fig:nolimit3a}{figures/figNoLimit3a}
%\mfigure{0in}{Zooming in to observing no limit as $x\to 0$ in Example \ref{ex_no_limit3}.}{fig:nolimit3b}{figures/figNoLimit3b}
...
{\ifthenelse{\boolean{latexml}\OR\isodd{\pageref{fig:nolimit3}}}{%
% do nothing
}{%
...
\begin{tabular}{ccc}
\myincludegraphics{figures/figNoLimit3a} & \myincludegraphics{figures/figNoLimit3b} &
\begin{tabular}[b]{cc}
 $x$ & $\sin(1/x)$ \\ \hline 0.1 & $-0.544021$ \\ 0.01 & $-0.506366$ \\ 0.001 & 0.82688 \\ 0.0001 & $-0.305614$ \\ $1.\times 10^{-5}$ & 0.0357488 \\
...
\mfigure{0in}{Interpreting a difference quotient as the slope of a secant line.}{fig:diffquot1}{figures/figDiffQuot1}
...
%			\noindent\begin{minipage}{\textwidth+\marginparwidth+\marginparsep}%\centering
			\mtable[-1in]{Secant lines of $f(x)$ at $x=1$ and $x=1+h$, for shrinking values of $h$ (i.e., $h\rightarrow 0$).}{fig:diff_quot_small_h}{\begin{tabular}{c}
...
%\ifthenelse{\boolean{longpage}}{\mtable{The difference quotient evaluated at values of $h$ near 0.}{table:diff_quot_smallh}{\begin{tabular}{cc}$h$ & $\frac{f(1+h)-f(1)}{h}$\vspace{1pt} \\ \hline $-0.5$ & 9.25 \\ $-0.1$ & 8.65 \\ $-0.01$ & 8.515 \\ 0.01 & 8.485 \\ 0.1 & 8.35 \\ 0.5 & 7.75 \end{tabular}} }
%{}
