...
\mtable[-3\baselineskip]{$\sin(x)/x$ near $x=1$ (top) and $x=0$ (bottom).}{fig:sinx_over_x}%
{\begin{tikzpicture}
...
\mtable{Graphically and nu\-mer\-i\-cal\-ly approximating a limit in \autoref{ex_limit1}.}{fig:limit1}{\begin{tikzpicture}
...
\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},minor x tick num=1,axis y line=middle,axis x line=middle,ymin=-1,ymax=110,xmin=-.1,xmax=2.1,name=myplot]
%
...
\mtable{Interpreting a difference quo\-tient as the slope of a secant line.}{fig:diffquot1}{\begin{tikzpicture}
...
\mtable[-3\baselineskip]{Secant lines of $f(x)$ at $x=1$ and $x=1+h$, for shrinking values of $h$ (i.e., $h\rightarrow 0$).}{fig:diff_quot_small_h}{%
\begin{tikzpicture}
...
\mtable[.25in]{The difference quotient for $f(x)=-1.5x^2+11.5x$ evaluated at values of $h$ near 0.}{table:diff_quot_smallh}{\begin{tabular}{lc}
