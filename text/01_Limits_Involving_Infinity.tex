...
\example{ex_inflim1}{Evaluating limits involving infinity}{Find $\displaystyle \lim_{x\rightarrow 1}\frac1{(x-1)^2}$ as shown in \autoref{fig:inflim1}.
...
\mfigure{0in}{Graphically showing that $f(x) = \dfrac{x^2-1}{x-1}$ does not have an asymptote at $x=1$.}{fig:noasy}{figures/fignoasy}
...
\item We say $\ds\lim_{x\to\infty} f(x)=L$ if for every $\epsilon>0$ there exists $M>0$ such that if $x\geq M$, then $|f(x)-L|<\epsilon$.\index{limit!at infinity}

\item We say $\ds\lim_{x\to-\infty} f(x)=L$ if for every $\epsilon>0$ there exists $M<0$ such that if $x\leq M$, then $|f(x)-L|<\epsilon$.
...
\]
%
\mtable[-2.5in]{Using a graph and a table to approximate a horizontal asymptote in \autoref{ex_hzasy1}.}{fig:hzasy1}{%
\begin{tabular}{c}
...
(a) & (b) & (c)
\end{tabular}}
\captionof{figure}{Considering different types of horizontal asymptotes.}
\label{fig:hzasy}
\end{lxfigure}
...
\mtable[-2in]{Visualizing the functions in \autoref{ex_hzasy3}.}{fig:hzasy3}{%
\begin{tabular}{c}
