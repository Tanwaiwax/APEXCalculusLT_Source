...
As a motivating example, consider $f(x) = 1/x^2$, as shown in \autoref{fig:oneoverxsquared}. Note how, as $x$ approaches 0, $f(x)$ grows very, very large. It seems appropriate, and descriptive, to state that $$\lim_{x\rightarrow 0} \frac1{x^2}=\infty.$$ Also note that as $x$ gets very large, $f(x)$ gets very, very small. We could represent this concept with notation such as $$\lim_{x\rightarrow \infty} \frac1{x^2}=0.$$

\mfigure{0in}{Graphing $f(x) = 1/x^2$ for values of $x$ near 0.}{fig:oneoverxsquared}{figures/figoneoverxsquared}
...
\example{ex_inflim1}{Evaluating limits involving infinity}{Find $\ds \lim_{x\to1}\frac1{(x-1)^2}$ as shown in \autoref{fig:inflim1}.
\mfigure{0in}{Observing infinite limit as $x\to 1$ in \autoref{ex_inflim1}.}{fig:inflim1}{figures/fignolimit2}}
...
\example{ex_inflim2}{Evaluating limits involving infinity}{Find $\ds\lim_{x\to0}\frac1x$, as shown in \autoref{fig:oneoverx}.
\mfigure{0in}{Evaluating $\ds\lim_{x\rightarrow 0}\frac1x$.}{fig:oneoverx}{figures/figoneoverx}}
...
\definition{def:vert_asymp}{Vertical Asymptote}
...
\mfigure{0in}{Graphically showing that\\
$f(x)=\dfrac{x^2-1}{x-1}$ does not have an asymptote at $x=1$.}{fig:noasy}{figures/fignoasy}

While the denominator does get small near $x=1$, the numerator gets small too, matching the denominator step for step. In fact, factoring the numerator, we get
...
%\setboxwidth{65pt}%
\definition{def:limit_at_infinity}{Limits at Infinity}
{~\\[-2\baselineskip]\begin{enumerate}
\item We say $\ds\lim_{x\to\infty} f(x)=L$ if for every $\epsilon>0$ there exists $M>0$ such that if $x\geq M$, then $\abs{f(x)-L}<\epsilon$.\index{limit!at infinity}
...
\definition{def:horiz_asymp}{Horizontal Asymptote}{The function $f(x)$ has a \textbf{horizontal asymptote at} $\mathbf{y=L}$ if either
...
\example{ex_hzasy1}{Approximating horizontal asymptotes}{Approximate the horizontal asymptote(s) of $\ds f(x)=\frac{x^2}{x^2+4}$.}
...
%\mfigure{0in}{Graphing $f(x) = \dfrac{x}{x^2+1}$}{fig:hzasy2}{figures/fighzasy2}

\autoref{fig:hzasy}(b) shows that $f(x) =x/\sqrt{x^2+1}$ has two horizontal asymptotes; one at $y=1$ and the other at $y=-1$.

%\mfigure{0in}{Graphing $f(x) = \dfrac{x}{\sqrt{x^2+1}}$ to see its two horizontal asymptotes.}{fig:hzasy3}{figures/fighzasy3}
...
\flushinner{%
\begin{tabular}{ccc}
\myincludegraphics{figures/fighzasy2} &
\myincludegraphics{figures/fighzasy3} &
\myincludegraphics{figures/fighzasy4} \\
(a) & (b) & (c)
\end{tabular}}
...
\keyidea{key:rat_lim_at_inf}{Finding Limits of Rational Functions at Infinity}{Let $f(x)$ be a rational function of the following form:
\[
f(x)
...
\keyidea{thm:lim_rational_fn_at_infty}{Limits of Rational Functions at Infinity}
...
\example{ex_sqrt_asy}{Horizontal Asymptotes Involving Square Roots}{Find the horizontal asymptotes of $\dfrac{x}{\sqrt{x^2+1}}$.}{We must consider the limits as $x\to \pm \infty$. When $x$ is very large, $x^2+1\approx x^2$ and thus $\sqrt{x^2+1}\approx \sqrt{x^2}=\abs x$.
\begin{align*}
...
\example{ex_hzasy2}{Finding a limit of a rational function}{Confirm analytically that $y=1$ is the horizontal asymptote of $\ds f(x) = \frac{x^2}{x^2+4}$, as approximated in \autoref{ex_hzasy1}.}
{Before using \autoref{thm:lim_rational_fn_at_infty}, let's use the technique of evaluating limits at infinity of rational functions that led to that theorem. The largest power of $x$ in $f$ is 2, so divide the numerator and denominator of $f$ by $x^2$, then take limits.
\begin{align*}
...
\example{ex_hzasy3}{Finding limits of rational functions}{(a) Analytically evaluate the following limits, and (b) Use \autoref{thm:lim_rational_fn_at_infty} to evaluate each limit.\\
	...
	\item	\begin{enumerate}
		\item Divide numerator and denominator by $x^3$.
			\begin{align*}
				...
				&=\frac{0+0+0}{1+0}=0
			\end{align*}
\mtable[-1in]{Visualizing the functions in \autoref{ex_hzasy3}.}{fig:hzasy3}{%
\begin{tabular}{c}
	\myincludegraphics{figures/fighzasy5} \\ (a) \\[10pt]
	\myincludegraphics{figures/fighzasy6} \\ (b) \\[10pt]
	\myincludegraphics{figures/fighzasy7} \\ (c)
\end{tabular}}
...
% todo Maybe some practical example here, like the cost to remove 100% of something and vertical asymptotes and the the long term limit of a population to demonstrate limits at infinity.  Or maybe just put these into the exercises.
