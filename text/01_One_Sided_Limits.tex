...
Let $f$ be a function defined on an open interval $I$ containing $c$, except possibly at $c$. \index{limit!does not exist} Then \[\lim_{x\to c}f(x) = L\] if, and only if, \[\lim_{x\to c^-}f(x) = L \quad \text{and} \quad \lim_{x\to c^+}f(x) = L.\]
\end{theorem}
...
\end{multicols}
%
\mtable[1in]{A graph of $f$ from \autoref{ex_onesideb}}{fig:onesidedb}{\begin{tikzpicture}
...
%
\mtable[-1in]{Graphing $f$ in \autoref{ex_onesidec}}{fig:onesidedc}{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
minor x tick num=1,axis y line=middle,axis x line=middle,
