...
\mtable{$f(x) = \cos x^2$ sketched along with its tangent line at $x=1$.}{fig:chain7}{\begin{tikzpicture}
...
\addplot [draw={\colorone},smooth,thick,domain=-3.14:3.14,samples=40] {cos(deg(x*x))};
...
\mtable[-.5in]{A series of gears to demonstrate the Chain Rule. Note how $\frac{dy}{dx} = \frac{dy}{du}\cdot\frac{du}{dx}$}{fig:chainrulegears}{\begin{tikzpicture}[>=latex,scale=.8]
