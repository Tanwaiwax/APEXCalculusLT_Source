...
\example{ex_chain1}{Exploring similar derivatives}{Find the derivatives of $F_1(x) = (1-x)^2$, $F_2(x) = (1-x)^3,$ and $F_3(x) = (1-x)^4.$ (We'll see later why we are using subscripts for different functions and an uppercase $F$.)}
...
\example{ex_chain2}{Using the Chain Rule}{Use the Chain Rule to find the derivatives of the functions given in \autoref{ex_chain1}.}
...
\theorem{thm:gen_power_rule}{Generalized Power Rule}{Let $g(x)$ be a differentiable function.\index{derivative!Generalized Power Rule}\index{Generalized Power Rule} Then \[\frac{d}{dx}\Big(g(x)^n\Big) = n\cdot \big(g(x)\big)^{n-1}\cdot g\primeskip'(x).\]}
...
\example{ex_chain3}{Using the Chain Rule}{Find the derivatives of the following functions:
\[
...
\mfigure{0in}{$f(x) = \cos x^2$ sketched along with its tangent line at $x=1$.}{fig:chain7}{figures/figchain7}

\example{ex_chain7}{Using the Chain Rule to find a tangent line}{Let $f(x) = \cos x^2$. Find the equation of the line tangent to the graph of $f$ at $x=1$.}
...
\example{ex_chain4}{Using the Product, Quotient and Chain Rules}{Find the derivatives of the following functions.
\[
...
\example{ex_chain6}{Using the Chain Rule multiple times}{Find the derivative of $y = \tan^5(6x^3-7x)$.}
...
\example{ex_chain5}{Using the Product, Quotient and Chain Rules}{Find the derivative of $f(x)=\dfrac{x\cos(x^{-2})-\sin^2(e^{4x})}{\ln x^2}$.}
...
%
%\example{ex_chain8}{The Chain Rule and exponential functions}{Use the Chain Rule to find the derivative of $y= a^x$ where $a>0$, $a\neq 1$ is constant.}
...
%If you look at it like a fraction it sort of looks like the $dx$ part ``divides out'' leaving $\displaystyle \frac{dy}{dt}.$  This isn't quite right, but it does work.  In fact you can extend this as
\[
 \frac{dy}{dt}
...
One of the most common ways of ``visualizing'' the Chain Rule is to consider a set of gears, as shown in \autoref{fig:chainrulegears}.
%
\mfigure{-1in}{A series of gears to demonstrate the Chain Rule. Note how $\frac{dy}{dx} = \frac{dy}{du}\cdot\frac{du}{dx}$}{fig:chainrulegears}{figures/figchainrulegears}
%
The gears have 36, 18, and 6 teeth, respectively. That means for every revolution of the $x$ gear, the $u$ gear revolves twice. That is, the rate at which the $u$ gear makes a revolution is twice as fast as the rate at which the $x$ gear makes a revolution. Using the terminology of calculus, the rate of $u$-change, with respect to $x$, is $\frac{du}{dx} = 2$. 
