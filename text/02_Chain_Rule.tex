...
\[\frac{d}{dx}\Bigl(g(x)^n\Bigr) = n\cdot \bigl(g(x)\bigr)^{n-1}\cdot g\primeskip'(x).\]
\end{theorem}
...
\begin{enumerate}
	...
	\[
	 y\primeskip'=e^{x^2}\cdot\frac{d}{dx}(x^2)=e^{-x^2}\cdot(-2x)=(-2x)e^{-x^2}.
	\]
\end{enumerate}
\end{example}
...
This pattern recognition process can be applied to lots of functions. In general, instead of writing ``anything'', we use $u$ as a generic function of $x$. We then say \[\frac{d}{dx}\Bigl(\ln u\Bigr) = \frac{u\primeskip'}{u}.\]
The following is a short list of how the Chain Rule can be quickly applied to familiar functions.

\begin{multicols}{2}
\begin{enumerate}
	\item	$\ds\frac{d}{dx}\Bigl(u^n\Bigr) = n\cdot u^{n-1}\cdot u\primeskip'.$
	\item	$\ds\frac{d}{dx}\Bigl(e^u\Bigr) = u\primeskip'\cdot e^u.$
	\item	$\ds\frac{d}{dx}\Bigl(\sin u\Bigr) = u\primeskip'\cdot \cos u.$
	\item	$\ds\frac{d}{dx}\Bigl(\cos u\Bigr) = -u\primeskip'\cdot \sin u.$
	\item	$\ds\frac{d}{dx}\Bigl(\tan u\Bigr) = u\primeskip'\cdot \sec^2 u.$
	\item[]\bigskip
\end{enumerate}
\end{multicols}
...
It is important to realize that we \emph{are not} dividing these terms; the derivative notation of $\frac{dy}{dx}$ is one symbol. It is equally important to realize that this notation was chosen precisely because of this behavior. It makes applying the Chain Rule easy with multiple variables. For instance,
%
