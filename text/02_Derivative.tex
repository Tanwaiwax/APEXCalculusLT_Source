...
Let $f$ be a continuous function on an open interval $I$ and let $c$ be in $I$.\index{derivative!at a point} The \sword{derivative of $f$ at $c$}, denoted $\fp(c)$, is \[\lim_{h\to 0}\frac{f(c+h)-f(c)}{h},\] provided the limit exists.
\end{definition}
...
\mtable{A graph of $f(x) = 3x^2+5x-7$ and its tangent lines at $x=1$ and $x=3$.}{fig:tangent1}{\begin{tikzpicture}
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
minor x tick num=1,axis y line=middle,axis x line=middle,
...
%
\mtable{$f(x) = \sin x$ graphed with an approximation to its tangent line at $x=0$.}{fig:tangentsinx}{\begin{tikzpicture}
...
axis x line=middle,ymin=-1.1,ymax=1.1,xmin=-3.5,xmax=3.5,name=myplot]
...
is \sword{the derivative of $f$}.\index{derivative!as a function}\index{derivative!notation}\bigbreak
...
% todo Tim should the graph of sin and its derivative use equal aspect ratio? check that it worked
\begin{center}
\captionsetup{type=figure}%
\begin{tikzpicture}
\begin{axis}[tick label style={font=\scriptsize}, axis y line=middle,axis x line=middle, ymin=-1.15, ymax=1.15,xmin=-7,xmax=7,name=myplot,axis equal,
...
\mtable{The absolute value function, $f(x) = \abs{x}$. Notice how the slope of the lines (and hence the tangent lines) abruptly changes at $x=0$.}{fig:absolutevalue}{\begin{tikzpicture}
...
When $x<0$:
	\begin{align*}
	\frac{d}{dx}\bigl(-x\bigr)
	&= \lim_{h\to 0}\frac{-(x+h) - (-x)}{h} \\
	&=	\lim_{h\to 0}\frac{-h}{h}\\
	&=	\lim_{h\to 0}-1 \\
	&=	-1.
	\end{align*}
When $x>0$, a similar computation shows that $\ds \frac{d}{dx}\bigl(x\bigr) = 1$.
...
\end{minipage}\\
%
\mtable[-1in]{A graph of the derivative of $f(x) = \abs{x}$.}{fig:absolutevalueprime}{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
minor x tick num=1,axis y line=middle,axis x line=middle,
...
\mtable{A graph of $f(x)$ as defined in \autoref{ex_diff_piecewise}.}{fig:piecewisesinx1}{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
...
\mtable{A graph of $\fp(x)$ in \autoref{ex_diff_piecewise}.}{fig:piecewisecosx1}{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
minor x tick num=1,xtick={1.57},xticklabels={$\frac{\pi}2$},axis y line=middle,
axis x line=middle,ymin=-.4,ymax=1.4,xmin=-.1,xmax=2.1,name=myplot]
