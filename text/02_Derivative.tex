...
%
\mtable{Computing the difference quotient.}{fig:diff_quotient}{\pdftooltip{\begin{tikzpicture}
...
\end{tikzpicture}}{A curve from (0,150) to (3,0), with a line segment meeting this curve near (2,100) and (2.4,80).  These intersections differ by h in the x direction, and f(2+h)-f(2) in the y direction.}}
%
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{The same curve from (0,150) to (3,0), with a line segment meeting this curve near (2,100) and (3,0).}
		&
\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
minor x tick num=1,axis y line=middle,axis x line=middle,
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{The same as the previous picture, but zoomed in on the two intersection points.}
\\(a) & (b) \\\addlinespace
\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
minor x tick num=1,axis y line=middle,axis x line=middle,
...
\mtable{A graph of $f(x) = 3x^2+5x-7$ and its tangent lines at $x=1$ and $x=3$.}{fig:tangent1}{\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
minor x tick num=1,axis y line=middle,axis x line=middle,
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A curve from (0,-7) up to (4,61) with two tangent lines.}}
...
%\mtable{A graph of $f(x)=3x^2+5x-7$, along with its normal line at $x=1$.}{fig:normal1}{\pdftooltip{\begin{tikzpicture}
...
%\node [right] at (myplot.right of origin) {\scriptsize $x$};
%\node [above] at (myplot.above origin) {\scriptsize $y$};
%\end{tikzpicture}}{A line increasing steeply up and right, intersected at right angles by another line decreasing slowly down and right.  The intersection occurs at (1,1).}}
%
...
%
\mtable{$f(x) = \sin x$ graphed with an approximation to its tangent line at $x=0$.}{fig:tangentsinx}{\pdftooltip{\begin{tikzpicture}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A wave that starts at (-π,0), goes down to (-π/2,-1), up to the origin and then (π/2,1), and back down to (π,0).  There is also a line similar to y=x that passes through the origin.}}
%
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{Two wave functions overlapping.}
...
\mtable{The absolute value function, $f(x) = \abs{x}$. Notice how the slope of the lines (and hence the tangent lines) abruptly changes at $x=0$.}{fig:absolutevalue}{\pdftooltip{\begin{tikzpicture}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A line segment from (-1,1) down to the origin and then up to (1,1).}}
...
\end{minipage}\\
%
\mtable{A graph of the derivative of $f(x) = \abs{x}$.}{fig:absolutevalueprime}{\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
minor x tick num=1,axis y line=middle,axis x line=middle,
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A horizontal line segment from (-1,-1) to a hollow dot at (0,-1), and then another horizontal line segment from a hollow dot at (0,1) to (1,1).}}
%
\noindent
...
%
\mtable{A graph of $f(x)$ as defined in \autoref{ex_diff_piecewise}.}{fig:piecewisesinx1}{\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A curve starting at the origin, going upward to (π/2,1) where it flattens out and becomes horizontal.}}
%
\[
...
\mtable{A graph of $\fp(x)$ in \autoref{ex_diff_piecewise}.}{fig:piecewisecosx1}{\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
minor x tick num=1,xtick={1.57},xticklabels={$\frac{\pi}2$},axis y line=middle,
axis x line=middle,ymin=-.4,ymax=1.4,xmin=-.1,xmax=2.1,name=myplot]
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A curve starting at (0,1) moving downward to (π/2,0) where it abruptly changes direction to become horizontal.}}
