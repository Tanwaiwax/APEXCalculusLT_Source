...
%	\flushinner{%
	\begin{tabular}{cc}
		...
		\myincludegraphics{figures/figderivfalling2}\\
		(a) & (b) \\\addlinespace
		\myincludegraphics{figures/figderivfalling3} &
		\myincludegraphics{figures/figderivfalling4}\\
		(c) & (d)\\
	\end{tabular}%}
...
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}
\captionof{figure}{The function $f(x)=\sin x$ and its derivative $\fp(x)=\cos x$.}
