...
{We apply Definition \ref{def:the_derivative}.
	\begin{align*}
		\fp(x) 	&= \lim_{h\to 0} \frac{f(x+h)-f(x)}{h}\\
						&=	\lim_{h\to 0} \frac{\frac{1}{x+h+1}-\frac{1}{x+1}}{h} \intertext{Now find a common denominator and subtract; factor $1/h$ out front to facilitate reading.}
...
We have found that when $f(x) = \sin x$, $\fp(x) = \cos x$ (see Figure \ref{fig:sin_and_dsin}).
\begin{figure}[!hb]
\centering
\begin{tikzpicture}
...
\end{figure}
%\mtable{The function $f(x)=\sin x$ and $\fp(x)=\cos x$.}{thelabel}{\begin{tikzpicture}
...
%         node [pos=.4, above right] {\scriptsize $f(x)$};
%\addplot [{\colortwo},smooth, domain=0:6.28318,thick] {cos(deg(x))}
%         node [pos=.4, below left] {\scriptsize $\fp(x)$};
%\draw[\colortwo] (axis cs:5.1,-1.005)--(axis cs:4.4,-1.005);
%\draw[\colortwo] (axis cs:2,1.005)--(axis cs:1.1,1.005);  
%\draw[gray] (axis cs:1.5708,0)--(axis cs:1.5708,1);
%\draw[gray] (axis cs:4.7123889,0)--(axis cs:4.7123889,-1);
...
%		\end{array}$$
%		
\mfigure{0in}{A graph of the derivative of $f(x) = \abs{x}$.}{fig:absolutevalueprime}{figures/figabsolutevalueprime}
...
Find the derivative of $f(x)$, where $\ds f(x) = \begin{cases}\sin x & x\leq \pi/2 \\ 1 & x>\pi/2 \end{cases}.$ See Figure \ref{fig:piecewisesinx1}.}
...
\mfigure{0in}{A graph of $f(x)$ as defined in Example \ref{ex_diff_piecewise}.}{fig:piecewisesinx1}{figures/figpiecewisesinx1}
\[
...
% 0 &=0\\
%\end{align*}
\mfigure{0in}{A graph of $\fp(x)$ in Example \ref{ex_diff_piecewise}.}{fig:piecewisecosx1}{figures/figpiecewisecosx1}
