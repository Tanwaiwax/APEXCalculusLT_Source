...
%		\begin{align*}
%			\frac{d}{dx}\Bigl(f(y)\Bigr) &= \frac{d}{dx}\Bigl(x\Bigr) \\
...
\mtable{A right triangle defined by $y=\sin ^{-1}(x/1)$ with the length of the third leg found using the Pythagorean Theorem.}{fig:inverse3}{\begin{tikzpicture}[x=.5\marginparwidth,y=.5\marginparwidth,>=stealth]
...
%Use \autoref{thm:deriv_inverse_functions} to compute $\ds \frac{d}{dx}\bigl(\ln x\bigr)$.
%\solution
...
\[\int\frac{dx}{4+(x+1)^2} =\int\frac{du}{4+u^2}=\frac12 \tan^{-1}(u/2)+C =\frac 12\tan^{-1}\left(\frac{x+1}2\right)+C.\]
\end{enumerate}
\end{example}
