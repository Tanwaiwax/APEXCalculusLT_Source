...
One could argue the \emph{only} good approximation, given the information provided, would be based on ``distance = rate $\times$ time.'' In this case, we assume a constant rate of 60 mph with a time of $5/60$ hours. Hence we would approximate the distance traveled as 5 miles.\bigskip
...
\begin{example}[The meaning of the derivative: World Population]\label{ex_der_meaning1}
...
\begin{example}[The meaning of the derivative: Manufacturing]\label{ex_der_meaning2}
...
Let's let $s(t)$ measure the distance traveled, in feet, of an object after $t$ seconds of travel. Then $s\primeskip'(t)$ has units ``feet per second,'' and $s\primeskip'(t)$ measures the \emph{instantaneous rate of distance change} --- it measures \textbf{velocity}.\index{velocity} 

Now consider $v(t)$, a velocity function. That is, at time $t$, $v(t)$ gives the velocity of an object. The derivative of $v$, $v\primeskip'(t)$, gives the \emph{instantaneous rate of velocity change} ---
\textbf{acceleration}. (We often think of acceleration in terms of cars: a car may ``go from 0 to 60 in 4.8 seconds.'' This is an \emph{average} acceleration, a measurement of how quickly the velocity changed.) If velocity is measured in feet per second, and time is measured in seconds, then the units of acceleration (i.e., the units of $v\primeskip'(t)$) are ``feet per second per second,'' or $($ft/s$)$/s. We often shorten this to ``feet per second squared,'' or ft/s$^2$, but this tends to obscure the meaning of the units.\index{acceleration}
...
\mbox{}\\[-2.5\baselineskip]\parbox[t]{\linewidth}{\begin{enumerate}
...
\begin{example}[Understanding the derivative: the rate of change]\label{ex_der_meaning3}
Consider
%
\mtable{A graph of $f(x)=x^2$ and tangent lines.}{fig:xsquared}{\pdftooltip{\begin{tikzpicture}
...
\end{tikzpicture}}{A curve from the origin to (4,16), with two lines tangent at separate locations.}}
%
$f(x) = x^2$ as shown in \autoref{fig:xsquared} with tangent lines at $x=1$ and $x=3$. It is clear that at $x=3$ the function is growing faster than at $x=1$, as it is steeper at $x=3$. How much faster is it growing?
\solution
We can answer this more easily after we learn to quickly compute derivatives in the following section. For now, we will answer graphically, by considering the slopes of the respective tangent lines.

With practice, one can fairly effectively sketch tangent lines to a curve at a particular point. In \autoref{fig:xsquared}, we have sketched the tangent lines to $f$ at $x=1$ and $x=3$, along with a grid to help us measure the slopes of these lines. At $x=1$, the slope is 2; at $x=3$, the slope is 6. Thus we can say not only is $f$ growing faster at $x=3$ than at $x=1$, it is growing \emph{three times as fast}.
\end{example}

\mtable{Graphs of $f$ and $\fp$ in \autoref{ex_der_meaning4}, along with tangent lines.}{fig:fwithderiv}{\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{The function f starts at (4,0), goes downward to near (1,-2), turns back upward through (2,0), goes through (3,4), and is leveling off as x approaches 4.  Its derivative starts at (0,-12), goes up to near (2,4), and approach (4,-1) at the graph finishes.  Near x=1, the tangent line to f has a slight negative slope, so that the derivative is slightly negative.  Near x=3, the tangent line to f has a positive slope, so that the derivative is positive.}}

\begin{example}[Understanding the graph of the derivative]\label{ex_der_meaning4}
...
\begin{example}[Approximation with the derivative]\label{ex_der_meaning5}
Consider again the graph of $f(x)$ and its derivative $\fp(x)$ in \autoref{ex_der_meaning4}.
%
\mtable{Zooming in on $f$ and its tangent line at $x=3$ for the function given in Examples \ref{ex_der_meaning4} and \ref{ex_der_meaning5}.}{fig:fwithderivzoom3}{\pdftooltip{\begin{tikzpicture}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{The previous image, zoomed in where the function is near (3,4).}}
%
...
% overfull hbox and a passive sentence
%We cannot understate the importance of understanding the derivative.
% seems wordy.  maybe just say it
...
\printexercises{exercises/02-02-exercises}
