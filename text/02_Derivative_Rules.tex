...
\theorem{thm:deriv_common}{Derivatives of Common Functions}{\mbox{}\smallskip\\[-\baselineskip]
\iflatexml\begin{enumerate}\else\begin{enumerate*}\fi\noindent
\lxAddClass{twoColumnListByRows}
\item \parbox[t]{13em}{\sword{Constant Rule:}\index{derivative!Constant Rule}
	$\ds \frac{d}{dx}\big( c\big) = 0$,\\
	\phantom{.}~where $c$ is a constant.}
\item \parbox[t]{13em}{\sword{Power Rule:}\index{derivative!Power Rule}\index{Power Rule!differentiation}
	$\ds \frac{d}{dx}\left(x^n\right) = nx^{n-1}$,\\
	\phantom{.}~where $n$ is any real number.} \iflatexml\else\\[1ex]\fi
\item \parbox{13em}{$\ds \frac{d}{dx}(\sin x) = \cos x$}
\item \parbox{13em}{$\ds \frac{d}{dx}(\cos x) = -\sin x$} \iflatexml\else\\[1.5ex]\fi
\item \parbox{13em}{$\ds \frac{d}{dx}\left(e^x\right) = e^x$}
\item \parbox{13em}{$\ds \frac{d}{dx}(\ln x) = \frac{1}{x}$}
\iflatexml\end{enumerate}\else\end{enumerate*}\fi\index{derivative!basic rules}}

This theorem starts by stating an intuitive fact: constant functions have a rate of change of zero, as they are \textit{constant}. Therefore their derivative is 0. The proof is left as an exercise.
...
&= \lim_{h\to 0}\dbinom{n}{1} x^{n-1} + \dbinom{n}{2} h x^{n-2}+ \dotsb +\dbinom{n}{n-1} h^{n-2}x + \dbinom {n}{n} h^{n-1},\\
...
\example{ex_deriv_rule1}{Using \autoref{thm:deriv_common} to find, and use, derivatives}
{Let $f(x)=x^3$. 
\begin{enumerate}
	\item	Find $\fp(x)$.
...
{\begin{enumerate}
	...
	\mfigure{0in}{A graph of $f(x) = x^3$, along with its derivative $\fp(x) = 3x^2$ and its tangent line at $x=-1$.}{fig:xcubedwithderiv}{figures/figxcubedwithderiv}	\item	We can use the tangent line to approximate $(-1.1)^3$ as $-1.1$ is close to $-1$. We have
...
\begin{enumerate}
	...
	\qquad$\ds \frac{d}{dx}\Big(c\cdot f(x)\Big) = c\cdot\frac{d}{dx}\Big(f(x)\Big) = c\cdot\fp(x)$.\index{derivative!Constant Multiple Rule}\index{Constant Multiple Rule!of derivatives}
\end{enumerate}}
...
\jmtVideo{3dJepii_rJ0}{basic-derivative-examples}{Derivatives --- Basic Examples}
...
\example{ex_der15}{Using Theorems \ref{thm:deriv_common} and \ref{thm:deriv_prop} to find derivatives}{Use Theorems \ref{thm:deriv_common} and \ref{thm:deriv_prop} to differentiate
...
\example{ex_der2}{Using the tangent line to approximate a function value}{Let $f(x) = \sin x + 2x+1$. Approximate $f(3)$ using an appropriate tangent line.}
...
{Let $y=f(x)$ be a differentiable function on $I$. \index{derivative!higher order}\index{derivative!notation}
		...
		\end{enumerate}}
\mnote[-1in]{\textbf{Note:} \autoref{def:Higher_Deriv} comes with the caveat ``Where the corresponding limits exist.'' With $f$ differentiable on $I$, it is possible that $\fp$ is \textit{not} differentiable on all of $I$, and so on.}
...
\example{ex_high_order}{Finding higher order derivatives}{Find the first four derivatives of the following functions:
...
Notice how all successive derivatives will also be 0.
	...
	\item	Employing \autoref{thm:deriv_common} and the Constant Multiple Rule, we can see that $$\fp(x) = \fp'(x) = \fp''(x) = f\,^{(4)}(x) = 5e^x.\eoehere$$
	\end{enumerate}}
