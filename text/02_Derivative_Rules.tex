...
\item \parbox{13em}{$\ds \frac{d}{dx}(\cos x) = -\sin x$} \iflatexml\else\\[1ex]\fi
...
\begin{proof}[Proof of Differentiation Power Rule when $n$ is a positive integer]
Let $f(x)= x^n$, where $n \in \mathbb{Z}^+$. By the definition of derivative,\\
\begin{align*}
\small
\fp(x) &= \lim_{h\to 0} \frac{(x+h)^n - x^n}{h} \\
&= \lim_{h\to 0} \frac{(x+h)^n - x^n}{h} \quad\text{\tiny use the Binomial Theorem to expand } (x+h)^n\\  
&=\lim_{h\to 0} \frac{x^n + \binom{n}{1} hx^{n-1} + \binom{n}{2} h^2x^{n-2}+ ... +\binom{n}{n-1} h^{n-1}x + \binom {n}{n} h^n)   -x^n}{h} \\
&= \lim_{h\to 0} \frac{\binom{n}{1} hx^{n-1} + \binom{n}{2} h^2x^{n-2}+ ... +\binom{n}{n-1} h^{n-1}x + \binom {n}{n}  h^n}{h} \\
&= \lim_{h\to 0} \frac{\binom{n}{1} hx^{n-1} + \binom{n}{2} h^2x^{n-2}+ ... +\binom{n}{n-1} h^{n-1}x + \binom {n}{n}  h^n}{h} \\
&= \lim_{h\to 0} \frac{h\left[\binom{n}{1} x^{n-1} + \binom{n}{2} h x^{n-2}+ ... +\binom{n}{n-1} h^{n-2}x + \binom {n}{n} h^{n-1}\right]}{h},{\tiny\text{ divide out}~h}\\
&= \lim_{h\to 0}~ \binom{n}{1} x^{n-1} + \binom{n}{2} h x^{n-2}+ ... +\binom{n}{n-1} h^{n-2}x + \binom {n}{n} h^{n-1},\\
&=  n x^{n-1} \text{ since } \binom{n}{1} = n \qedhere
\end{align*}
\end{proof}

We proved Theorem \ref{thm:deriv_common} part 3 in a previous section and part 4 is left as an exercise.  In parts 5 and 6 we see something incredible about the functions $y=e^x$ and $y=\ln x$. We will use these rules freely, unfortunately their proofs will have to wait until we know a few more calculus techniques.
...
{\begin{enumerate}
	...
	\mfigure{0in}{A graph of $f(x) = x^3$, along with its derivative $\fp(x) = 3x^2$ and its tangent line at $x=-1$.}{fig:xcubedwithderiv}{figures/figxcubedwithderiv}
\end{enumerate}
...
\jmtVideo{3dJepii_rJ0}{basic-derivative-examples}{
...
\mnote{\textbf{Note:} Definition \ref{def:Higher_Deriv} comes with the caveat ``Where the corresponding limits exist.'' With $f$ differentiable on $I$, it is possible that $\fp$ is \textit{not} differentiable on all of $I$, and so on.}
