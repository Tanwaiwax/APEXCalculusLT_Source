...
\begin{tabular}{ll}
\item \sword{Constant Rule:}	$\dfrac d{dx}\bigl( c\bigr) = 0$, &
...
%One special case of the Power Rule is when $n=1$, i.e., when $f(x) = x$. What is $\fp(x)$? According to the Power Rule, \[\fp(x) = \frac{d}{dx}\bigl(x\bigr) = \frac{d}{dx}\bigl(x^1\bigr) = 1\cdot x^0 = 1.\] In words, we are asking ``At what rate does $f$ change with respect to $x$?'' Since $f$ \emph{is} $x$, we are asking ``At what rate does $x$ change with respect to $x$?'' The answer is: 1. They change at the same rate.\\
...
We were a bit pedantic here, showing every step. Normally we would do all the arithmetic and steps in our head and readily find $\ds \frac{d}{dx}\Bigl(3x^2+5x+7\Bigr) = 6x+5$.
