...
\[\frac{d}{dx}\Bigl(\sin y\Bigr) = \cos y \cdot y\primeskip'.\] %The derivative of $\sin(y)$ is $\cos(y)y\primeskip'$.  The reason for this is the chain rule. Since $y$ itself depends on $x$, the term $\sin(y)$ is really a function inside of a function, $y$ being a function of $x$.
...
%% the following derives d/dx x^r, but implicit derivation assumes that the derivative already exists
%One hole in our current understanding of derivatives is this: what is the derivative of the square root function? That is,
%\[\frac{d}{dx}\bigl(\sqrt{x}\bigr) = \frac{d}{dx}\bigl(x^{1/2}\bigr) = \text{?}\]
%
...
%\[\frac{d}{dx}\bigl(x^{1/2}\bigr) = \frac12x^{-1/2} = \frac{1}{2\sqrt{x}}.\]
%
...
%	y &= x^{m/n} \\
%	y^n &= x^m \\
%	\frac{d}{dx}\bigl(y^n\bigr) &= \frac{d}{dx}\bigl(x^m\bigr) \\
...
The astroid and folium of Descartes also appear in \autoref{sec:param_eqs}.  Other important implicit curves are the conic sections, which appear in \autoref{sec:conic_sections}.  Also, the lemniscate, cardioid, and lima\c con seen in \autoref{sec:polar} have particularly nice implicit representations.\bigskip
...
%	\item		$\frac{d}{dx}\bigl(a^x\bigr) = \ln a\cdot a^x$ \\
%	\item		$\frac{d}{dx}\bigl(\log_a x\bigr) = \frac{1}{\ln a}\cdot\frac{1}{x}$ &
%	\item		$\frac{d}{dx}\bigl(\sin^{-1}x\bigr) = \frac{1}{\sqrt{1-x^2}}$ &
%	\item		$\frac{d}{dx}\bigl(\csc^{-1}x\bigr) = -\frac{1}{\abs{x}\sqrt{x^2-1}}$ \\
%	\item		$\frac{d}{dx}\bigl(\tan^{-1}x\bigr) = \frac{1}{1+x^2}$ &
%	\item		$\frac{d}{dx}\bigl(\cos^{-1}x\bigr) = -\frac{1}{\sqrt{1-x^2}}$
%	\item		$\frac{d}{dx}\bigl(\sec^{-1}x\bigr) = \frac{1}{\abs{x}\sqrt{x^2-1}}$
%	\item		$\frac{d}{dx}\bigl(\cot^{-1}x\bigr) = -\frac{1}{1+x^2}$
\end{tabular}
\end{anywhereenum}
\end{theorem}
