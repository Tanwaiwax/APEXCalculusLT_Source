...
{Let $f$ and $g$ be differentiable functions on an open interval $I$.\index{Product Rule!differentiation}\index{derivative!Product Rule} Then $fg$ is a differentiable function on $I$, and \[\frac{d}{dx}\Big(f(x)g(x)\Big) = f(x)g\primeskip'(x) + \fp(x)g(x).\]}
...
\mfigure{1in}{A graph of $y = 5x^2\sin x$ and its tangent line at $x=\pi/2$.}{fig:5xsquaredsinx}{figures/fig5xsquaredsinx}
}\\
...
\begin{align*}
	...
	\qquad\text{(now add 0 to the numerator)}\\
	...
	&=\lim_{h\to0} \frac{f(x+h)g(x+h)-f(x+h)g(x)}{h}+\lim_{h\to0}\frac{f(x+h)g(x)-f(x)g(x)}{h}\quad\text{(factor)} \\
	&=\lim_{h\to0} f(x+h)\frac{g(x+h)-g(x)}{h}+\lim_{h\to0}\frac{f(x+h)-f(x)}{h}g(x)\qquad\text{(apply limits)}\\
...
\small   
\frac{d}{dx}\left(\frac{f(x)}{g(x)}\right)
&=\lim_{h\to 0}\left[\left(\frac{f(x+h)g(x)\mathbf{-f(x)g(x)+f(x)g(x)}-f(x)g(x+h)}{g(x+h)g(x)}\right)\cdot\frac1h\right] \\[.5\baselineskip]
...
&=\frac{g(x)f'(x)-f(x)g'(x)}{[g(x)]^2}\qedhere
\end{align*}
\end{proof}
...
\mfigure{0in}{A graph of $y=\tan x$ along with its tangent line at $x=\pi/4$.}{fig:tanx}{figures/figtanx}}\\
...
{We found in Example \ref{ex_quot1} that $\ds \fp(x) = \frac{10x\sin x - 5x^2\cos x}{\sin^2 x}$. We now find $\fp$ using the Product Rule, considering $f$ as $f(x) = 5x^2\csc x$.
\begin{align*}
	\fp(x)
...
When we stated the Power Rule in Section \ref{sec:basic_diff_rules} we claimed that it worked for all $n\in\mathbb{R}$ but only provided the proof for $n\in\mathbb{Z}^+$. The next example uses the Quotient Rule tp provide justification of the Power Rule for  $n\in\mathbb{Z}$, $n\neq0$.\\
...
\begin{align*}
	...
	\text{\small (apply result from Example \ref{ex_deriv_power})}\\
	&= \parbox{60pt}{$\ds-\frac{n}{x^{n+1}}$}\text{\small (rewrite algebraically)} \\
	&= -nx^{-(n+1)} \\
	&= -nx^{-n-1}.
\end{align*}
Thus, for all $n\in\mathbb{Z}$, $n\neq 0$ we can officially apply the Power Rule: multiply by the power, then subtract 1 from the power.
...
{\begin{enumerate}
	...
	\item	As $x\neq 0$, we can divide through by $x$ first, giving $\ds f(x) = x-3+\frac{1}x$. Now apply the Power Rule to see $$\fp(x) = 1-\frac{1}{x^2},$$  the same result as before.
\end{enumerate}
\vskip-\baselineskip}\\
