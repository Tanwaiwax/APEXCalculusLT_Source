...
Let $f$ and $g$ be differentiable functions on an open interval $I$.\index{Product Rule!differentiation}\index{derivative!Product Rule} Then $f\cdot g$ is a differentiable function on $I$, and \[\frac{d}{dx}\Bigl(f(x)g(x)\Bigr) = f(x)g\primeskip'(x) + \fp(x)g(x).\]
\end{theorem}

\sword{Important:} $\ds\frac{d}{dx}\Bigl(f(x)g(x)\Bigr) \neq \fp(x)g\primeskip'(x)$. While this answer is simpler than the Product Rule, it is wrong.  We can show that this is wrong by considering \\
$f(x)=x^2$ and $g(x)=x^5$. \\
...
\begin{example}[Using the Quotient Rule to find $\frac{d}{dx}\bigl(\tan x\bigr)$.]\label{ex_der_tan}
