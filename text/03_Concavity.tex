...
%
%\begin{center}
%\begin{tikzpicture}[alt={alt={Blue curve dips, rises, dips; inflection at x≈2,3 splits concave-up sides (f''>0) from concave-down middle (f''<0).}]%[scale=2]
%%\draw (-4,-3) grid (4,2);
%%\fill (0,0) circle (.1);
%%
...
\mtable[.5in]{A graph of $S(t)$ in \autoref{ex_conc3} along with $S\primeskip'(t)$.}{fig:conc3b}{\begin{tikzpicture}[alt={Blue S(t) and red S′(t); red dips to −10, crosses 0 at t=1.6, then rises; axes t=(0–3), y=(−12–22).}]
...
\mtable[2\baselineskip]{A graph of $f(x) = x^4$. Clearly $f$ is always concave up, despite the fact that $\fpp(x) = 0$ when $x=0$. It this example, the \emph{possible} point of inflection $(0,0)$ is not a point of inflection.}{fig:concavity5}{\begin{tikzpicture}[alt={Blue quartic f(x)=x^4, symmetric U always concave up; axes x=(−1–1), y=(0–1).}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,
...
\mtable[.5in]{Demonstrating the fact that relative maxima occur when the graph is concave down and relative minima occur when the graph is concave up.}{fig:concavity6}{\begin{tikzpicture}[alt={Blue curve concave down to relative max near (−1,9), then concave up to relative min near (1,−10); axes x=(−2–2), y=(−10–10).}]
 \begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
   axis y line=middle,axis x line=middle,name=myplot,
