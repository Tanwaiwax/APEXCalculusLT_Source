...
This section explores how knowing information about \fpp\ gives information about $f$.\\ %reaches its maximum rate of increase or decrease, in other words, when the function is changing at its most rapid rate, we usually have $\fpp=0$.
...
\keyidea{idea:geom_concav}{Geometric Interpretation of Concavity}
{A function is concave up if its graph lies above its tangent lines, so that it curves upward. A function is concave down if its graph lies below its tangent lines, so that it curves downward.}
...
\mfigure{-10in}{A function $f$ with a concave up graph. Notice how the slopes of the tangent lines, when looking from left to right, are increasing.}{fig:concavity1}{figures/figconcavity1}
%
%When $\fpp>0$, the shape of the graph of $f$ looks like a part of the graph below.
...
\mfigure{0in}{A function $f$ with a concave down graph. Notice how the slopes of the tangent lines, when looking from left to right, are decreasing.}{fig:concavity2}{figures/figconcavity2}
...
\mnote[-.5in]{\textbf{Note:} A mnemonic for remembering what concave up/down means is: ``Concave up is like a cup; concave down is like a frown.'' It is admittedly terrible, but it works.
%
...
\mfigure{1in}{A graph of a function with its inflection points marked. The intervals where concave up/down are also indicated.}{fig:concavity4}{figures/figconcavity4}
...
The number line in Figure \ref{fig:conc1}(a) illustrates the process of determining concavity; Figure \ref{fig:conc1}(b) shows a graph of $f$ and \fpp, confirming our results. Notice how $f$ is concave down precisely when $\fpp(x)<0$ and concave up when $\fpp(x)>0$.
}\\
%\myincludegraphics[scale=.5]{apex-second-derivative_img4}

\mtable{A number line determining the concavity of $f$ and a graph of $f$ used in Example \ref{ex_conc1}.}{fig:concline1}{\begin{tabular}{c}
\myincludegraphics{figures/figconcline1} \\ (a) \\
\myincludegraphics{figures/figconc1} \\ (b)
\end{tabular}}
...
\put(5.95,-.5){$1$}
\put(6,-.25){\line(0,1){.5}}
\put(4,0){\vector(1,0){4}}
\put(4,0){\vector(-1,0){4}}
\put(.4,.4){\begin{tabular}{c}$\fpp<0\Rightarrow$\\$f$ c.\ down\end{tabular}}
\put(1.4,.4){\begin{tabular}{c}$f$\\undef\end{tabular}}
\put(2.4,.4){\begin{tabular}{c}$\fpp>0\Rightarrow$\\$f$ c.\ up\end{tabular}}
\put(4.4,.4){\begin{tabular}{c}$\fpp<0\Rightarrow$\\$f$ c.\ down\end{tabular}}
\put(5.4,.4){\begin{tabular}{c}$f$\\undef\end{tabular}}
\put(6.4,.4){\begin{tabular}{c}$\fpp>0\Rightarrow$\\$f$ c.\ up\end{tabular}}
\end{picture}
\caption{Number line for $f$ in Example \ref{ex_conc2}.}\label{fig:concline2}
\end{figure}

\mfigure{0in}{A graph of $f(x)$ and $\fpp(x)$ in Example \ref{ex_conc2}.}{fig:conc2}{figures/figconc2}
...
\mfigure{0in}{A graph of $S(t)$ in Example \ref{ex_conc3}, modeling the sale of a product over time.}{fig:conc3}{figures/figconc3}
}
...
\centering
\scriptsize
\begin{tabular}{c p{2.2em} p{2.2em} p{2.2em} p{2.2em} p{2.2em} p{2.2em}}
%\begin{tabular}{|c| p{2.2em} |p{2.2em} |p{2.2em} |p{2.2em} |p{2.2em} |p{2.2em}|}
\multicolumn{2}{c|}{ } & \multicolumn{3}{c|}{ } & \multicolumn{2}{c}{ } \\\cline{3-7}
\multicolumn{2}{c|}{ } & \multicolumn{3}{c|}{ } & \multicolumn{2}{c}{ } \\[.6ex]
\vphantom{\big|}$x$ & \multicolumn{2}{|c}{$0$} & & \multicolumn{2}{c}{\zbox{$\sqrt{\frac43}\approx1.16$}} & \\[.5ex]
\vphantom{\big|}$\fpp$ & \multicolumn{1}{|c|}{ } & \multicolumn{3}{c|}{$<0$} & \multicolumn{2}{c}{$>0$} \\[.5ex]\hline
\vphantom{\big|}$f$ & \multicolumn{2}{|c}{ } & \quad\zbox{c.\ down} & \multicolumn{2}{c}{ } & \zbox{c.\ up} \\
&&&&&&
\end{tabular}
%\setlength{\unitlength}{3.5em}
%\begin{picture}(6,1.5)(0,-.5)
%\put(0,-.25){\line(0,1){.5}}
%\put(2.6,-.5){$\sqrt{4/3}\approx1.16$}
%\put(3,-.25){\line(0,1){.5}}
%\put(0,0){\vector(1,0){6}}
%\put(1.3,.4){\begin{tabular}{c}$\fpp<0\Rightarrow$\\$f$\ \ c.\ down\end{tabular}}
%\put(4.4,.4){\begin{tabular}{c}$\fpp>0\Rightarrow$\\$f$\ \ c.\ up\end{tabular}}
%\end{picture}
\caption{Number line for $f$ in Example \ref{ex_conc3}.}\label{fig:concline3}
\end{figure}
...
\mfigure{-1in}{A graph of $S(t)$ in Example \ref{ex_conc3} along with $S\primeskip'(t)$.}{fig:conc3b}{figures/figconc3b}
...
\mfigure{0in}{A graph of $f(x) = x^4$. Clearly $f$ is always concave up, despite the fact that $\fpp(x) = 0$ when $x=0$. It this example, the \textit{possible} point of inflection $(0,0)$ is not a point of inflection.}{fig:concavity5}{figures/figconcavity5}

%\paragraph{A note about concavity}
%
%In general, concavity can change only where either the second derivative is 0, where there is a vertical asymptote, or (rare in practice) where the second derivative is undefined.  But concavity doesn't \emph{have} to change at these places.  For instance, if $f(x)=x^4$, then $\fpp(0)=0$, but there is no change of concavity at 0 and also no inflection point there.   Moreover, if $f(x)=1/x^2$, then $f$ has a vertical asymptote at 0, but there is no change in concavity at 0.  See the figure below.
...
\mfigure{1in}{Demonstrating the fact that relative maxima occur when the graph is concave down and relative minima occur when the graph is concave up.}{fig:concavity6}{figures/figconcavity6}
...
\mfigure{1in}{A graph of $f(x)$ in Example \ref{ex_conc4}. The second derivative is evaluated at each critical point. When the graph is concave up, the critical point represents a local minimum; when the graph is concave down, the critical point represents a local maximum.}{fig:conc4}{figures/figconc4}
