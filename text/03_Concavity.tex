...
\mtable[-1in]{A function $f$ with a graph that is (a) concave up and (b) concave down. Notice how the slopes of the tangent lines, when looking from left to right, are (a) increasing and (b) decreasing.}{fig:concavity1}{\begin{tabular}{c}
\myincludegraphics{figures/figconcavity1} \\
(a) \\
\myincludegraphics{figures/figconcavity2} \\
(b)
\end{tabular}}
...
%\ifthenelse{\boolean{longpage}}{\bigskip}{}
...
\mfigure{0in}{A graph of a function with its inflection points marked. The intervals where concave up/down are also indicated.}{fig:concavity4}{figures/figconcavity4}
...
\example{ex_conc1}{Finding intervals of concave up/down, inflection points}{Let $f(x)=x^3-3x+1$. Find the inflection points of $f$ and the intervals on which it is concave up/down.}
...
\mtable{A number line determining the concavity of $f$ and a graph of $f$ used in \autoref{ex_conc1}.}{fig:concline1}{\begin{tabular}{c}
\begin{tikzpicture}
...
\example{ex_conc2}{Finding intervals of concave up/down, inflection points}{Let $f(x)=x/(x^2-1)$. Find the inflection points of $f$ and the intervals on which it is concave up/down.}
...
\mtable{A graph of $f(x)$ and $\fpp(x)$ in \autoref{ex_conc2}.}{fig:conc2}{\begin{tikzpicture}
 \begin{axis}[width=\marginparwidth+25pt,tick label style={font=\scriptsize},
   axis y line=middle,axis x line=middle,name=myplot,
   ymin=-11,ymax=11,xmin=-3.2,xmax=3.2]
  \addplot [thick, {\colorone},smooth,samples=25,domain=-3.2:-1.047] {x/(x^2-1)};
  \addplot [thick, {\colorone},smooth,samples=25,domain=-.955:.955] {x/(x^2-1)};
  \addplot [thick, {\colorone},smooth,samples=25,domain=1.047:3.2] {x/(x^2-1)}
   node[pos=.5,left,color=black] {\scriptsize $f(x)$};
  \addplot [{\colortwo},thick,smooth,samples=25,domain=-3.2:-1.45]
   {(2*x*(x^2+3))/((x^2-1)^3)};
  \addplot [{\colortwo},thick,smooth,samples=25,domain=-.553:.553]
   {(2*x*(x^2+3))/((x^2-1)^3)};
  \addplot [{\colortwo},smooth,thick,samples=25,domain=1.45:3.2]
...
\example{ex_conc3}{Understanding inflection points}{The sales of a certain product over a three-year span are modeled by $S(t)= t^4-8t^2+20$, where $t$ is the time in years, shown in \autoref{fig:conc3}.  Over the first two years, sales are decreasing.  Find the point at which sales are decreasing at their greatest rate.
\mfigure{0in}{A graph of $S(t)$ in \autoref{ex_conc3}, modeling the sale of a product over time.}{fig:conc3}{figures/figconc3}}
...
\mfigure{0in}{A graph of $S(t)$ in \autoref{ex_conc3} along with $S\primeskip'(t)$.}{fig:conc3b}{figures/figconc3b}
...
\mfigure{-1in}{A graph of $f(x) = x^4$. Clearly $f$ is always concave up, despite the fact that $\fpp(x) = 0$ when $x=0$. It this example, the \textit{possible} point of inflection $(0,0)$ is not a point of inflection.}{fig:concavity5}{figures/figconcavity5}
...
\mtable{Demonstrating the fact that relative maxima occur when the graph is concave down and relative minima occur when the graph is concave up.}{fig:concavity6}{\begin{tikzpicture}
 \begin{axis}[width=\marginparwidth+25pt,tick label style={font=\scriptsize},
   axis y line=middle,axis x line=middle,name=myplot,
   ymin=-11,ymax=11,xmin=-2.1,xmax=2.1]
  \addplot [thick, {\colorone},smooth,domain=-2.021:2.021] {5*x^3-15*x};
...
\example{ex_conc4}{Using the Second Derivative Test}{Let $f(x)=100/x + x$.  Find the critical points of $f$ and use the Second Derivative Test to label them as relative maxima or minima.}
{\mfigure{-1in}{A graph of $f(x)$ in \autoref{ex_conc4}. The second derivative is evaluated at each critical point. When the graph is concave up, the critical point represents a local minimum; when the graph is concave down, the critical point represents a local maximum.}{fig:conc4}{figures/figconc4}
%
