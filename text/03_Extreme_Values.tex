...
%\ifthenelse{\boolean{longpage}}% if longpage
			...
			%} %end longpage
			%{% not longpage
				\mtable[-3in]{Graphs of functions with and without extreme values.}{fig:extreme}{\begin{tabular}{c} \myincludegraphics{figures/figextreme1} \\[1ex] (a) \\[2ex] \myincludegraphics{figures/figextreme2}  \\[1ex] (b) \\[2ex] \myincludegraphics{figures/figextreme3} \\[1ex] (c) \end{tabular}}
			%}

\mnote[1in]{\textbf{Note:} The extreme values of a function are ``$y$'' values, values the function attains, not the input values.}
...
\theorem{thm:extremeVal}{The Extreme Value Theorem}%
{Let $f$ be a continuous function defined on a closed interval $I$. Then $f$ has both a maximum and minimum value on $I$.%\hfill\hyperref[pf:extremeVal]{(See the proof.)}
\index{Extreme Value Theorem}
}

As with Theorem \ref{thm:IVT}, this theorem seems obvious but depends on there being enough real numbers to find the extrema.  To see that it does not work with the rational numbers, we can consider the function $f(x)=x^3-6x$ on $[-2,2]$.

% TODO: someone else should read what I wrote
%It isn't too hard to see why this should be true.  We can find $x$-values so that the $y$-values get as large (or small) as possible.  Then we can take a subset of these $x$-values that cluster around some point.  Because the $y$-values got as large (or small) as possible, the same is still true about the subset.  Because $f$ is continuous, we can take limits of everything, and $f$ has its maximum (or minimum) at that point.
% don't have an explanation that needs subsequential limits if we can't say ``subsequential limits''
...
\mfigure{0in}{A graph of $f(x) = 2x^3-9x^2$ as in Example \ref{ex_extval1}.}{fig:extval1}{figures/figextval1}}
...
\mfigure{0in}{A graph of $f(x) = (3x^4-4x^3-12x^2+5)/5$ as in Example \ref{ex_extval2}.}%
...
\mfigure{0in}{A graph of $f(x) = (x-1)^{2/3}+2$ as in Example \ref{ex_extval3}.}{fig:extval3}{figures/figextval3}
}
...
{Let $f$ be defined at $c$. The value $c$ is a \textbf{critical number} (or \textbf{critical value}) of $f$ if $\fp(c)=0$ or $\fp(c)$ is not defined.\index{critical number}\index{critical point}\bigskip

If $c$ is a critical number of $f$, then the point $(c,f(c))$ is a \textbf{critical point} of $f$.}

\theorem{thm:criticalpts}{Fermat's Theorem: Relative Extrema and Critical Points}
{Let a function $f$ have a relative extrema at the point $(c,f(c))$. Then $c$ is a critical number of $f$.}%\hfill\hyperref[pf:criticalpts]{(See the proof.)}

% TODO have someone else check this explanation
It isn't too hard to see why this should be true.  If $\fp$ is defined at a relative extreme, then the tangent line must be horizontal.  Otherwise, we'd be able to move along the graph in the direction given by the tangent line.  Be careful to understand that this theorem states  ``All relative extrema occur at critical points.'' It does not say ``All critical numbers produce relative extrema.'' For instance, consider $f(x) = x^3$. Since $\fp(x) = 3x^2$, it is straightforward to determine that $x=0$ is a critical number of $f$. However, $f$ has no relative extrema, as illustrated in Figure \ref{fig:extreme4}.\bigskip

\mfigure{0in}{A graph of $f(x)=x^3$ which has a critical value of $x=0$, but no relative extrema.}{fig:extreme4}{figures/figextreme4}

Theorem \ref{thm:extremeVal} states that a continuous function on a closed interval will have absolute extrema, that is, both an absolute maximum and an absolute minimum. These extrema occur either at the endpoints or at critical values in the interval. We combine these concepts to offer a strategy for finding extrema.
...
Find the extreme values of $f(x) = 2x^3+3x^2-12x$ on $[0,3]$, graphed in Figure \ref{fig:extval4}(a).}
{We follow the steps outlined in Key Idea \ref{idea:extrema}. We first evaluate $f$ at the endpoints: $$f(0)=2(0)^3+3(0)^2-12(0) = 0 \quad \text{and}\quad f(3)=2(3)^3+3(3)^2-12(3) =45.$$
...
\mtable[-.5in]{A graph and table of extreme values of $f(x) = 2x^3+3x^2-12x$ on $[0,3]$ as in Example \ref{ex_extval4}.}{fig:extval4}{\begin{tabular}{c}
\myincludegraphics{figures/figextval4} \\ (a) \\
\begin{tabular}{cc} $x$ & $f(x)$ \\ \hline \rule{0pt}{10pt} 0 & 0 \\ 1 & $-7$\\3 & 45  \end{tabular} \\ (b)
\end{tabular}}

Note that all this was done without the aid of a graph; this work followed an analytic algorithm and did not depend on any visualization. Figure \ref{fig:extval4}(a) shows $f$ and we can confirm our answer, but it is important to  understand that these answers can be found without graphical assistance.
...
Find the maximum and minimum values of $f$ on $[-4,2]$, where $$f(x) = \begin{cases} (x-1)^2 & x\leq 0 \\ x+1 & x>0 \end{cases} .$$
}
{Here $f$ is piecewise--defined, but we can still apply Key Idea \ref{idea:extrema} because it is continuous. Evaluating $f$ at the endpoints gives: 
$$ f(-4)=(-4-1)^2=(-5)^2 = 25 \quad \text{and} \quad f(2)=2+1 = 3.$$

\mtable[.5in]{A table of extreme values and graph of $f(x)$ on $[-4,2]$ as in Example \ref{ex_extval5}.}{fig:extval5}{\begin{tabular}{c}
\begin{tabular}{cc} $x$ & $f(x)$ \\ \hline \rule{0pt}{10pt} $-4$ & 25 \\ 0 & 1 \\ 2 & 3\end{tabular} \\ (a) \\
\myincludegraphics{figures/figextval5} \\ (b)
\end{tabular}}
...
So we have three important $x$ values to consider: $x= -4, 2$ and $0$. We have already evaluated the first two, and $f(0)=(0-1)^2=(-1)^2=1$.  Collecting these values into Figure \ref{fig:extval5}(a), we see that the absolute minimum of $f$ is 1 and the absolute maximum of $f$ is $25$. Our answer is confirmed by the graph of $f$ in Figure \ref{fig:extval5}.}\\
...
\mtable[-3in]{Finding the extrema of $f(x)= \cos (x^2)$ in Example \ref{ex_extval6}.}{table:ext6}{\begin{tabular}{cc} $x$ & $f(x)$ \\ \hline \rule{0pt}{10pt} $-2$ & $-0.65$ \\ $-\sqrt{\pi}$ & $-1$ \\ 0 & 1\\ $\sqrt{\pi}$ & $-1$ \\ 2 & $-0.65$ \end{tabular}}
...
\mfigure{-1.5in}{A graph of $f(x)=\cos(x^2)$ on $[-2,2]$ as in Example \ref{ex_extval6}.}{fig:extval6}{figures/figextval6}
}\\
...
\mtable[0in]{A graph and table of extrema of $f(x)=\sqrt{1-x^2}$ on $[-1,1]$ as in Example \ref{ex_extval7}.}{fig:extval7}{\begin{tabular}{c}
\myincludegraphics{figures/figextval7} \\ (a) \\
\begin{tabular}{cc} $x$ & $f(x)$ \\ \hline \rule{0pt}{10pt} $-1$ & 0 \\ 0 & 1 \\ 1 & 0 \end{tabular} \\ (b)
\end{tabular}}

Using the Chain Rule, we find $\ds \fp(x) = \frac{-x}{\sqrt{1-x^2}}$. The critical points of $f$ are found when $\fp(x) = 0$ or when $\fp$ is undefined. It is straightforward to find that $\fp(x) = 0$ when $x=0$, and $\fp$ is undefined when $x=\pm 1$, the endpoints of the interval. The table of important values is given in Figure \ref{fig:extval7}(b). The maximum value is 1, and the minimum value is 0.
}\\


\mnote[1.5in]{\textbf{Note:}  We implicitly found the derivative of $x^2+y^2=1$, the unit circle, in Example \ref{ex_implicit7} as $\frac{dy}{dx} = -x/y$. In Example \ref{ex_extval7}, half of the unit circle is given as $y=f(x) = \sqrt{1-x^2}$. We found $\fp(x) = \frac{-x}{\sqrt{1-x^2}}$. Recognize that the denominator of this fraction is $y$; that is, we again found $\fp(x) = \frac{dy}{dx} = -x/y.$}
