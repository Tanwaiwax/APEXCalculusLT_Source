...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{A curve starting near (-2,-0.6), moving downward to a bottom at (-1.7,-1), turning back to a flat section where it crosses the y-axis at y=1, and then repeating the process in a mirror image across the y-axis to finish near (2,-0.6).}
\\(b)}
