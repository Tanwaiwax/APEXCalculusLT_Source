...
\mnote{\textbf{Note:} Theorem \ref{thm:incr_decr} (parts 1 and 2) also holds if $\fp(c) = 0$ for a finite number of values of $c$ in $I$.}
...
{Using Key Idea \ref{idea:incr_decr}, we first find the critical values of $f$. We have $\fp(x) = 3x^2+2x-1 = (3x-1)(x+1)$, so $\fp(x) = 0$ when $x=-1$ and when $x=1/3$. We see that $\fp$ is never undefined.
...
\begin{figure}[!hb]
\centering
\setlength{\unitlength}{4em}
\begin{picture}(4,1.5)(-2,-.5)
\scriptsize
\put(-1.1,-.4){$-1$}
\put(-1,-.25){\line(0,1){.5}}
\put(.233,-.4){$1/3$}
\put(.333,-.25){\line(0,1){.5}}
\put(-.333,0){\vector(1,0){2}}
\put(-.333,0){\vector(-1,0){2}}
\put(-2,.5){\begin{tabular}{c}$\fp>0\Rightarrow$\\$f$ incr\end{tabular}}
\put(-.8,.5){\begin{tabular}{c}$\fp<0\Rightarrow$\\$f$ decr\end{tabular}}
\put(.6,.5){\begin{tabular}{c}$\fp>0\Rightarrow$\\$f$ incr\end{tabular}}
\end{picture}
...
\item[Subinterval 3,] $(1/3,\infty)$:\quad Pick an arbitrarily large value for $p>1/3$ and note that $\fp(p) =3p^2+2p-1 >0$. We conclude that $f$ is increasing on $(1/3,\infty)$.
\end{description}
...
\mfigure{1in}{A graph of $f(x)$ in Example \ref{ex_incr1}, showing where $f$ is increasing and decreasing.}{fig:incr1}{figures/figincr1}
}\\
...
\mtable[1in]{The four cases of Theorem \ref{thm:first_der}}{fig:first_deriv_examples}{\begin{tabular}{cc}
\begin{tikzpicture}
\begin{axis}[width=.6\marginparwidth,
axis y line=none,axis x line=none,
name=myplot,ymin=-1,ymax=.5,xmin=-1,xmax=1
]
\addplot[thick,{\colorone},smooth,domain=-1:1,<->] { -x^2 };
...
\addplot[thick,{\colorone},smooth,domain=-1:1] { x^3 };
\filldraw (axis cs:0,0) node [below] {\small$c$} circle (1pt);
\end{axis}
\end{tikzpicture}
&
...
\item[Interval 4,] $(3,\infty)$:\quad Choosing an very large number $p$ from this subinterval will give a positive numerator and (of course) a positive denominator. So $f$ is increasing on $(3,\infty)$.
\end{description}

\mfigure{0in}{A graph of $f(x)$ in Example \ref{ex_incr2}, showing where $f$ is increasing and decreasing.}{fig:incr2}{figures/figincr2}
...
\put(4,0){\vector(-1,0){4}}
\put(.4,.4){\begin{tabular}{c}$\fp>0\Rightarrow$\\$f$ incr\end{tabular}}
\put(2.4,.4){\begin{tabular}{c}$\fp<0\Rightarrow$\\$f$ decr\end{tabular}}
\put(4.4,.4){\begin{tabular}{c}$\fp<0\Rightarrow$\\$f$ decr\end{tabular}}
\put(6.4,.4){\begin{tabular}{c}$\fp>0\Rightarrow$\\$f$ incr\end{tabular}}
\put(1.6,.6){\begin{tabular}{c}rel.\\max\end{tabular}}
\put(3.6,.6){\begin{tabular}{c}f\\undef\end{tabular}}
\put(5.6,.6){\begin{tabular}{c}rel.\\min\end{tabular}}
\end{picture}
...
\put(4,0){\vector(1,0){4}}
\put(4,0){\vector(-1,0){4}}
\put(.4,.4){\begin{tabular}{c}$\fp<0\Rightarrow$\\$f$ decr\end{tabular}}
\put(2.4,.4){\begin{tabular}{c}$\fp>0\Rightarrow$\\$f$ incr\end{tabular}}
\put(4.4,.4){\begin{tabular}{c}$\fp<0\Rightarrow$\\$f$ decr\end{tabular}}
\put(6.4,.4){\begin{tabular}{c}$\fp>0\Rightarrow$\\$f$ incr\end{tabular}}
\put(1.6,.6){\begin{tabular}{c}rel.\\min\end{tabular}}
\put(3.6,.6){\begin{tabular}{c}rel.\\max\end{tabular}}
\put(5.6,.6){\begin{tabular}{c}rel.\\min\end{tabular}}
\end{picture}
...
\item[Interval 4,] $(1,\infty)$: Similar work to that done for the other three intervals shows that $\fp(x)>0$ on $(1,\infty)$, so $f$ is increasing on this interval.
\end{description}

We conclude by stating that $f$ is increasing on $(-1,0) \cup (1,\infty)$ and decreasing on $(-\infty,-1) \cup (0,1)$. The sign of \fp\ changes from negative to positive around $x=-1$ and $x=1$, meaning by Theorem \ref{thm:first_der} that $f(-1)$ and $f(1)$ are relative minima of $f$. As the sign of \fp\ changes from positive to negative at $x=0$, we have a relative maximum at $f(0)$. Figure \ref{fig:incr3} shows a graph of $f$, confirming our result. We also graph $\fp$, highlighting once more that $f$ is increasing when $\fp>0$ and is decreasing when $\fp<0$.\vspace{-\baselineskip}}\\

\mfigure{0in}{A graph of $f(x)$ in Example \ref{ex_incr3}, showing where $f$ is increasing and decreasing.}{fig:incr3}{figures/figincr3}

We examine one more example.\\

\example{inc_dec_trig}{Using the First Derivative Test with Trigonometry}{
Find the intervals on which $f(\theta)=\cos\theta-\cos^2\theta$ is increasing and decreasing and find the relative extrema on the interval $[0,2\pi]$.}{We see that $\fp(\theta)=-\sin\theta+2\cos\theta\sin\theta=\sin\theta(2\cos\theta-1)$.  Therefore, $\fp(\theta)=0$ when $\theta=0,\frac\pi3,\pi,\frac{5\pi}3,2\pi$.  This breaks our number line into four intervals.
...
\item[Interval 4,] $(\frac{5\pi}3,2\pi)$: When $\theta=\frac{5\pi}6$, $\sin\theta<0$ and $2\cos\theta-1>0$ so that $\fp<0$.
\end{description}

This means that $f$ is increasing on $(0,\frac\pi3)\cup(\pi,\frac{5\pi}3)$ and decreasing on $(\frac\pi3,\pi)\cup(\frac{5\pi}3,2\pi)$, so that the relative maxima are $f(\frac\pi3)$ and $f(\frac{5\pi}3)$ and the relative minimum is $f(\pi)$.  (The values $f(0)$ and $f(2\pi)$ would also be relative minima, but relative extrema are not allowed to occur at the endpoints of an interval.\vspace{-\baselineskip}}\\
