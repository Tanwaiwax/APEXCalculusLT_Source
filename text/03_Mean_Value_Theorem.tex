...
\mtable{Distance traveled as a function of time.  Must a tangent line parallel the average slope?}{fig:mvt_ex}{\begin{tikzpicture}
\begin{axis}[width=\marginparwidth, tick label style={font=\scriptsize},
			ymajorgrids=true, xmajorgrids=true, xmin=0, xmax=2, ymin=0, ymax=100,
			xlabel={Time (hours)}, ylabel={Distance (miles)}]
\addplot [{\colorone},thick,smooth] coordinates {(0,0)(.5,10)(1,50)(1.5,90)(2,100)};
\addplot [{\colortwo},thick,smooth] coordinates {(0,0)(2,100)};
\end{axis}
\end{tikzpicture}}
...
%\end{minipage}}%
\mtable[1in]{A graph of $f_1(x) = 1/x^2$ and $f_2(x) = |x|$ in Example \ref{ex_mvt1}.}{fig:mvt1}%
{\begin{tabular}{c}
\myincludegraphics{figures/figmvt1}\\
(a)\\[10pt]
...
%
%\mfigure{0in}{A graph of $f_1(x) = 1/x^2$ in Example \ref{ex_mvt1}.}{fig:mvt1}{figures/figmvt1}
%
%\mfigure{0in}{A graph of $f_2(x) = |x|$ in Example \ref{ex_mvt1}.}{fig:mvt2}{figures/figmvt2}
\vskip-\baselineskip}\\
...
\mfigure{0in}{A graph of $f(x) = x^3-5x^2+3x+5$, where $f(a) = f(b)$. Note the existence of $c$, where $a<c<b$, where $\fp(c)=0$.}{fig:mvt3}{figures/figmvt3}
...
Suppose for this paragraph that $f$ had two (or more) roots.  Then by Rolle's Theorem, there is some $c$ in between the roots so that $0=\fp(c)=3+3x^2+56x^6$.  But this cannot happen, since $\fp$ is always at least 3.
...
\mfigure{0in}{Demonstrating the Mean Value Theorem in Example \ref{ex_mvt2}.}{fig:mvt4}{figures/figmvt4}
}\\
