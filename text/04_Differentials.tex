...
We introduce two new variables, $dx$ and $dy$ in the context of a formal definition. %We defined $dx = \Delta x$; that is, it is a small change in $x$. We define
...
%  \draw(axis cs:.3,.09)circle(2pt);
  ...
  \draw(axis cs:.3,.09)
   ...
   --node[pos=.3,right,xshift=-.3em]{$\Big\}\scriptstyle dy$}
     node[pos=.5,right,xshift=1em]{$\Bigg\}\scriptstyle \Delta y$}(axis cs:.8,.64);
\end{axis}
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}
% todo Tim bigger brace for the \Delta y?
...
\begin{enumerate}
	...
	\item		$dy = \fp(x)dx$ which, by \autoeqref{eq:differential}, is an \textit{approximation} of the change in $y$ value as $x$ changes by $\Delta x$; $dy \approx \Delta y$.
\end{enumerate}
}
...
\example{ex_diffal1}{Finding and using differentials}{Consider $f(x) = x^2$. Knowing $f(3) = 9$, approximate $f(3.1)$.}
...
\example{ex_diffal2}{Using differentials to approximate a function value}{Approximate $\sqrt{4.5}$.}
...
\example{ex_diffal3}{Finding differentials}{In each of the following, find the differential $dy$.
\[
