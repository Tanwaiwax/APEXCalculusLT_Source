...
{\mtable{Graphing $f(x) = \sin x$ and its tangent line at $x=\pi/3$ in order to estimate $\sin 1.1$.}{fig:diffal1}{\begin{tabular}{c}
 \myincludegraphics{figures/figdiffal1a}\\
 (a)\\\addlinespace
 \myincludegraphics{figures/figdiffal1} \\
 (b)
\end{tabular}
}
}
...
\begin{enumerate}
	...
	\item		$dy = \fp(x)dx$ which, by \autoref{eq:differential}, is an \textit{approximation} of the change in $y$ value as $x$ changes by $\dx$; $dy \approx \dy$.
\end{enumerate}
}
