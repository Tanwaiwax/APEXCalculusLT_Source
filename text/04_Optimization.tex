...
%
\mtable{A sketch of the enclosure in \autoref{ex_opt1}.}{fig:opt1}{\begin{tikzpicture}[scale=1.3]
\ifthenelse{\boolean{latexml}}{\shadedraw[fill=green!75!black]}{\shadedraw[top color=green,bottom color=green!50!black]}
 (0,0)--(33.6pt,35.28pt)--(95.2pt,35.28pt)--(61.6pt,0)--cycle;
\begin{scope}[cm={1,1.05,0,1,(0,0)}]
\foreach \x in {0,...,7} { % left side
 \ifthenelse{\boolean{latexml}}{\shadedraw[xscale=.3,shift={(\x*14 pt,0)},fill=brown!70!white]}{\shadedraw[xscale=.3,shift={(\x*14 pt,0)},left color=brown!40!white,right color=brown!99!white]}
 (0,0)-- ++(0,20pt)-- ++(4pt,4pt)-- ++(6pt,0pt)-- ++(4pt,-4pt)-- ++(0pt,-20pt)--cycle;
}
\end{scope}
\begin{scope}[shift={(33.6pt,35.28pt)}]
\foreach \x in {0,...,10} { % back side
 \ifthenelse{\boolean{latexml}}{\shadedraw[xscale=.4,shift={(\x*14 pt,0)},fill=brown!70!white]}{\shadedraw[xscale=.4,shift={(\x*14 pt,0)},left color=brown!40!white,right color=brown!99!white]}
 (0,0)-- ++(0,20pt)-- ++(4pt,4pt)-- ++(6pt,0pt)-- ++(4pt,-4pt)-- ++(0pt,-20pt)--cycle;
}
\end{scope}
...
%		\end{enumerate}
%\textit{\small (continued)\dotsc}
%\end{keyidea}
%
%\addtocounter{keyidea}{-1}
%\begin{keyidea}[Solving Optimization Problems --- Continued]\label{idea:optimizationb}
...
\mtable{A sketch of the enclosure in \autoref{ex_opt2}.}{fig:opt2}{\begin{tikzpicture}[scale=1.3]
...
\example{ex_opt3}{Optimization: minimizing cost}{A power line needs to be run from a power station located on the beach to an offshore facility. \autoref{fig:opt3b} shows the distances between the power station to the facility.
...
\begin{tikzpicture}[scale=.67]
 \ifthenelse{\boolean{latexml}}{\draw [draw={\colorone},thick,fill=\colorone!40]}{\draw [draw={\colorone},thick,left color=\colorone!60,right color=\colorone!20]}
...
%   \ifthenelse{\boolean{latexml}}{\draw [draw={\colorone},fill=\colorone!40]}{\draw [draw={\colorone},left color=\colorone!60,right color=\colorone!20]}
%   (-1,-.2) -- (-1,-3.3)
...
\example{ex_min_surf_area}{Optimization: Minimizing Surface Area}{Design a closed cylindrical can of volume $8$ ft$^3$ so that it uses the least amount of metal. In other words, minimize the surface area of the can.}{Following the strategy of \autoref{idea:optimization}, we make a sketch in \autoref{fig:min_surf_area} and identify the quantity to be minimized as the surface area of the cylinder. The formula for the surface area is our fundamental equation  since it relates all of our relevant quantities.
\[
 A
