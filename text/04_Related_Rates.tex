...
\mnote{\textbf{Note:} This section relies heavily on implicit differentiation, so referring back to Section \ref{sec:imp_deriv} may help.}
...
\keyidea{idea:related_rates}{Solving Related Rates Problems}
{~\\[-\baselineskip]
\begin{enumerate}
	\item	Understand the problem. Clearly identify what quantity is to be found. Make a sketch if helpful.
	\item	Create equations relevant to the context of the problem, using the information given.
	\item	Differentiate the equations from the previous item.
	\item	Substitute the known quantities, and solve for the missing one.
\end{enumerate}}

The important thing to remember is that you must differentiate before you substitute values.  Otherwise, you'll substitute a constant for what should be a variable, and it's derivative will be zero.  Consider another example.\\
...
\mfigure{0in}{A sketch of a police car (at bottom) attempting to measure the speed of a car (at right) in Example \ref{ex_rr3}.}{fig:rr3}{figures/figrr3}
...
\mnote{\textbf{Note:} Example \ref{ex_rr3} is both interesting and impractical. It highlights the difficulty in using radar in a non--linear fashion, and explains why ``in real life'' the police officer would follow the other driver to determine their speed, and not pull out pencil and paper.
...
\mfigure{1in}{Tracking a speeding car (at left) with a rotating camera.}{fig:rr4}{figures/figrr4}
...
% TODO: an example with similar triangles
...
% TODO: exercises from below

% problem with water in & water out
% problem with isosceles triangle
% problem with person, light & shadow
% television camera & rocket?
% lighthouse beam?
