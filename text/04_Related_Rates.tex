...
The topic of \sword{related rates} takes this one step further: knowing the \emph{rate} at which one quantity is changing can determine the \emph{rate} at which the other changes.\index{related rates}
...
\begin{align*}
C 	&= 2\pi r\\
\frac{d}{dt}\bigl(C\bigr) &= \frac{d}{dt}\bigl(2\pi r\bigr) \\
...
\begin{align*}
V &= \frac18A\\
\frac{d}{dt}\bigl(V\bigr) &= \frac{d}{dt}\left(\frac18A\right)\\
...
\begin{align*}
 V &= \frac\pi8 r^2 \\
 \frac{d}{dt}\bigl(V\bigr) &= \frac{d}{dt}\bigl(\frac\pi8 r^2\bigr) \\
...
%\begin{align*}
%	A 	&= \pi r^2 \\
%	\frac{d}{dt}\bigl(A\bigr) &= \frac{d}{dt}\bigl(\pi r^2\bigr)\\
...
\mtable{A sketch of a police car (at bottom) attempting to measure the speed of a car (at right) in \autoref{ex_rr3}.}{fig:rr3}{\begin{tikzpicture}[>=latex]
...
\mnote[-1in]{\textbf{Note:} \autoref{ex_rr3} is both interesting and impractical. It highlights the difficulty in using radar in a non-linear fashion, and explains why ``in real life'' the police officer would follow the other driver to determine their speed, and not pull out pencil and paper.\bigskip\\
...
\mtable[1in]{Tracking a speeding car (at left) with a rotating camera.}{fig:rr4}{\begin{tikzpicture}[>=latex]
