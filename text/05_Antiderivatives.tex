...
{The set of all antiderivatives of $f(x)$ is the \textbf{indefinite integral of $f$}, denoted by $$\int f(x) \ dx.$$\index{indefinite integral}\index{integration!indefinite}}

Using Definitions \ref{def:antider} and \ref{def:indef_int}, we can say that $$\int f(x) \ dx = F(x) + C.$$
...
\begin{itemize}
	...
	\item	Rule \#\ref{thm:d_sum_diff_rule} is the Sum/Difference Rule:\index{integration!Sum/Difference Rule}\index{Sum/Difference Rule!of integration} we can split integrals apart when the integrand contains terms that are added/subtracted, as we did in \autoref{ex_anti3}. So:
	\begin{align*}
		\int(3x^2+4x+5)\ dx
