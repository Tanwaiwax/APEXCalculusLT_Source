...
% todo Tim check this and next
\begin{theorem}[Derivatives and Antiderivatives]\label{thm:indef_alg}
\begin{minipage}{.5\linewidth}
\begin{enumerate}
\item\label{thm:d_const_mult_rule} $\frac{d}{dx}\bigl(cf(x) \bigr) = c\cdot \fp(x)$
\item\label{thm:d_sum_diff_rule} $\frac{d}{dx}\bigl(f(x)\pm g(x) \bigr) =$ \\
...
\begin{minipage}{.5\linewidth}
\begin{enumerate}
\item $\int c\cdot f(x)\ dx = c\cdot \int f(x)\ dx$
\item $\int \bigl(f(x)\pm g(x)\bigr)\ dx =$ \\
\null\qquad$\int f(x)\ dx\pm \int g(x)\ dx$
\item $\int 0\ dx = C$
\end{enumerate}
\end{minipage}
\end{theorem}
...
\item\label{thm:d_power_rule} $\frac{d}{dx}\bigl(x^n \bigr) = n\cdot x^{n-1}$
\item\label{thm:d_ln_rule} $\frac{d}{dx}\bigl(\ln\abs{x}\bigr) = \frac1 x$
\item $\frac{d}{dx}\bigl(e^ x \bigr) = e^x$
\item $\frac{d}{dx}\bigl(\sin x \bigr) = \cos x$
\item $\frac{d}{dx}\bigl(\cos x \bigr) = -\sin x$
\item $\frac{d}{dx}\bigl(\tan x \bigr) = \sec^2 x$
\item $\frac{d}{dx}\bigl(\cot x \bigr) = -\csc^2 x$
\item $\frac{d}{dx}\bigl(\sec x \bigr) = \sec x\tan x$
\item $\frac{d}{dx}\bigl(\csc x \bigr) = -\csc x\cot x$ \quad\null
\end{enumerate}
\columnbreak
...
\item $\int \cos x\ dx = \phantom{-}\sin x+C$
\item $\int \sin x\ dx = -\cos x+C$
\item $\int \sec^2 x\ dx = \phantom{-}\tan x+C$
\item $\int \csc^2 x\ dx = -\cot x+C$
\item $\int \sec x\tan x\ dx = \phantom{-}\sec x+C$
