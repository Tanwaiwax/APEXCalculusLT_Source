...
%
\mtable[.5in]{A graph of $f(x)$ in \autoref{ex_defint4}.}{fig:defint4}{\begin{tikzpicture}[alt={Blue peak at x=1.1,y=1 then descends to (5,−1); shaded triangle above x-axis 0–3 and below 3–5; axes $x,y$.}}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
...
\begin{tikzpicture}[alt={Blue line $f(x)=2x-4$; triangles $R_1$ above and $R_2$ below x-axis; axes $x,y$.}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
...
%
\mtable{A graph of a velocity in \autoref{ex_defint6}.}{fig:defint6}{\begin{tikzpicture}[alt=alt={Blue wavy $v(t)$ curve; shaded +38 above, two −11 lobes below t-axis; axes $t,v$.}]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,xtick=\empty,
...
\begin{tikzpicture}[alt={alt={Number line centered at 0; path: 0 to −11 left, across to 27 right, back toward 16; dots at −11 and 27; axis x.},x=.7em,y=2.5em]
 \draw[<->](-12.9,1)--(28.9,1);
 \foreach\x in {-12,-11,...,28} { \draw(\x,.8)--(\x,1.2); }S
...
\mtable{What is the area below $y=x^2$ on $[0,3]$? The region is not a usual geometric shape.}{fig:defint7}{\begin{tikzpicture}[alt={Blue curve $y=x^{2}$; shaded region under the curve from x = 0 to 3; axes $x,y$.}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
