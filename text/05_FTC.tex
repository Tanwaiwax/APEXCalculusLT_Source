...
\mtable{The area of the shaded region is $F(x) = \int_a^x f(t)\ dt$.}{fig:ftc1}{\begin{tikzpicture}
...
Let $f$ be continuous on $[a,b]$ and let $F(x) = \int_a^x f(t)\ dt$. Then $F$ is a continuous function on $[a,b]$, differentiable on $(a,b)$, and\index{Fundamental Theorem of Calculus}\index{integration!Fun. Thm. of Calc.}
\[\Fp(x)=f(x).\]
\end{theorem}
...
%Because $x$ is fixed, $f(x)$ is a constant and is equal to $\frac1h\int_x^{x+h}f(x)\ dt$.  Because $f$ is continuous at $x$, for any $\epsilon>0$, we can find a $\delta>0$ so that $\abs{x-t}<\delta$ implies $\abs{f(x)-f(t)}<\epsilon$.  In this case,
%\begin{multline*}
% \abs{\frac1h\int_x^{x+h}f(t)\ dt-f(x)}
% =\abs{\frac1h\int_x^{x+h}f(t)-f(x)\ dt} \\ % this step may be confusing
