...
\mtable[-.2in]{Table of values used to approximate $\ds\int_{-\frac{\pi}4}^{\frac{\pi}2}\sin(x^3)\ dx$ in \autoref{ex_num2}.}{fig:num2a}{%
...
\mtable[.2in]{Approximating\\ $\ds\int_{-\frac{\pi}4}^{\frac{\pi}2}\sin(x^3)\ dx$ in \autoref{ex_num2} using (top) the left hand rule and (bottom) the right hand rule.}{fig:num2b}{%
\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
...
This is the heart of the \textbf{Trapezoidal Rule}, where a definite integral $\ds \int_a^b f(x) \ dx$ is approximated by using trapezoids of equal widths to approximate the corresponding area under $f$. Using $n$ equally spaced subintervals with endpoints $x_0$, $x_1$, \dots, $x_n$, we again have $\ds \Delta x = \frac{b-a}n$. Thus:
\begin{align*}
	\int_a^b f(x)\ dx
...
Revisit \autoref{ex_num2} and approximate $\ds\int_{-\frac{\pi}{4}}^{\frac{\pi}{2}} \sin (x^3)\ dx$ using the Trapezoidal Rule and 10 equally spaced subintervals.
\solution
...
\begin{equation}
\int_{x_1}^{x_3} f(x)\ dx = \frac{x_3-x_1}{6}\bigl(y_1+4y_2+y_3\bigr).\label{eq:simpsons}
\end{equation}
...
\[\int_1^3 g(x) \ dx = \frac{3-1}{6}\bigl(3+4(1)+2\bigr)= 3.\]
...
\mtable[3\baselineskip]{A table of values to approximate $\int_{-\frac{\pi}4}^{\frac{\pi}2}\sin(x^3)\ dx$ in \autoref{ex_num6}, along with a graph of the function.}{fig:num6a}{%
\begin{tabular}{ll}\lxBeginTableHead
...
\begin{tabular}{ll}
Left Hand Rule: &
$\ds\int_a^b f(x)\ dx \approx \Delta x\Bigl[f(x_0) + f(x_1) + \dotsb + f(x_{n-1})\Bigr]$.\\
Right Hand Rule: &
$\ds\int_a^b f(x)\ dx \approx \Delta x\Bigl[f(x_1) + f(x_2) + \dotsb + f(x_n)\Bigr]$.\\
Midpoint Rule: &
$\ds\int_a^b f(x)\ dx \approx \Delta x\Bigl[f\bigl(\frac{x_0+x_1}2\bigr) + f\bigl(\frac{x_1+x_2}2\bigr) + \dotsb + f\bigl(\frac{x_{n-1}+x_n}2\bigr)\Bigr]$.\\
Trapezoidal Rule: &
$\ds\int_a^b f(x)\ dx \approx \frac{\Delta x}2\Bigl[f(x_0) + 2f(x_1) + 2f(x_2) +\dotsb + 2f(x_{n-1})+ f(x_n)\Bigr]$.\\
Simpson's Rule: &
$\ds\int_a^b f(x)\ dx \approx \frac{\Delta x}3\Bigl[f(x_0) + 4f(x_1) + 2f(x_2) +\dotsb + 4f(x_{n-1})+ f(x_n)\Bigr]$ {\small ($n$ even)}.
\end{tabular}
\end{keyidea}
}
...
\begin{align*}
\int_0^{0.2}v(t)\ dt &\approx \frac{1}{360}\Bigl[f(x_1)+4f(x_2) + 2f(x_3) + \dots + 4f(x_n)+f(x_{n+1})\Bigr]\\
