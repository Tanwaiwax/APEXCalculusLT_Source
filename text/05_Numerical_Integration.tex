...
%\subsection*{An Antiderivative Cannot Be Computed}
\bigskip
...
\mtable[-2in]{Graphically representing three definite integrals that cannot be evaluated using antiderivatives.}{fig:numerical1}{\myincludegraphics{figures/fignumerical1a}\vskip 25pt \myincludegraphics{figures/fignumerical1b}\vskip 25pt\myincludegraphics{figures/fignumerical1c}}
...
\mtable{Approximating $\ds\int_0^1e^{-x^2}\ dx$ in \autoref{ex_num1}.}{fig:num1}{\myincludegraphics{figures/fignum1b}\vskip 25pt \myincludegraphics{figures/fignum1a}}

Using the Left Hand Rule, we have:
\begin{align*}
	\sum_{i=1}^n f(x_{i-1})\dx
	&= \big(f(x_0)+f(x_1)+f(x_2) + f(x_3) + f(x_4)\big)\dx \\
...
\mtable[-2.5in]{Table of values used to approximate $\ds\int_{-\frac{\pi}4}^{\frac{\pi}2}\sin(x^3)\ dx$ in \autoref{ex_num2}.}{fig:num2a}{%
	\begin{tabular}{lrll} 
	$x_i$ & Exact & Approx. & $\sin(x_i^3)$ \\ \midrule
...
\mtable[-1in]{Approximating $\ds\int_{-\frac{\pi}4}^{\frac{\pi}2}\sin(x^3)\ dx$ in \autoref{ex_num2}.}{fig:num2b}{\myincludegraphics{figures/fignum2b}\vskip 25pt \myincludegraphics{figures/fignum2a}}
\begin{align*}
...
\mfigure{0in}{Approximating $\int_0^1 e^{-x^2}\ dx$ using 5 trapezoids of equal widths.}{fig:num3a}{figures/fignum3a}
...
\mfigure{0in}{The area of a trapezoid.}{fig:trapezoid}{figures/figtrapezoid}
...
There are many things to observe in this example. Note how each term in the final summation was multiplied by both 1/2 and by $\dx = 0.2$. We can factor these coefficients out, leaving a more concise summation as:\\
\flushinnerequ{\small
\[
...
\example{ex_num5}{Using Simpson's Rule}{Approximate $\ds\int_0^1 e^{-x^2}\ dx$ using Simpson's Rule and 4 equally spaced subintervals.\clearpage}
...
1 & 0.368
\end{tabular} \\ (a) \\
\myincludegraphics{figures/fignum5b} \\ (b)}
We begin by making a table of values as we have in the past, as shown in \autoref{fig:num5a}(a). Simpson's Rule states that $$\int_0^1e^{-x^2}\ dx \approx \frac{0.25}{3}\Big[1+4(0.939)+2(0.779)+4(0.570) + 0.368\Big] = 0.7468\overline{3}.$$
...
{\autoref{fig:num6a}(a) shows the table of values that we used in the past for this problem, shown here again for convenience. Again, $\dx = (\pi/2+\pi/4)/10 \approx 0.236$.
...
\begin{tabular}{ll} 
  ...
  $\phantom{-}1.57$ & $-0.670$ \\
\end{tabular} \\ (a) \\
\myincludegraphics{figures/fignum6b} \\ (b)}
...
%\begin{lxfigure}
%
%\captionof{figure}{Approximating $\int_{-\frac{\pi}4}^{\frac{\pi}2}\sin(x^3)\ dx$ in \autoref{ex_num6} with Simpson's Rule and 10 equally spaced intervals.}
%\label{fig:num6b}
%\end{lxfigure}
...
\theorem{thm:numerical_error}{Error Bounds in Numerical Integration}%
...
\[\fpp(x) = e^{-x^2}(4x^2-2).\]
\mfigure{0in}{Graphing $\fpp(x)$ in \autoref{ex_num7} to help establish error bounds.}{fig:num7a}{figures/fignum7a}
...
$$0.7445-0.0067 = .7378 \leq \int_0^1e^{-x^2}\ dx \leq 0.7512 = 0.7445 + 0.0067.$$ We had earlier computed the exact answer, correct to 4 decimal places, to be 0.7468, affirming the validity of \autoref{thm:numerical_error}.\\
...
\mfigure{0in}{Graphing $f\,^{(4)}(x)$ in \autoref{ex_num7} to help establish error bounds.}{fig:num7b}{figures/fignum7b}
...
$$0.74683-0.00026 = .74657 \leq \int_0^1e^{-x^2}\ dx \leq 0.74709 = 0.74683 + 0.00026.$$ Once again we affirm the validity of \autoref{thm:numerical_error}.}

% todo: an example of ``find the n so that the estimate is within ? of the actual''
...
%
\mtable{Speed data collected at 30 second intervals for \autoref{ex_num8}.}{fig:num8}{\begin{tabular}{rr}
Time & \parbox{30pt}{\centering Speed \\ (mph)}\rule[-9pt]{0pt}{5pt} \\ \midrule
0 & 0 \\
 1 & 25 \\ 2 & 22 \\
 3 & 19 \\ 4 & 39 \\
 5 & 0 \\ 6 & 43\\
