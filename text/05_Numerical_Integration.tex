...
\mtable[-2in]{Graphically representing three definite integrals that cannot be evaluated using antiderivatives.}{fig:numerical1}{\myincludegraphics{figures/fignumerical1a}\bigskip\\ \myincludegraphics{figures/fignumerical1b}\bigskip\\ \myincludegraphics{figures/fignumerical1c}}
...
$$
% todo maybe use \pi/4 to \pi/2 in (b) instead?
...
\mtable{Approximating $\ds\int_0^1e^{-x^2}\ dx$ in \autoref{ex_num1} using (top) the left hand rule and (bottom) the right hand rule.}{fig:num1}{\myincludegraphics{figures/fignum1b}\bigskip\\
\myincludegraphics{figures/fignum1a}}
...
\mtable{Approximating\\ $\ds\int_{-\frac{\pi}4}^{\frac{\pi}2}\sin(x^3)\ dx$ in \autoref{ex_num2} using (top) the left hand rule and (bottom) the right hand rule.}{fig:num2b}{\myincludegraphics{figures/fignum2b}\\ \myincludegraphics{figures/fignum2a}}
...
$$0.7445-0.0067 = .7378 \leq \int_0^1e^{-x^2}\ dx \leq 0.7512 = 0.7445 + 0.0067.$$ We had earlier computed the exact answer, correct to 4 decimal places, to be 0.7468, affirming the validity of \autoref{thm:numerical_error}.\bigskip\\
