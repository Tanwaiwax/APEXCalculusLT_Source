...
\mtable{A graph of $f(x) = 4x-x^2$. What is the area of the shaded region?}{fig:rie1a}{\begin{tikzpicture}[alt={Blue parabola y = 4x - x^2 from 0 to 4; shaded region beneath; axes x and y.}]
...
%\mtable[1in]{Approximating $\ds\int_0^4(4x-x^2)\dd x$ using the Right Hand Rule in \autoref{ex_rie2}.}{fig:rie2b}{\begin{tikzpicture}[alt={Blue parabola y=4x-x^{2} on 0-4; four  red right-endpoint rectangles (heights 3, 4, 3, 0) from x=1 to 4,  the rectangle over 3–4 flushes with the x-axis; axes x,y.}] 
%\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
%axis y line=middle,axis x line=middle,name=myplot,axis on top,xtick={1,2,3,4},
%ytick={1,2,3,4},ymin=-1,ymax=5,xmin=-.5,xmax=4.5]
%\addplot [draw={\coloronefill},fill={\coloronefill},area style,domain=0:4]
...
\mtable[-3\baselineskip]{Approximating $\int_0^4(4x-x^2)\dd x$ with the Right Hand Rule and 16 evenly spaced subintervals.}{fig:rie7}[{\begin{tikzpicture}alt={Blue parabola y = 4x - x^2 on 0–4; sixteen narrow red rectangles use right endpoints, shading under curve; axes x and y.}]
...
% not \ds because of line break
\mtable[1in]{Approximating $\int_{-2}^3 (5x+2)\dd x$ using the Midpoint Rule and 10 evenly spaced subintervals in \autoref{ex_rie8}.}{fig:rie8}{\begin{tikzpicture}[alt={Blue line y = 5x + 2 from x = -2 to 3; ten equal rectangles (midpoint rule) shade area under line; axes x and y.}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
...
\mtable[-1in]{Approximating $\ds\int_{-1}^5 x^3\dd x$ using the Left Hand Rule and 10 evenly spaced subintervals.}{fig:rie9}{\begin{tikzpicture}[alt=alt={Blue curve y = x^3 from x = -1 to 5; ten left-end red rectangles approximate area; axes x and y.}]
\begin{axis}[width=1.16\marginparwidth,%
