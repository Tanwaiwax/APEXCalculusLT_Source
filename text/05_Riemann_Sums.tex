...
Approximate the value of $\ds\int_0^4 (4x-x^2)\ dx$ using the Left Hand Rule, the Right Hand Rule, and the Midpoint Rule, using 4 equally spaced subintervals.
\solution
...
%\iflatexml
\begin{enumerate}
%\lxAddClass{twoColumnList}\else\begin{enumerate}[series=summationprop]\fi
...
%\begin{example}
%Use technology to approximate $\ds\int_0^4(4x-x^2)\ dx$ using the Left Hand Rule and 16 and 100 equally spaced intervals.
%
...
Approximate $\ds\int_0^4 (4x-x^2)\,dx$ using the Right Hand Rule and summation formulas with 16 and 1000 equally spaced intervals.
\solution
...
% of $\ds\int_0^4 (4x-x^2)\ dx$. That is,
...
Approximate $\ds\int_{-2}^3 (5x+2)\ dx$ using the Midpoint Rule and 10 equally spaced intervals.
\solution
...
Revisit $\ds\int_0^4(4x-x^2)\ dx$ yet again. Approximate this definite integral using the Right Hand Rule with $n$ equally spaced subintervals.
\solution
...
Both common sense and high-level mathematics tell us that as $n$ gets large, the approximation gets better. In fact, if we take the \emph{limit} as $n\rightarrow \infty$, we get the \emph{exact area} described by $\ds\int_0^4 (4x-x^2)\ dx$. That is, 
\begin{align*}
	\int_0^4 (4x-x^2)\ dx
...
Find a formula that approximates $\ds\int_{-1}^5 x^3\ dx$ using the Right Hand Rule and $n$ equally spaced subintervals, then take the limit as $n\to\infty$ to find the exact area.
\solution
We see that %Following \autoref{idea:riemann}, we have
