...
\mfigure{0in}{A graph of $f(x) = 4x-x^2$. What is the area of the shaded region?}{fig:rie1a}{figures/figrie1a}
...
\mfigure{0in}{Approximating $\int_0^4(4x-x^2)\ dx$ using rectangles. The heights of the rectangles are determined using different rules.}{fig:rie1b}{figures/figrie1b}
...
\mfigure{0in}{Approximating $\int_0^4(4x-x^2)\ dx$ using the Left Hand Rule in Example \ref{ex_rie2}.}{fig:rie2a}{figures/figrie2a}
...
\mfigure{1in}{Approximating $\int_0^4(4x-x^2)\ dx$ using the Right Hand Rule in Example \ref{ex_rie2}.}{fig:rie2b}{figures/figrie2b}

In this example, these rectangle seem to be the mirror image of those found in Figure \ref{fig:rie2a}. (This is because of the symmetry of our shaded region.) Our approximation gives the same answer as before, though calculated a different way:
	\begin{align*} f(1)\cdot 1 + f(2)\cdot 1+ f(3)\cdot 1+f(4)\cdot 1 &=\\
	3+4+3+0&= 10.
	\end{align*}

\mfigure{2in}{Approximating $\int_0^4(4x-x^2)\ dx$ using the Midpoint Rule in Example \ref{ex_rie2}.}{fig:rie2c}{figures/figrie2c}
...
\setboxwidth{75pt}
\ifthenelse{%
\boolean{latexml}\OR\isodd{\pageref{thm:summation}}%
}{%
% do nothing
}{%
\noindent\hspace{-75pt}%
...
\iflatexml\begin{enumerate}\lxAddClass{twoColumnList}\else\begin{enumerate}[series=summationprop]\fi
...
{\begin{align*}
\sum_{i=1}^6 (2i-1)
&=\sum_{i=1}^6 2i-\sum_{i=1}^6 (1) && \text{(Theorem \ref{thm:summation}(2))}\\
&=\left( 2\sum_{i=1}^6 i\right) -\sum_{i=1}^6 (1)
&& \text{(Theorem \ref{thm:summation}(3))}\\
&=2\left(\frac{6(6+1)}{2}\right)-6 && \text{(Theorem \ref{thm:summation}(1,5))}\\
&=2(21)-6=36
\end{align*}
...
\begin{tikzpicture}[>=latex]
	...
	\draw [{\colorone},->] (a) -- (1.2,.75);
	\draw [{\colorone}] (2,2.25) node [text width=60pt,align=center] (b) {\scriptsize \centering number of subintervals\\[-5pt] between $x_0$ and $x_i$};
	\draw [{\colorone},->] (b) -- (1.95,1.25);
	\draw [{\colorone}] (3,0) node [text width=60pt,align=center] (c) {\scriptsize \centering subinterval\\[-5pt] size};
	\draw [{\colorone},->] (c) -- (2.45,.75);
\end{tikzpicture}
\end{center}
...
\noindent \textbf{Left Hand Rule:} $\ds\sum_{i=1}^{16} f(x_{i-1})\Delta x$ \vskip 5pt

\noindent \textbf{Right Hand Rule:} $\ds\sum_{i=1}^{16} f(x_i)\Delta x$\vskip 5pt

\noindent \textbf{Midpoint Rule:} $\ds\sum_{i=1}^{16} f\left(\frac{x_{i-1}+x_i}2\right)\Delta x$
\index{Left Hand Rule}\index{Right Hand Rule}\index{Midpoint Rule}
\smallskip

% TODO: note that the x_i are different because there is no 0th subinterval?
...
The following example lets us practice using the Left Hand Rule and the summation formulas introduced in Theorem \ref{thm:summation}.\\
...
&=\sum_{i=1}^{16}4i\Delta x^2-\sum_{i=1}^{16}i^2\Delta x^3 &&\text{(Theorem \ref{thm:summation}(2))}\\
&\omit$\ds=(4\Delta x^2)\sum_{i=1}^{16}i
-(\Delta x^3)\sum_{i=1}^{16}i^2$\hfill(*) % omit makes the hfill work
&&\text{(Theorem \ref{thm:summation}(3))}\\
&=4\left(\frac14\right)^2\left(\frac{(16)(17)}{2}\right) -\left(\frac14\right)^3\left(\frac{(16)(17)(33)}{6}\right)&&\text{(Theorem \ref{thm:summation}(5,6))}\\
&=34-\frac{187}{8}=\frac{85}{8}=10.625\\
\end{align*}
\drawexampleline
\mtable{Approximating $\int_0^4(4x-x^2)\ dx$ with the Right Hand Rule and 16 evenly spaced subintervals.}{fig:rie7}{\begin{tikzpicture}
...
\end{tikzpicture}}
We were able to sum up the areas of 16 rectangles with very little computation. In Figure \ref{fig:rie7} the function and the 16 rectangles are graphed. While some rectangles over-approximate the area, others under-approximate the area by about the same amount. Thus our approximate area of 10.625 is likely a fairly good approximation.
...
\vspace{-2\baselineskip}}\\[2\baselineskip]
...
\mfigure{0in}{An example of a general Riemann sum to approximate $\int_0^4(4x-x^2)\ dx$.}{fig:riedef}{figures/figriedef}
...
Let's do another example.\bigskip
...
\begin{align*}
	...
	&= \sum_{i=1}^{10} \left(5\left(\frac i2-\frac94\right) + 2\right)\Delta x\\
	&= \Delta x\sum_{i=1}^{10}\left[\left(\frac{5i}2\right) - \frac{37}4\right]\\
	&= \Delta x\left(\frac52\sum_{i=1}^{10} (i) -
	 \sum_{i=1}^{10}\left(\frac{37}4\right)\right) \\
	&= \frac12\left(\frac52\cdot\frac{(10)(11)}2 - 10\cdot\frac{37}4\right)  \\
	&= \frac{45}2 = 22.5
\end{align*}
\mfigure{0in}{Approximating $\int_{-2}^3 (5x+2)\ dx$ using the Midpoint Rule and 10 evenly spaced subintervals in Example \ref{ex_rie8}.}{fig:rie8}{figures/figrie8}
...
{Using Key Idea \ref{idea:riemann}, we know $\Delta x = \frac{4-0}{n} = 4/n$. We also find $x_i = 0 + \Delta xi = 4i/n$.
...
\begin{align*}
	...
	&&\left(\parbox{30pt}{\scriptsize \centering recall $\Delta x = \frac4n$}\right)\\
	&=\frac{32(n+1)}{n} - \frac{32(n+1)(2n+1)}{3n^2}
	&&\text{\scriptsize (now simplify)} \\
...
{Following Key Idea \ref{idea:riemann}, we have $\Delta x = \frac{5-(-1)}{n} = \frac6n$. We have $x_i = (-1) + i\Delta x$.
...
\begin{align*}
	...
	&= \sum_{i=1}^n\left(i^3\Delta x^4 - 3i^2\Delta x^3 + 3i\Delta x^2 -\Delta x\right) \quad \text{\scriptsize (now split up summation)}\\
	&= \Delta x^4 \sum_{i=1}^n i^3 -3\Delta x^3 \sum_{i=1}^n i^2+ 3\Delta x^2 \sum_{i=1}^n i - \sum_{i=1}^n \Delta x \\
...
\intertext{\scriptsize (use $\Delta x = 6/n$)}
	&= \frac{1296}{n^4}\cdot\frac{n^2(n+1)^2}{4}
	- 3\frac{216}{n^3}\cdot\frac{n(n+1)(2n+1)}{6} + 3\frac{36}{n^2}\frac{n(n+1)}2 -6
...
\mtable{Approximating $\int_{-1}^5 x^3\ dx$ using the Left Hand Rule and 10 evenly spaced subintervals.}{fig:rie9}{\begin{tikzpicture}
\begin{axis}[width=\marginparwidth+25pt,%
...
\foreach \x [evaluate=\x as \y using \x^3] in {-1,-.4,...,4.4001} {
  \addplot[thick,draw={\colortwo}] coordinates
    { (\x,0) (\x,\y) (\x+.6,\y) (\x+.6,0) };
}
\end{axis}
...
The previous two examples demonstrated how an expression such as $$\sum_{i=1}^n f(x_i)\Delta x$$ can be rewritten as an expression explicitly involving $n$, such as $\frac{32}3(1-\frac1{n^2})$. 
...
\theorem{thm:riemannSum}{Definite Integrals and the Limit of Riemann Sums}
...
Now that we have more tools to work with, we can justify the remaining properties in Theorem \ref{thm:defintprop}. 
...
\quad \text{(by Theorem \ref{thm:riemannSum}(2))}\\
&=\lim_{n\to\infty} 0\\
&=0\\
\end{align*}

\item Applying Theorem \ref{thm:riemannSum}(2), we have:
\[\int_a^b f(x)\,dx=\lim_{n\to\infty}\sum_{i=1}^n f(c_i)\Delta x.\]
When we compute $\int_b^a f(x)\,dx$, we can use the same partitions and the same points $c_i$, so the heights $f(c_i)$ will remain the same. Since we want to start at $x=b$ and finish at $x=a$, we use $\widetilde\Delta x=\frac{a-b}n=-\Delta x$. We now have:
\begin{align*}
\int_b^a f(x)\,dx
&=\lim_{n\to\infty} \sum_{i=1}^n f(c_i)\widetilde\Delta x
&&\text{(Theorem \ref{thm:riemannSum}(2))}\\
&=\lim_{n\to\infty} \sum_{i=1}^n f(c_i)(-\Delta x)\\
&=\lim_{n\to\infty} -\left(\sum_{i=1}^n f(c_i)\Delta x\right)
&&\text{(using Theorem \ref{thm:summation}(3))}\\
&=-\lim_{n\to\infty} \sum_{i=1}^n f(c_i)\Delta x\\
&=-\int_a^b f(x)\,dx &&\text{(Theorem \ref{thm:riemannSum}(2))}
\end{align*}
...
\theorem{thm:further_def_int_props}{Further Properties of the Definite Integral}{
...
\begin{proof}
Before justifying these properties, note that for any subdivision of $[a,b]$ we have: \[\sum_{i=0}^{n-1} \Delta x=n\frac{b-a}n=b-a.\]
To see why (a) holds, let $k$ be a constant. We apply Theorem \ref{thm:riemannSum} to see that:
\begin{align*}
\int_a^b k\,dx &=\lim_{n\to\infty}\sum_{i=1}^n k\Delta x\\
&=\lim_{n\to\infty}k\left( \sum_{i=1}^n \Delta x\right)
&&\text{(using Theorem \ref{thm:summation})}\\
...
&&\text{(Theorem \ref{thm:riemannSum})}\\
&\leq \lim_{n\to\infty}\sum_{i=1}^n f(c_i)\Delta x\\
&=\int_a^b f(x)\,dx &&\text{(Theorem \ref{thm:riemannSum})}
\end{align*}
