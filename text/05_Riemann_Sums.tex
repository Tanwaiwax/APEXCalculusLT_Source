...
Consider the region given in \autoref{fig:rie1a}, which is the area under $y=4x-x^2$ on $[0,4]$. What is the signed area of this region -- i.e., what is $\int_0^4(4x-x^2)\ dx$?
...
\mtable{Approximating $\int_0^4(4x-x^2)\ dx$ in \autoref{ex_rie2}.}{fig:rie2a}{\begin{tabular}{c}
...
%\mfigure{1in}{Approximating $\int_0^4(4x-x^2)\ dx$ using the Right Hand Rule in \autoref{ex_rie2}.}{fig:rie2b}{figures/figrie2b}
...
%\mfigure{0in}{Approximating $\int_0^4(4x-x^2)\ dx$ using the Midpoint Rule in \autoref{ex_rie2}.}{fig:rie2c}{figures/figrie2c}
...
\captionof{figure}{Understanding summation notation.}\label{fig:rie_notation}
\end{center}
\end{lxfigure}
...
\example{ex_rie3}{Using summation notation}{Let the numbers $\{a_i\}$ be defined as $a_i = 2i-1$ for integers $i$, where $i\geq 1$. So $a_1 = 1$, $a_2 = 3$, $a_3 = 5$, etc. (The output is the positive odd integers). Evaluate the following summations:
...
\example{ex_rie4}{Evaluating summations using \autoref{thm:summation}}
...
Consider again $\int_0^4(4x-x^2)\ dx$. We will approximate this definite integral using 16 equally spaced subintervals and the Right Hand Rule in \autoref{ex_rie7}. Before doing so, it will pay to do some careful preparation.\index{Riemann Sum}
...
Thus approximating $\int_0^4(4x-x^2)\ dx$ with 16 equally spaced subintervals can be expressed as follows, where $\Delta x = 4/16 = 1/4$:\smallskip\\
...
\example{ex_rie7}{Approximating definite integrals using sums}{Approximate $\int_0^4 (4x-x^2)\,dx$ using the Right Hand Rule and summation formulas with 16 and 1000 equally spaced intervals.}
...
\end{flalign*}%\\
\begin{align*}\phantom{\int_0^4 (4x-x^2)\,dx}
...
Using many, many rectangles, we likely have a good approximation of $\int_0^4 (4x-x^2)\ dx$. That is, \[\int_0^4 (4x-x^2)\ dx\approx 10.666656.\eoehere\]}
...
Usually, Riemann sums are calculated using one of the three methods we have introduced. The uniformity of construction makes computations easier. We have $\Delta x_i = \Delta x = \dfrac{b-a}n$ and the $i^\text{ th}$ term of the partition is $x_i = a + i\Delta x$.  Then the Left Hand Rule uses $c_i=x_{-1}$, the Right Hand Rule uses $c_i=x_i$, and the Midpoint Rule uses $c_i=\dfrac{x_{i-1}+x_i}{2}$.
...
\example{ex_rie8}{Approximating definite integrals with sums}{Approximate $\int_{-2}^3 (5x+2)\ dx$ using the Midpoint Rule and 10 equally spaced intervals.}
...
\begin{align*}
	...
	&= \frac{45}2 = 22.5
\end{align*}
\mfigure{-1in}{Approximating $\int_{-2}^3 (5x+2)\ dx$ using the Midpoint Rule and 10 evenly spaced subintervals in \autoref{ex_rie8}.}{fig:rie8}{figures/figrie8}
...
\example{ex_rie9}{Approximating definite integrals with a sum formula}{Revisit $\int_0^4(4x-x^2)\ dx$ yet again. Approximate this definite integral using the Right Hand Rule with $n$ equally spaced subintervals.}
...
\example{ex_rie10}{Approximating definite integrals with a sum formula}{Find a formula that approximates $\int_{-1}^5 x^3\ dx$ using the Right Hand Rule and $n$ equally spaced subintervals, then take the limit as $n\to\infty$ to find the exact area.}
{We see that %Following \autoref{idea:riemann}, we have
...
\mtable[-1in]{Approximating $\int_{-1}^5 x^3\ dx$ using the Left Hand Rule and 10 evenly spaced subintervals.}{fig:rie9}{\begin{tikzpicture}
\begin{axis}[width=\marginparwidth+25pt,%
...
The following theorem states that we can use any of our three rules to find the exact value of a definite integral $\int_a^b f(x)\ dx$. It also goes two steps further. The theorem states that the height of each rectangle doesn't have to be determined following a specific rule, but could be $f(c_i)$, where $c_i$ is any point in the $i^\text{ th}$ subinterval, as discussed before Riemann Sums where defined in \autoref{def:rie_sum}.
