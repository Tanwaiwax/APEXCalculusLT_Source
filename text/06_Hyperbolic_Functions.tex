...
\mtable{Using trigonometric functions to define points on a circle and hyperbolic functions to define points on a hyperbola.}{fig:hfcircle}{\pdftooltip{\begin{tikzpicture}
...
\end{tikzpicture}}{The unit circle, with a shaded sector in the first quadrant of angle θ and area θ/2.}
\vspace{10pt}
\pdftooltip{\begin{tikzpicture}
 ...
 \node [right] at (myplot.right of origin) {\scriptsize $x$};
 \node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{The unit hyperbola.  The region in the first quadrant left of the right half and to the right of the segment connecting the origin to (coshθ,sinhθ) has been shaded.}}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{cosh is in blue, and makes a U shape above the x axis.  For positive x, cosh is well approximated by exp(x/2).  For negative x, cosh is well approximated by exp(-x/2).}%
\qquad
\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=.55\textwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,ymin=-10.9,ymax=10.9,
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{sinh is in blue, and looks vaguely like y=x*x*x.  For positive x, sinh is well approximated by exp(x/2).  For negative x, sinh is well approximated by -exp(-x/2).}
\\[20pt]
\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=.55\textwidth,tick label style={font=\scriptsize},
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{tahn is in blue, has a horizontal asymptote of y=±1 as x approaches ±∞, and behaves like y=x near the origin.  coth is in red, and looks like 1+1/x for positive x and -1+1/x for negative x.}%
\qquad
\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=.55\textwidth,tick label style={font=\scriptsize},
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{sech is in blue, has a horizontal asymptote of y=0 as x approaches ±∞, and behaves like 1-x*x/2 near the origin.  csch is in red, and looks like y=1/x.}
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{cosh for positive x is in blue.  Its inverse is in red, and has been reflected across the dashed line y=x.}
&
\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,axis equal,
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{sinh is in blue.  Its inverse is in red, and has been reflected across the dashed line y=x.}
\\[15pt]
\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{Inverse tanh is in blue, and looks like (trigonometric) tangent.  Inverse coth is in red, and looks like 1/(x-1) for x>1, and 1/(x+1) for x<-1.}
&
\pdftooltip{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
...
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}{Inverse sech is in blue, starting with a vertical asymptote as x approaches 0 from the right, and going to the point (1,0).  Inverse csch is in red, and looks like 1/x.}
\end{tabular}}
...
\begin{tabular}{lll}
\item $\ds\int\frac1{\sqrt{x^2-a^2}}\dd x$ & $\ds{}=\cosh^{-1}\left(\frac xa\right)+C$; $0<a<x$ & $\ds{}=\ln\left(x+\sqrt{x^2-a^2}\right)+C$ \\
...
\item $\ds\int\frac1{x\sqrt{a^2-x^2}}\dd x$ &
$\ds{}=-\frac1a\sech^{-1}\frac{\abs x}a+C$; $0<\abs x<a$ & $\ds{}=\frac1a\ln\abs{\frac x{a+\sqrt{a^2-x^2}}}+C$ \\
...
\item $\ds\int\frac1{x\sqrt{x^2+a^2}}\dd x$ & $\ds{}=-\frac1a\csch^{-1}\frac{\abs x}a+C$; $x\neq 0,\ a>0$ & $\ds{}=
