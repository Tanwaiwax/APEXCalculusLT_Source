...
\mfigure{0in}{Graphing $\ds f(x)=\frac{1}{1+x^2}$.}{fig:improper1}{figures/figimproper1}
...
{\begin{enumerate}
\item		Let $f$ be a continuous function on $[a,\infty)$. For $t \geq a$ let \index{integration!improper}\index{improper integration}\index{convergence!of improper int.}\index{divergence!of improper int.}
...
\mfigure{0in}{A graph of $f(x) = \frac{1}{x^2}$ in \autoref{ex_impint1} part 1.}{fig:impint1a}{figures/figeximpint1a}
	\begin{align*}
	...
	\item	\mbox{}\\[-3\baselineskip]
\mfigure{0in}{A graph of $f(x) = \frac{1}{x}$ in \autoref{ex_impint1} part 2.}{fig:impint1b}{figures/figeximpint1b}
	\begin{align*}
	...
	\item	\mbox{}\\[-3\baselineskip]
\mfigure{0in}{A graph of $f(x) = e^x$ in \autoref{ex_impint1} part 3.}{fig:impint1c}{figures/figeximpint1c}
	\begin{align*}
		\int_{-\infty}^0 e^x \ dx
...
\mfigure{0in}{A graph of $f(x) = \frac{1}{1+x^2}$ in \autoref{ex_impint1} part 4.}{fig:impint1d}{figures/figeximpint1d}
\begin{align*}
...
\mfigure{0in}{A graph of $f(x) = \frac{\ln x}{x^2}$ in \autoref{ex_impint2}.}{fig:impint2}{figures/figeximpint2}
\begin{align*}
...
\mfigure{0in}{A graph of $f(x)=\frac{1}{\sqrt{x}}$ in \autoref{ex_impint3}.}{fig:impint3}{figures/figimpint3}
...
\int_{-1}^1\frac1{x^2}\ dx &= -\frac1x\Big|_{-1}^1\\
			...
			&=-2.
\end{align*}
\mfigure{0in}{A graph of $f(x)=\frac{1}{x^2}$ in \autoref{ex_impint3}.}{fig:impint3b}{figures/figimpint3b}
...
Our first tool is to understand the behavior of functions of the form $\ds \frac1{x\primeskip^p}$.
...
\mfigure{0in}{Plotting functions of the form $1/x\,^p$ in \autoref{ex_impint4}.}{fig:impint4}{figures/figimpint4}
...
{\begin{enumerate}
\item		The improper integral $\ds \int_1^\infty\frac1{x\primeskip^p}\ dx$ converges when $p>1$ and diverges when $p\leq 1.$\index{convergence!of improper int.}\index{divergence!of improper int.}
\item		The improper integral $\ds \int_0^1\frac1{x\primeskip^p}\ dx$ converges when $p<1$ and diverges when $p\geq 1.$
\end{enumerate}}

A basic technique in determining convergence of improper integrals is to compare an integrand whose convergence is unknown to an integrand whose convergence is known. We often use integrands of the form $1/x\primeskip^p$ to compare to as their convergence on certain intervals is known. This is described in the following theorem.
...
\mfigure{0in}{Graphs of $f(x) = e^{-x^2}$ and $f(x)= 1/x^2$ in \autoref{ex_impint5}.}{fig:impint5}{figures/figimpint5}
...
\mfigure{0in}{Graphs of $f(x) = 1/\sqrt{x^2-x}$ and $f(x)= 1/x$ in \autoref{ex_impint5}.}{fig:impint5b}{figures/figimpint5b}
\end{enumerate}}
...
\mfigure{0in}{Graphing $f(x)=\frac{1}{\sqrt{x^2+2x+5}}$ and $f(x)=\frac1x$ in \autoref{ex_impint6}.}{fig:impint6}{figures/figimpint6}
\[\lim_{x\to\infty} \frac{x^2}{x^2+2x+5}.\]
This converges to 1, meaning the original limit also converged to 1. As $x$ gets very large, the function $\frac1{\sqrt{x^2+2x+5}}$ looks very much like $\frac1x$. Since we know that $\ds\int_3^{\infty} \tfrac1x\ dx$ diverges, by the Limit Comparison Test we know that $\ds\int_3^\infty\tfrac1{\sqrt{x^2+2x+5}}\ dx$ also diverges. \autoref{fig:impint6} graphs $f(x)=1/\sqrt{x^2+2x+5}$ and $f(x)=1/x$, illustrating that as $x$ gets large, the functions become indistinguishable.}
