...
\int e^x\cos x\, dx & = \frac{1}{2}\bigl(e^x\sin x + e^x\cos x\bigr).
\end{align*}
...
\solution
The same strategy of $dv=dx$ that we used above works here.  Let $u=\tan^{-1} x$ and $dv=dx$.  Then $du=1/(1+x^2)\,dx$ and $v=x$.  The Integration by Parts formula gives
