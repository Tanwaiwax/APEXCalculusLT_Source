\section{Choosing the Right Technique}\label{sec:int_techniques}
...
We used this idea when we were looking at integrals involving trigonometric functions. For example consider the following integral. $$\int \cos^2 x\ dx$$ the integral can't be done as it is, however by recalling the identity, $$\cos^2 x = \frac{1}{2}(1 + \cos 2x)$$ the integral becomes very easy to do.
...
% todo an example (and exercises?) with Weierstrass' method (x=tan(t/2))

% todo reduction formulas?

\inputexercises{exercises/06_09_exercises}
