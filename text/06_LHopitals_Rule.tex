...
\theorem{thm:LHR}{L'H\^opital's Rule}
{\begin{enumerate}
...
\end{enumerate}}
% todo if we define ``an open interval containing infinity'', then these could be combined
...
\[\lim_{x\to 0} \frac{2x}{\sin x}  \stackrel{\ \text{ by LHR \rule[-5pt]{0pt}{3pt}} \ }{=} \frac{2}{\cos x} = 2 .\]
Thus $\ds \lim_{x\to0}\frac{x^2}{1-\cos x}=2.$

\item \mbox{}\\[-2\baselineskip]
\[\lim_{x\to-3}\frac{x^3+27}{x^2+9} =\frac 0{18}=0\]
We cannot use l'H\^opital's Rule in this case because the original limit does not return an indeterminate form, so l'H\^opital's Rule does not apply.
...
\youtubeVideo{kEnwac_9lyg}{L'Hospitals Rule --- Indeterminate Powers}
...
Interpretation: since this limit evaluates to 0, it means that for large $x$, there is essentially no difference between $\ln (x+1)$ and $\ln x$; their difference is essentially 0.
	...
	\item	The limit $\ds \lim_{x\to\infty} x^2-e^x$ initially returns the indeterminate form $\infty-\infty$. We can rewrite the expression by factoring out $x^2$; $\ds x^2-e^x = x^2\left(1-\frac{e^x}{x^2}\right).$ We need to evaluate how $e^x/x^2$ behaves as $x\to\infty$:
...
\]\clearpage}
{\begin{enumerate}
\item		This is equivalent to a special limit given in \autoref{thm:lim_continuous}; these limits have important applications within mathematics and finance. Note that the exponent approaches $\infty$ while the base approaches 1, leading to the indeterminate form $1^\infty$. Let $f(x) = (1+1/x)^x$; the problem asks to evaluate $\ds\lim_{x\to\infty}f(x)$. Let's first evaluate $\ds \lim_{x\to\infty}\ln\big(f(x)\big)$.
\begin{align*}
...
\lim_{x\to0^+} \ln\big(f(x)\big) &= \lim_{x\to0^+} \ln\left(x^x\right) \\
			...
			&= 0.
\end{align*}
\mfigure{0in}{A graph of $f(x)=x^x$ supporting the fact that as $x\to 0^+$, $f(x)\to 1$.}{fig:LHR4}{figures/figLHR4}
