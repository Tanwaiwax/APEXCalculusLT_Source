...
\begin{theorem}[L'H\^opital's Rule, Part 1]\label{thm:LHR_1}
...
\begin{theorem}[L'H\^opital's Rule, Part 2]\label{thm:LHR_2}
...
\begin{example}[Using L'H\^opital's Rule]\label{ex_lhr1}
...
\begin{example}[Applying L'H\^opital's Rule to other indeterminate forms]\label{ex_LHR3}
...
\begin{keyidea}[{\parbox[t]{200pt}{Evaluating Limits Involving Indeterminate Forms $0^0$, $1^\infty$ and $\infty^0$}}]\label{idea:LHR_power}
...
\begin{example}[Using L'H\^opital's Rule with indeterminate forms involving exponents]\label{ex_LHR4}
Evaluate the following limits.
\[
