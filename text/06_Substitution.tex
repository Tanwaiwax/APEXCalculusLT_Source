...
\mnote{\textbf{Note:} Recall from \autoref{sec:differentials} that the differential of $x$, denoted $dx$, is any nonzero real number.  If $u$ is a function of $x$, then the differential of $u$, denoted $du$, is defined by $du=u'(x)\ dx$.}
...
\example{ex_sub1}{Integrating by substitution}{Evaluate $\ds \int x\sin(x^2+5)\ dx$.}
...
\example{ex_sub2}{Integrating by substitution}{Evaluate $\ds \int \cos(5x)\ dx$.}
...
\example{ex_sub10}{Integrating by substitution}{Evaluate $\ds \int \sin x\cos x\ dx$.}
{There is not a composition of function here to exploit; rather, just a product of functions. Do not be afraid to experiment; when given an integral to evaluate, it is often beneficial to think ``If I let $u$ be \textit{this}, then $du$ must be \textit{that} \dotso'' and see if this helps simplify the integral at all.
...
\example{ex_sub4}{Integrating by substitution}{Evaluate $\ds\int x\sqrt{x+3}\ dx$.}
...
\example{ex_sub5}{Integrating by substitution}{Evaluate $\ds \int \frac{1}{x\ln x}\ dx$.}
...
The next three examples will help fill in some missing pieces of our antider\-ivative knowledge. We know the antiderivatives of the sine and cosine functions; what about the other standard functions tangent, cotangent, secant and cosecant? We discover these next.

\example{ex_sub6}{Integration by substitution: antiderivatives of $\tan x$}{Evaluate $\ds \int \tan x\ dx.$}
...
\example{ex_sub7}{Integrating by substitution: antiderivatives of $\sec x$}{Evaluate $\ds\int \sec x\ dx$.}
...
{
\tcbset{grow to right by=20pt}
\begin{theorem}[Antiderivatives of Trigonometric Functions]\label{thm:triganti}
\begin{minipage}{.45\linewidth}\index{integration!of trig. functions}\small
	\begin{enumerate}
...
\example{ex_sub11}{Integration by alternate methods}{Evaluate $\ds\int \frac{x^2+2x+3}{\sqrt{x}}\ dx$ with, and without, substitution.}
...
\example{ex_sub12}{Definite integrals and substitution: changing the bounds}{Evaluate $\ds\int_0^2 \cos(3x-1)\ dx$ using \autoref{thm:subst_def_int}.}
...
\mtable{Graphing the areas defined by the definite integrals of \autoref{ex_sub12}.}{fig:subst12}{\begin{tikzpicture}
...
\mtable{Graphing the areas defined by the definite integrals of \autoref{ex_subst13}.}{fig:subst13}{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
...
\end{tikzpicture}
\\(b)}
\example{ex_subst13}{Definite integrals and substitution: changing the bounds}{Evaluate $\ds \int_0^{\pi/2} \sin x \cos x\ dx$ using \autoref{thm:subst_def_int}.}
{We saw the corresponding indefinite integral in \autoref{ex_sub10}. In that example we set $u = \sin x$ but stated that we could have let $u = \cos x$. For variety, we do the latter here.
...
\example{ex_subst_def_3}{Definite integrals and substitution: changing the bounds}{Evaluate $\ds\int_0^2 xe^{x^2+1}\ dx$ using \autoref{thm:subst_def_int}.}{We note the composition of functions and let $u=x^2+1$, hence $du=2x\,dx$. We divide the differential by 2 to get $\frac{du}2=x\,dx$.
...
\begin{align*}
	...
	&= \frac12(e^5-e^1) \\
	&= \frac e2(e^4-1).\eoehere %\approx 147.41316\\
\end{align*}}
