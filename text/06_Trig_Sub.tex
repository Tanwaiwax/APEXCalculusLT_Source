...
\mbox{}\\[-2\baselineskip]\index{integration!trig. subst.}\begin{enumerate}
	\item For integrands containing $\sqrt{a^2-x^2}$:\smallskip\\
	...
	\item For integrands containing $\sqrt{x^2+a^2}$:\smallskip\\
...
\solution
Using \autoref{idea:trigsub}(b), we recognize $a=\sqrt{5}$ and  set $x= \sqrt{5}\tan \theta$. This makes $\dd x = \sqrt{5}\sec^2\theta\dd\theta$. We will use the fact that $\sqrt{5+x^2} = \sqrt{5+5\tan^2\theta} = \sqrt{5\sec^2\theta} = \sqrt{5}\sec\theta.$ Substituting, we have:
\begin{align*}
...
\end{align*}
So we have $a=1/2$, and following \autoref{idea:trigsub}(c), we set $x= \frac12\sec\theta$, and hence $\dd x = \frac12\sec\theta\tan\theta\dd\theta$. %The Key Idea also shows that $\sqrt{x^2-1/2^2} = \frac12\tan\theta$. 
...
\mtable[-\baselineskip]{A reference triangle for \autoref{ex_trigsub2}}{fig:sec_tri}{\begin{tikzpicture}
...
\solution
We use \autoref{idea:trigsub}(a) with $a=2$, $x=2\sin \theta$, $\dd x = 2\cos \theta d\theta$ and hence $\sqrt{4-x^2} = 2\cos\theta$. This gives
\begin{align*}
...
Using \autoref{idea:trigsub}(b), let $x=\tan\theta$, $\dd x=\sec^2\theta\dd\theta$ and note that $x^2+1 = \tan^2\theta+1 = \sec^2\theta$. Thus
\begin{align*}
...
\solution
Using \autoref{idea:trigsub}(b), we set $x=5\tan\theta$, $\dd x = 5\sec^2\theta\dd\theta$, and note that $\sqrt{x^2+25} = 5\sec\theta$. As we substitute, we change the bounds of integration.
