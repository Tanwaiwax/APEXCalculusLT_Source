...
%\[\frac{d}{dx}\bigl(\tan^{-1}5x\bigr) = \frac{5}{1+25x^2}.\]
...
%	\item $\ds \int \sec^3\theta\ d\theta = \frac12\Bigl(\sec \theta\tan \theta + \ln\abs{\sec \theta+\tan \theta}\Bigr)+C$
%	\item	$\ds \int \cos^2\theta\ d\theta = \int \frac12\bigl(1+\cos(2\theta)\bigr)\ d\theta = \frac12\bigl(\theta+\sin\theta\cos\theta\bigr)+C.$
%\end{enumerate}
%\end{keyidea}
