...
\subsection{Integrals of the form \texorpdfstring{$\ds \int \sin^m x\cos^n x\ dx$}{∫(sin x)\^{}m (cos x)\^{}n dx}}
...
\subsection{Integrals of the form \texorpdfstring{$\ds\int\tan^mx\sec^nx\ dx$}{∫(tan x)\^{}m (sec x)\^{}n dx}}
...
\subsection{Integrals of the form \texorpdfstring{$\ds\int\cot^mx\csc^nx\,dx$}{∫(cot x)\^{}m (csc x)\^{}n dx}}
...
\subsection{Integrals of the form \texorpdfstring{$\ds \int\sin(mx)\sin(nx)\ dx,$ $\ds\int \cos(mx)\cos(nx)\ dx$, and $\ds\int \sin(mx)\cos(nx)\ dx$.}{∫sin(mx)sin(nx)dx, ∫cos(mx)cos(nx)dx, and ∫sin(mx)cos(nx)dx}}
