...
\example{ex_trigint1}{Integrating powers of sine and cosine}{Evaluate $\ds\int\sin^5x\cos^8x\ dx$.}
...
\example{ex_trigint2}{Integrating powers of sine and cosine}{Evaluate $\ds \int\sin^5x\cos^9x\ dx$.}
{The powers of both the sine and cosine factors are odd, therefore we can apply the techniques of \autoref{idea:trig_int_1} to either power. We choose to work with the power of the cosine factor since the previous example used the sine factor's power.
...
\end{align*}
%
%Another approach would be to rewrite $\sin^5x$ as
%\begin{align*}
% \sin^5x&=\sin^4x\sin x\\
% &=(1-\cos^2x)^2\sin x\\
...
% todo Tim easier to use u=cos x. How to phrase
...
\]
\mfigure{0in}{A plot of $f(x)$ and $g(x)$ from \autoref{ex_trigint2} and the Technology Note.}{fig:trigint2}{figures/figtrigint2}%
...
\example{ex_sub8}{Integrating powers of sine and cosine}{Evaluate $\ds \int \sin^2x\ dx$.}
...
\example{ex_trigint3}{Integrating powers of sine and cosine}{Evaluate $\ds\int\cos^4x\sin^2x\ dx$.}
...
\example{ex_trigint5}{Integrating powers of tangent and secant}{Evaluate $\ds\int \tan^2x\sec^6x\ dx$.}
...
\example{ex_trigint6}{Integrating powers of tangent and secant}{Evaluate $\ds\int \sec^3x\ dx$.}
...
\end{align*}
\mnote{\textbf{Note:} Remember that in \autoref{ex_sub7}, we found that $\int\sec x\ dx=\ln\abs{\sec x+\tan x}+C$}
...
\example{ex_trigint7}{Integrating powers of tangent and secant}{Evaluate $\ds\int\tan^6x\ dx$.}
...
\example{ex_int_cot_csc}{Integrating powers of cotangent and cosecant}{Evaluate $\ds\int\cot^2x\csc^4x\,dx$}{Since the power of cosecant is even we will let $u=\cot x$ and save a $\csc^2x$ for the resulting $du=-\csc^2x\,dx$.
\begin{align*}
...
\[-\frac13\cot^3x-\frac15\cot^5x+C.\eoehere\]}
...
\example{ex_trigint4}{Integrating products of $\sin(mx)$ and $\cos(nx)$}{Evaluate $\ds\int\sin(5x)\cos(2x)\ dx$.}
...
% todo Tim Fourier series problem?