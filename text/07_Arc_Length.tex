...
\begin{tikzpicture}[alt={Blue y = sin x arch on 0 < x < pi, peak at (pi⁄2 , 1); ticks at pi⁄4, π⁄2, 3pi⁄4.}]
...
\end{tikzpicture}
\\(a)\\
\begin{tikzpicture}[alt={Same sin curve; red polyline through five black nodes approximates it on 0 < x < pi.}]
 \begin{axis}[width=\marginparwidth,%height=.46\marginparwidth,
...
%
\mtable{A graph of $f(x) = x^{3/2}$ from \autoref{ex_arc1}.}{fig:arc1}{\begin{tikzpicture}[alt={Blue y = x^3/2 curve on 0 < x < 4 rises steeply, reaching about 8 at x = 4.}]
...
\mtable{A graph of $f(x) =\frac18x^2-\ln x$ from \autoref{ex_arc2}.}{fig:arc2}{\begin{tikzpicture}[alt={Blue y = 1/8x^2 − ln x on 0 < x < 3 dives from 1, bottoms near (2, -0.2), then turns upward.}]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
...
\myincludeasythree{Blue arc y = f(x) between x = a and x = b; short red chord shows one sample slice}.
...
3Droo=150.00000160608386}{\marginparwidth}{Revolving that slice about the x-axis gives a purple band on a blue tubular surface.}{figures/figarc4_b_3D}
% todo Tim should this be 3D at all?
\\(a)\\
\myincludeasythree{Blue band on blue tube: frustum formed when the red chord of the slice is revolved about the x-axis}
3Droll=126.0060482236849,
3Dortho=0.004875946324318647,
3Dc2c=0.44462037086486816 0.39410829544067383 0.8043577671051025,
3Dcoo=67.21983337402344 3.5258262157440186 -29.566415786743164,
...
\myincludeasythree{Blue sinus-shaped surface: y = sin x (0 < x < pi) revolved about the x-axis, axes shown}
...
\myincludeasythree{Blue horn-like surface: y = x^2 (0 < x < 1) rotated about x-axis, tip at (0,0,0) flaring to x = 1}
...
\\(a)\\[15pt]
\myincludeasythree{Blue parabolic bowl: y = x^2 (0 < x < 1) revolved about the y-axis; vertex at origin, rim near y = 1}
3Droll=138.26595091591264,
3Dortho=0.0048407199792563915,
...
\myincludeasythree{Infinite blue trumpet (Gabriel’s Horn): surface from rotating y = 1 / x, 1 < x < infty, about the x-axis}
