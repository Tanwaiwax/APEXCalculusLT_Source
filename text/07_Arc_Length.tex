...
This \textit{is} a Riemann Sum. As long as \fp\ is continuous, we can invoke \autoref{thm:riemannSum} and conclude
...
x & \sqrt{1+\cos^2x} \\ \midrule
 ...
 \pi  & \sqrt{2} \\
\end{array}$$}
\autoref{fig:arc3} gives $\sqrt{1+\cos^2x}$ evaluated at 5 evenly spaced points in $[0,\pi]$. Simpson's Rule then states that 
\begin{align*}
...
\[A=2\pi r L,\]
where $r$ is the average of the two radii.  The length is given by $L$; we use the material just covered by arc length to state that
...
\end{enumerate}}
{\begin{enumerate}
	\item		Like the integral in \autoref{ex_sa1}, this integral is easier to setup than to actually integrate:
\begin{align*}
...
\end{align*}
	The solid formed by revolving $y=x^2$ around the $x$-axis is graphed in \autoref{fig:sa2} (a).
