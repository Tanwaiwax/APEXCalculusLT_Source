...
\begin{tikzpicture}
 \begin{axis}[width=1.16\marginparwidth,%height=.46\marginparwidth,
...
\mtable{Zooming in on the $i^{\text{th}}$ subinterval $[x_{i-1},x_i]$ of a partition of $[a,b]$.}{fig:arcintro2}{\begin{tikzpicture}
...
\autoref{fig:arcintro2} zooms in on the $i^{\text{th}}$ subinterval where $y=f(x)$ is approximated by a straight line segment. The dashed lines show that we can view this line segment as the hypotenuse of a right triangle whose sides have length $\dx_i$ and $\dy_i$. Using the Pythagorean Theorem, the length of this line segment is $\ds \sqrt{(\dx_i)^2 + (\Delta y_i)^2}.$ Summing over all subintervals gives an arc length approximation
$$L \approx \sum_{i=1}^n \sqrt{(\dx_i)^2 + (\Delta y_i)^2}.$$

As it is written, this is \textit{not} a Riemann Sum. While we could conclude that taking a limit as the subinterval length goes to zero gives the exact arc length, we would not be able to compute the answer with a definite integral. We need first to do a little algebra.
...
\mfigure{0in}{A graph of $f(x) = x^{3/2}$ from \autoref{ex_arc1}.}{fig:arc1}{figures/figarc1}	
\example{ex_arc1}{Finding arc length}{Find the arc length of $f(x) = x^{3/2}$ from $x=0$ to $x=4$.}
...
\begin{align*}
	...
	&=	\frac{8}{27}\left(10^{3/2}-1\right)\text{units}.\eoehere % \approx 9.07 
\end{align*}}

\example{ex_arc2}{Finding arc length}{Find the arc length of $\ds f(x) =\frac18x^2-\ln x$ from $x=1$ to $x=2$.}
{A graph of $f$ is given in \autoref{fig:arc2}; the portion of the curve measured in this problem is in bold.
\mfigure{0in}{A graph of $f(x) =\frac18x^2-\ln x$ from \autoref{ex_arc2}.}{fig:arc2}{figures/figarc2}	
...
\begin{align*}
	...
	&=	\frac38+\ln 2\ \text{units}.\eoehere % \approx 1.07
\end{align*}}
...
\example{ex_arc3}{Approximating arc length numerically}{Find the length of the sine curve from $x=0$ to $x=\pi$.}
...
\mtable[-.5in]{Establishing the formula for surface area.}{fig:surface_intro}{%
...
3Droo=150.00000160608386}{width=\marginparwidth}{figures/figarc4_b_3D}
...
3Dc2c=0.44462037086486816 0.39410829544067383 0.8043577671051025,
3Dcoo=67.21983337402344 3.5258262157440186 -29.566415786743164,
3Droo=150.00000160608386}{width=\marginparwidth}{figures/figarc4_3D}
...
\example{ex_sa1}{Finding surface area of a solid of revolution}{Find the surface area of the solid formed by revolving $y=\sin x$ on $[0,\pi]$ around the $x$-axis, as shown in \autoref{fig:sa1}.}
...
3Dcoo=72.58285522460938 2.891160011291504 -26.545438766479492,
3Droo=149.99998703790175,
3Dlights=Headlamp,add3Djscript=asylabels.js}{}{0in}{Revolving $y=\sin x$ on $[0,\pi]$ about the $x$-axis.}{fig:sa1}{figures/figsa1}
%\mfigure{0in}{Revolving $y=\sin x$ on $[0,\pi]$ about the $x$-axis.}{fig:sa1}{figures/figsa1}
% todo write out the integration in ``Finding surface area of a solid of revolution''
\begin{align*}
	\text{SA}
	&=	2\pi\int_0^\pi \sin x\sqrt{1+\cos^2x}\ dx \\
	&=	-2\pi\int_1^{-1}\sqrt{1+u^2}\ du \qquad\qquad \text{substitute $u=\cos x$} \\
	&=	2\pi\int_{-\pi/4}^{\pi/4}\sec^3\theta\ d\theta \qquad\qquad \text{substitute $u=\tan\theta$} \\
...
(b)
\end{tabular}}
\example{ex_sa2}{Finding surface area of a solid of revolution}{Find the surface area of the solid formed by revolving the curve $y=x^2$ on $[0,1]$ about:\\
\null\hfill 1.\qquad the $x$-axis\hfill2.\qquad the $y$-axis.\hfill\null}
{\begin{enumerate}
...
%\mfigure{0in}{A graph of Gabriel's Horn.}{fig:gabriel}{figures/figgabriel}
}
