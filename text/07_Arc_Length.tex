...
\mfigure{0in}{Zooming in on the $i\,^\text{th}$ subinterval $[x_i,x_{i+1}]$ of a partition of $[a,b]$.}{fig:arcintro2}{figures/figarcintrod}
%
...
\end{align*}
\mfigure{0in}{A graph of $f(x) = x^{3/2}$ from Example \ref{ex_arc1}.}{fig:arc1}{figures/figarc1}	
...
\end{align*}
\mfigure{0in}{A graph of $f(x) =\frac18x^2-\ln x$ from Example \ref{ex_arc2}.}{fig:arc2}{figures/figarc2}	
...
\mtable{A table of values of $y=\sqrt{1+\cos^2x}$ to evaluate a definite integral in Example \ref{ex_arc3}.}{fig:arc3}{%
$$\begin{array}{cc}
...
Find the surface area of the solid formed by revolving the curve $y=x^2$ on $[0,1]$ about:
		...
		\item		the $y$-axis.
		\end{enumerate}
\mtable{The solids used in Example \ref{ex_sa2}.}{fig:sa2}{%
\begin{tabular}{c}
\myincludegraphicsthree{width=125pt,3Dmenu,activate=onclick,deactivate=onclick,
...
Our final example is a famous mathematical ``paradox.''\bigskip
