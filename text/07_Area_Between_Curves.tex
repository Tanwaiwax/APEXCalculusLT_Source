...
\solution
\mtable{Graphing an enclosed region in \autoref{ex_abc1}.}{fig:abc1}{\begin{tikzpicture}[alt={Blue upper curve sin x + 2 and lower curve 0.5 cos 2x − 1; shaded area between curves from x 0 to 4 pi; axes x and y.}]
...
\mtable{Sketching the region enclosed by $y=x^2+x-5$ and $y=3x-2$ in \autoref{ex_ftc6}.}{fig:ftc6}{\begin{tikzpicture}[alt={Blue parabola y = x^2 + x − 5 and blue line y = 3x − 2 intersect at −1 and 3; axes x and y.}]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
...
\mtable{Graphing a region for \autoref{ex_abc3}.}{fig:abc3}{\begin{tikzpicture}[alt={Blue region: upper curve y = sqrt(x)+2 on 0–1, then y = -(x-1)^2+3 on 1–2; lower edge y = 2; axes x and y.}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
...
\mtable{The region used in \autoref{ex_abc3} with boundaries relabeled as functions of $y$.}{fig:abc3b}{\begin{tikzpicture}[alt={Horizontal view: shaded band between y = 2 and y = 3; left edge x = (y-2)^2, right edge x = sqrt(3-y)+1; a red horizontal slice shows Delta y; axes x and y.}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,xtick={1,2},
ymin=-.1,ymax=3.5,xmin=-.1,xmax=2.5]
\addplot[draw={\coloronefill},fill={\coloronefill},area style,domain=0:1]{sqrt(x)+2}\closedcycle;
