...
% todo: make this 3d figure into color?
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{}{0in}{Orienting a pyramid along the $x$-axis in \autoref{ex_disk0}.}{fig:disk0}{figures/figcross_area1}
...
3Dcoo=60.78499984741211 0.48087620735168457 -14.641631126403809,
3Droo=150.00000157638937,
3Dlights=Headlamp,add3Djscript=asylabels.js}{}{0in}{Cutting a slice in the pyramid in \autoref{ex_disk0} at $x=3$.}{fig:disk0a}{figures/figcross_area1a}
...
Start with a function $y=f(x)$ from $x=a$ to $x=b$. Revolving this curve about a horizontal axis creates a three-dimensional solid whose cross sections are disks (thin circles), perpendicular to the axis of rotation. Let $R(x)$ represent the radius of the cross-sectional disk at $x$; the area of this disk is $\pi [R(x)]^2$. Applying \autoref{thm:volume_by_cross_section} gives the Disk Method.
 ...
 \begin{tabular}{c}
  \begin{tikzpicture}
   \begin{axis}[width=\marginparwidth+25pt,
    ...
    \draw[{\colorone},fill={\coloronefill}] plot[smooth,domain=1:2]
...
\example{ex_disk1}{Finding volume using the Disk Method}{Find the volume of the solid formed by revolving the curve $y=1/x$, from $x=1$ to $x=2$, around the $x$-axis.}
...
\mtable{Sketching a solid in \autoref{ex_disk2}.}{fig:disk2}{%
  ...
  \begin{tikzpicture}
   \begin{axis}[width=\marginparwidth+25pt,
    ...
    \draw [thick,{\colortwo}] (axis cs: 0,.75) --
...
\example{ex_disk2}{Finding volume using the Disk Method}{Find the volume of the solid formed by revolving the curve $y=1/x$, from $x=1$ to $x=2$, about the $y$-axis.}
{Since the axis of rotation is vertical, our perpendicular cross sections have thickness $\Delta y$ and radius $x=R(y)$. We need to convert the function into a function of $y$ and convert the $x$-bounds to $y$-bounds. Since $y=1/x$ defines the curve, we rewrite it as $x=1/y$. The bound $x=1$ corresponds to the $y$-bound $y=1$, and the bound $x=2$ corresponds to the $y$-bound $y=1/2$. 

Thus we are rotating the curve $x=1/y$, from $y=1/2$ to $y=1$ about the $y$-axis to form a solid. The region of revolution is sketched in \autoref{fig:disk2}(a), the curve and sample sample disk are sketched in \autoref{fig:disk2}(b), and a full sketch of the solid is in \autoref{fig:disk2}(b). We integrate to find the volume:
\begin{align*}
...
\begin{tabular}{ccc}
 \begin{tikzpicture}
  \begin{axis}[width=\marginparwidth+25pt,
    ...
    \draw[{\colorone},fill={\coloronefill}]
...
%(c)
\end{tabular}
\caption{Establishing the Washer Method.}\label{fig:washeridea}
\end{figure}
...
\begin{tabular}{c}
 \begin{tikzpicture}
  \begin{axis}[width=\marginparwidth+25pt,tick label style={font=\scriptsize},
    ...
    extra x ticks={1},ytick={1},ymin=-.2,ymax=1.2,xmin=-.1,xmax=1.2]
   ...
   \draw [thick,{\colortwo}] (axis cs: 0,.81) --
    node[pos=.4,above,color=black]{\scriptsize $R(x)$}(axis cs:.9,.81);
   \draw [thick,{\colortwo}] (axis cs: 0,.5) --
    node[pos=.4,above,color=black]{\scriptsize $r(x)$} (axis cs:.25,.5);
  \end{axis}
  \node [right] at (myplot.right of origin) {\scriptsize $x$};
  \node [above] at (myplot.above origin) {\scriptsize $y$};
 \end{tikzpicture}
...
\begin{tabular}{c}
 \begin{tikzpicture}
  \begin{axis}[width=\marginparwidth+25pt,tick label style={font=\scriptsize},
    ...
    \draw[{\colorone},fill={\coloronefill}] (axis cs:1,1) -- (axis cs:2,1) --(axis cs:2,3)-- cycle;
    \draw [thick,{\colortwo}] (axis cs: 0,2.6) --
      node[pos=.4,below,color=black]{\scriptsize$r(x)$} (axis cs: 1.8,2.6);
    \draw [thick,{\colortwo}] (axis cs: 0,1.2) --
...
\mtable[-2in]{Sketching the solid in \autoref{ex_wash_4}.}{fig:wash4}{\begin{tabular}{c}
 \begin{tikzpicture}
  \begin{axis}[width=\marginparwidth+25pt,
     tick label style={font=\scriptsize},axis y line=middle,
     axis x line=middle,name=myplot,axis on top,
   ...
   \addplot [{\coloronefill},fill={\coloronefill},domain=0:1] {sqrt(x)};
   \addplot [{\coloronefill},fill={\coloronefill},domain=0:1] {x};
   \addplot [{\colorone},smooth,thick,domain=0:1] {x};
   \addplot [{\colorone},smooth,thick,domain=0:1,samples=100] {sqrt x};
   \draw [dashed] (axis cs:-.2,2) -- (axis cs:2.2,2);
   \draw [{\colortwo},thick] (axis cs:.81,0) --
    node[pos=.5,right,color=black]{\scriptsize$R(x)$} (axis cs:.81,.9);
   \draw [{\colortwo},thick] (axis cs:.3,0) --
