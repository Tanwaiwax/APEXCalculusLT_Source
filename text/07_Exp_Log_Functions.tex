...
\keyidea{ki_exp_func_props}{Properties of Exponential Functions}{For $a>0$ and $a\neq1$ the exponential function $f(x)=a^x$ satisfies:\\
...
\keyidea{ki_log_laws}{Properties of Logarithms}{For $a,x,y>0$ and $a\ne1$, we have\\
\begin{minipage}{.5\linewidth}
\begin{enumerate}
\item$\log_a(xy)=\log_ax+\log_ay$\stepcounter{enumi}
\item$\log_xy=\dfrac{\log_ay}{\log_ax}$, when $x\ne1$\stepcounter{enumi}
\item$\log_a1=0$
\end{enumerate}
\end{minipage}\begin{minipage}{.5\linewidth}
\begin{enumerate}\stepcounter{enumi}
\item$\log_a\dfrac xy=\log_ax-\log_ay$\stepcounter{enumi}
\item$\log_ax^y=y\log_ax$\stepcounter{enumi}
\item$\log_aa=1$
\end{enumerate}
\end{minipage}}
...
\keyidea{ki_log_func_props}{Properties of Logarithmic Functions}{For $a>0$ and $a\ne1$ the logarithmic function $f(x)=\log_a x$ satisfies:
\begin{enumerate}
\item The domain of $f(x)=log_a x$ is $(0,\infty)$ and the range is $(-\infty,\infty)$.
\item $y=\log_a x$ if and only if $a^y=x$.
\item $\ds\lim_{x\to\infty}\log_a x=\begin{cases}\infty&\text{ if $a>1$}\\-\infty&\text{ if $a<1$}\end{cases}$
...
Now we
%In \autoref{sec:deriv_inverse_function}, \autoref{ex_deriv_lnx}, we found that $\frac{d}{dx}\ln x=\frac1x$. We
...
\theorem{thm_int_exp_log}{Derivatives and Antiderivatives of Exponentials and Logarithms}{Given a base $a>0$ and $a\neq 1$, the following hold:\\
\begin{minipage}[t]{.5\textwidth}
\begin{enumerate}
\item $\dfrac d{dx}e^x=e^x$
\item $\dfrac d{dx}a^x=a^x\ln a$
\item $\dfrac d{dx}\ln x=\dfrac1x$
\item $\dfrac d{dx}\log_a x=\dfrac1{x\ln a}$
\end{enumerate}
\end{minipage}%
\begin{minipage}[t]{.5\textwidth}
...
\mtable{A plot of $y=x^x$.}{fig:logdiffa}{\begin{tikzpicture}
...
\example{ex_implicit10}{Using Logarithmic Differentiation}{Given $y=x^x$, use logarithmic differentiation to find $y\primeskip'$.}
...
\mtable{A graph of $y=x^x$ and its tangent line at $x=1.5$.}{fig:implicit10}{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,
ymin=-.1,ymax=4.2,xmin=-.1,xmax=2.2]
\addplot[draw={\colorone},smooth,thick,domain=.01:2] {exp(x*ln(x))};
...
\printexercises{exercises/07_exp_log_exercises}
