...
\mtable[-2\baselineskip]{A function $f$ along with its inverse $f\primeskip^{-1}$. (Note how it does not matter which function we refer to as $f$; the other is $f\primeskip^{-1}$.)}{fig_inverse1}{\pdftooltip{\begin{tikzpicture}
...
\mtable{The function $f(x)=x^2$ is not one-to-one.}{fig_parab_oto}{\pdftooltip{\begin{tikzpicture}
...
\pdftooltip{\begin{tikzpicture}[x=1ex,y=1ex]
 \draw(0,0)
