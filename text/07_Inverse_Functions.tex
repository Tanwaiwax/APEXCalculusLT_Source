...
\keyidea{ki_inv_funcs}{Inverse Functions}{For a one-to–one function $f$,
\begin{itemize}
...
\item To check the composition we plug $f(x)$ in for $x$ in the definition of $g$ as follows: \[ g(f(x))=\frac{f(x)-1}3=\frac{(3x+1)-1}3=\frac{3x}3=x\]
...
%\[\text{1.}\quad f(x)=5x+3\qquad\qquad\text{2.}\quad g(x)=x^3+1.\]}{\begin{enumerate}
...
\begin{tikzpicture}[baseline={(current bounding box.center)}]
 \begin{axis}[x=.7cm, y=.7cm,
  ...
  \addplot [{\colorone},smooth,thick,domain=-3.14:3.14] (x,{sin(deg(x))});
 \end{axis}
...
\\ (a) & (b)
\end{tabular}
\caption{(a) A portion of $y=\sin x$. (b) A one--to--one portion of $\sin x$ along with $\sin^{-1}x$.}
\label{fig_sin_arc}
\end{lxfigure}
...
\begin{tikzpicture}[baseline={(current bounding box.center)}]
 \begin{axis}[x=.5cm,y=.5cm,
  ...
  \addplot [{\colorone},smooth,thick,domain=-1.3:1.3] (x,{tan(deg(x))})
   node[pos=.8,above right]{$\tan x$};
  \addplot [{\colortwo},smooth,thick,domain=-1.3:1.3] ({tan(deg(x))},x)
...
\\ (a) & (b)
\end{tabular}
\caption{(a) A portion of $y=\tan x$. (b) A one--to--one portion of $y=\tan x$ along with $\tan^{-1}$.}
\label{fig_tan_arc}
\end{lxfigure}
...
\example{eg_inv_trig}{Evaluating Inverse Trigonometric Functions}{Find exact values for the following:
...
\end{tikzpicture}}
We use this information to construct a right triangle with angle $\theta$, where the adjacent side over the hypotenuse must equal 11/15. Applying the Pythagorean Theorem we find that
\[y=\sqrt{15^2-11^2}=\sqrt{104}=2\sqrt{26}.\]
Finally, we have:
\[\tan(\cos^{-1}(11/15))=\tan(\theta)=\frac{2\sqrt{26}}{11}.\eoehere\]
\end{enumerate}}
