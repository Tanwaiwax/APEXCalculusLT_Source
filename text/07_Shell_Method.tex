...
\begin{align*}
	...
	&= 2\pi\int_0^1 \bigl(6x-2x^2\bigr)\ dx \\
...
\begin{example}[Finding volume using the Washer Method and the Shell Method]\label{ex_shell_wash}
...
% Horizontal Axis  & $\ds \pi\int_a^b \bigl(R(x)^2-r(x)^2\bigr)\ dx$ & & $\ds 2\pi\int_c^d r(y)h(y)\ dy$ \\ \\
% Vertical Axis &  $\ds\pi \int_c^d\bigl(R(y)^2-r(y)^2\bigr)\ dy$ & & $\ds 2\pi\int_a^b r(x)h(x)\ dx$
% \end{tabular}
