...
\mnote[-1in]{\textbf{Factorial:} The expression $3!$ refers to the number $3\cdot2\cdot1 = 6$.
\index{factorial}\\
...
\mtable[.5in]{Plotting sequences in \autoref{ex_seq1}.}{fig:seq1b}{%
\begin{tikzpicture}
...
\noindent We gave one extra term to begin to show the pattern of signs is ``$-$, $-$, $+$, $+$, $-$, $-$, \dots'', due to the fact that the exponent of $-1$ is a special quadratic. This sequence is plotted in \autoref{fig:seq1b}(c).\eoehere
\end{enumerate}
\end{example}
...
\item\mbox{}\\[-2.5\baselineskip]
%
\mtable[2.5\baselineskip]{A plot of $\{a_n\}=\{\frac{n^2+1}{n+1}\}$ in \autoref{ex_seq7}(b).}{fig:seq7b}{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
...
\item%
%
\mtable[4\baselineskip]{A plot of $\{a_n\}=\{\frac{n^2-9}{n^2-10n+26}\}$ in \autoref{ex_seq7}(c).}{fig:seq7c}{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
