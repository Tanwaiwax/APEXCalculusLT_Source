...
\begin{tikzpicture}[alt={Blue dots for sequence 3^n / n^! at 1 ≤ n ≤ 4; y values =3, 4.5, 4.5, 3.4; plotted on discrete n-axis.}]
...
\end{tikzpicture}
\\(a)\\
\begin{tikzpicture}[alt={Blue dots for sequence 4 + (–1)^n at 1 ≤ n ≤ 4; points alternate 3 and 5; discrete n-axis plot.}]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,xtick={1,2,3,4},
ytick={1,2,3,4,5},ymin=-.1,ymax=5.5,xmin=-.1,xmax=4.5]
...
\end{tikzpicture}
\\(b)\\
\begin{tikzpicture}[alt={Blue dots for sequence (–1)^{(n+1)/2}/n^2 at 1 ≤ n ≤ 4; alternating signs, magnitudes shrink toward 0.}]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
...
\mtable{Scatter plots of the sequences in \autoref{ex_seq4}.}{fig:seq4}{%
 \begin{tikzpicture}[alt=Blue points for the sequence aₙ = (3n^2 − 2 n + 1)/(n^2 − 1000) with 20 < n < 100; y values drop from about 8 to a cluster near 3.}]
  \begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
 ...
 \end{tikzpicture}
\\(a)\\
\begin{tikzpicture}[alt={Blue points for a_n = cos n with 1 < n < 100; dots oscillate densely in −1 < y < 1.}]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
...
\end{tikzpicture}
\\(b)\\
\begin{tikzpicture}[alt={Blue points for aₙ = (−1)^n⁄n with 1 < n < 20; alternating signs shrink toward 0 in −1 < y < 1}]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
...
\end{align*}
%
\mtable{A plot of a sequence in \autoref{ex_seq5}, part 2.}{fig:seq5}{\begin{tikzpicture}[alt={Blue dots for a_n = (−1)^n·(n + 1)/n with 1 < n < 20; positives drop from 2 to 1, negatives rise −2 to −1; alternating toward 1 on y-axis.}]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
...
\begin{tikzpicture}[alt={Blue dots: a_n = 1 ⁄ n for 1 < n < 10; values decline from 1 to near 0 along the n-axis.}]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
...
\end{tikzpicture}
\\(a)\\[10pt]
\begin{tikzpicture}[alt={Blue dots: a_n = 2^n for 1 < n < 8; points soar from 2 up to about 256 on the y-axis.}]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
...
\end{aligned}$\hfill\null\\
%
\mtable[-7\baselineskip]{A plot of $\{a_n\}=\{\frac{n+1}n\}$ in \autoref{ex_seq7}(a).}{fig:seq7a}{\begin{tikzpicture}[alt={Blue dots: a_n = (n + 1) ⁄ n for 1 < n < 10; sequence slips from 2 toward 1.}]
...
\item
%
\mtable[5\baselineskip]{A plot of $\{a_n\}=\{\frac{n^2+1}{n+1}\}$ in \autoref{ex_seq7}(b).}{fig:seq7b}{\begin{tikzpicture}[alt={Blue dots: a_n = (n^2 + 1) ⁄ (n + 1) for 1 < n < 10; roughly linear rise, y climbs 2 to 10.}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
...
\item%
%
\mtable[10\baselineskip]{A plot of $\{a_n\}=\{\frac{n^2-9}{n^2-10n+26}\}$ in \autoref{ex_seq7}(c).}{fig:seq7c}{\begin{tikzpicture}[alt={Blue dots: a_n = (n^2 − 9) ⁄ (n^2 − 10n + 26) for 1 < n < 11; peaks near n = 3 then tapers.}]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
...
\item
%
\mtable{A plot of $\{a_n\}=\{n^2/n!\}$ in \autoref{ex_seq7}(d).}{fig:seq7d}{\begin{tikzpicture}[alt={Blue dots: a_n = n^2 ⁄ n^! for 1 < n < 11; rapid drop from 2 to near 0.}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
