...
\item Does the series contain factorials or constants raised to powers involving $n$? If so, then the Ratio Test may work. Note that if the series term contains a factorial then the only test that we have that will work is the Ratio Test.  (If you find that $L>1$, then the divergence test also would have worked.)
...
\begin{example}[Testing Series]\label{eg_strat_test}
...
\printexercises{exercises/08-Series-Strategies}
