...
\mtable[.1in]{Plotting $f$ and $p_{13}$.}{fig:taypolyintroc}{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
ymin=-5.5,ymax=8.5,xmin=-4.2,xmax=4.4]
\addplot [draw={\colorone},domain=-4:4,smooth,thick,samples=50] {exp(x)*sin(deg(x))*cos(deg(x))+2};
...
%\begin{align*}
%y\primeskip' &= y^2\\
%\frac{d}{dx}\bigl(y\primeskip'\bigr) &= \frac{d}{dx}\bigl(y^2\bigr)\\
...
%\begin{align*}
%\frac{d}{dx}\bigl(y\primeskip''\bigr) &= \frac{d}{dx} \bigl(2yy\primeskip'\bigr)\\
