...
\definition{def:tangent_par}{Tangent Lines}
...
\example{ex_parcalc1}{Tangent and Normal Lines to Curves}{Let $x=5t^2-6t+4$ and $y=t^2+6t-1$, and let $C$ be the curve defined by these equations.
\begin{enumerate}
...
{\begin{enumerate}
	...
	\mfigure{0in}{Graphing tangent and normal lines in \autoref{ex_parcalc1}.}{fig:parcalc1}{figures/figparcalc1}
	% todo the tangent and normal lines aren't dark enough?
...
\example{ex_parcalc2}{Tangent and Normal Lines to a Circle}{\mbox{}\\[-2\baselineskip]
\begin{enumerate}
...
\begin{align*}
	...
	&= (\tan t_0)x,
\end{align*}%
\mfigure{0in}{Illustrating how a circle's normal lines pass through its center.}{fig:parcalc2}{figures/figparcalc2}%
...
% todo refer back to previous section??
...
\mfigure{0in}{A graph of an astroid.}{fig:parcalc3}{figures/figplanecurve1}
}
...
% todo should this be in terms of f and g?
...
\example{ex_parcalc4}{Concavity of Plane Curves}{Let $x=5t^2-6t+4$ and $y=t^2+6t-1$ as in \autoref{ex_parcalc1}. Determine the $t$-intervals on which the graph is concave up/down.}
...
\end{align*}
\mfigure{0in}{Graphing the parametric equations in \autoref{ex_parcalc4} to demonstrate concavity.}{fig:parcalc4}{figures/figparcalc4}
...
\example{ex_parcalc5}{Concavity of Plane Curves}{Find the points of inflection of the graph of the parametric equations $x=\sqrt{t}$, $y=\sin t$, for $0\leq t\leq 16$.}
...
\mtable{In (a), a graph of $\frac{d^2y}{dx^2}$, showing where it is approximately 0. In (b), graph of the parametric equations in \autoref{ex_parcalc5} along with the points of inflection.}{fig:parcalc5}{%
\myincludegraphics{figures/figparcalc5b}\\(a)\\
\myincludegraphics{figures/figparcalc5}\\(b)}
...
\keyidea{ki_param_area}{Area Under a Parametric Curve}{The area under the parametric curve given by $x=f(t)$, $y=g(t)$, for $f(\alpha)=a<x<b=f(\beta)$ is
...
\example{eg_cycloid_area}{Finding the area under a parametric curve}{Determine the area under the cycloid given by the parametric equations
\[x=6(\theta-\sin\theta)\qquad y=6(1-\cos\theta)\qquad 0\le\theta\le2\pi.\]}{First, notice that we've switched the parameter to $\theta$ for this problem.  This is to make sure that we don't get too locked into always having $t$ as the parameter.
...
Recall in \autoref{sec:arc_length} we found the arc length of the graph of a function, from $x=a$ to $x=b$, to be $$L = \int_a^b\sqrt{1+\left(\frac{dy}{dx}\right)^2}\ dx.$$
...
\example{ex_parcalc6}{Arc Length of a Circle}{Find the arc length of the circle parametrized by $x=3\cos t$, $y=3\sin t$ on $[0,3\pi/2]$. 
}
...
\example{ex_parcalc7}{Arc Length of a Parametric Curve}{The graph of the parametric equations $x=t(t^2-1)$, $y=t^2-1$ crosses itself as shown in \autoref{fig:parcalc7}, forming a ``teardrop.'' Find the arc length of the teardrop.
\mfigure{0in}{A graph of the parametric equations in \autoref{ex_parcalc7}, where the arc length of the teardrop is calculated.}{fig:parcalc7}{figures/figparcalc7}}
...
\example{ex_parcalc8}{Surface Area of a Solid of Revolution}{Consider the teardrop shape formed by the parametric equations $x=t(t^2-1)$, $y=t^2-1$ as seen in \autoref{ex_parcalc7}. Find the surface area if this shape is rotated about the $x$-axis, as shown in \autoref{fig:parcalc8}.}
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.5,trim=5mm 5mm 5mm 5mm,clip}{0in}{Rotating a teardrop shape about the $x$-axis in \autoref{ex_parcalc8}.}{fig:parcalc8}{figures/figparcalc8}}
