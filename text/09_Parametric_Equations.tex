...
\begin{gathered}
\text{Use a function}\\f\text{ to find }x\\\big(x=f(t)\big)\\\\
\text{Use a function}\\g\text{ to find }y\\\big(y=g(t)\big)
\end{gathered}
...
{Let $f$ and $g$ be continuous functions on an interval $I$. The \textbf{graph} of the \textbf{parametric equations} $x=f(t)$ and $y=g(t)$ is the set of all points $\big(x,y\big) = \big(f(t),g(t)\big)$ in the Cartesian plane, as the \textbf{parameter} $t$ varies over $I$. A \textbf{curve} is a graph along with the parametric equations that define it.
...
\mtable[-1in]{A table of values of the parametric equations in \autoref{ex_pareq1} along with a sketch of their graph.}{fig:pareq1}{%
...
2 & 4 & 3
\end{tabular}\\(a)\\
\myincludegraphics{figures/figpareq1}\\(b)}
\example{ex_pareq1}{Plotting parametric functions}{Plot the graph of the parametric equations $x=t^2$, $y=t+1$ for $t$ in $[-2,2]$.}
...
\end{tabular}
\\(a)\\
\myincludegraphics{figures/figpareq2}\\(b)}
...
\mtable{The equation $f(x)=x^2-x-6$ with different parameterizations.}{fig_parab_param}{\begin{tikzpicture}
...
\example{ex_pareq5}{Converting from rectangular to parametric}{Find parametric equations for $f(x)=x^2-x-6$.}
...
\example{ex_par_circle}{Converting from rectangular to parametric}{Find parametric equations for the circle $x^2+y^2=4$.}{We will present three different approaches:
...
\example{ex_para_ellipse}{Converting from rectangular to parametric}{Find a parameterization that traces the ellipse $\dfrac{(x-2)^2}{9}+\dfrac{(y+3)^2}{4}=1$ starting at the point $(-1,-3)$ in a clockwise direction.}{Applying the Pythagorean Identity, $\cos^2t+\sin^2t=1$, we set $\cos^2 t=\dfrac{(x-2)^2}{9}$ and $\sin^2 t=\dfrac{(y+3)^2}{4}$. Solving these equations for $x$ and $y$ we set $x=-3\cos t+2$ and $y=2\sin t-3$  for $0\leq t\leq 2\pi$.}

\example{ex_para_hyper}{Converting from rectangular to parametric}{Find a parameterization for the hyperbola $\dfrac{(x-2)^2}{9}-\dfrac{(y-3)^2}{4}=1$.}{We use an alternative form of the Pythagorean Identity, $\sec^2t-\tan^2t=1$. We let  $\ds \sec^2 t=\frac{(x-2)^2}{9}$ and $\tan^2 t=\dfrac{(y-3)^2}{4}$. Solving these equations for $x$ and $y$ we have $x=3\sec t +2$ and $y=2\tan t +3$ for $0\leq t\leq 2\pi$.}

\example{ex_pareq6}{Converting from rectangular to parametric}{An object is fired from a height of 0ft and lands 6 seconds later, 192ft away. Assuming ideal projectile motion, the height, in feet, of the object can be described by $h(x) = -x^2/64+3x$, where $x$ is the distance in feet from the initial location. (Thus $h(0) = h(192) = 0$ft.) Find parametric equations $x=f(t)$, $y=g(t)$ for the path of the projectile where $x$ is the horizontal distance the object has traveled at time $t$ (in seconds) and $y$ is the height at time $t$.}
...
\mfigure{0in}{Graphing projectile motion in \autoref{ex_pareq6}.}{fig:pareq6}{figures/figpareq6}
...
\example{ex_pareq7}{Eliminating the parameter}{Find a rectangular equation for the curve described by $$ x= \frac{1}{t^2+1}\quad \text{and}\quad y=\frac{t^2}{t^2+1}.$$ }
...
\mfigure{0in}{Graphing parametric and rectangular equations for a graph in \autoref{ex_pareq7}.}{fig:pareq7}{figures/figpareq7}
...
\example{ex_pareq8}{Eliminating the parameter}{Eliminate the parameter in $x=4\cos t+3$, $y= 2\sin t+1$}
...
\mfigure{0in}{Graphing the parametric equations $x=4\cos t+3$, $y=2\sin t+1$ in \autoref{ex_pareq8}.}{fig:pareq8}{figures/figpareq8} 
...
\example{ex_pareq3}{Shifting the graph of parametric functions}{Sketch the graph of the parametric equations $x=t^2+t$, $y=t^2-t$.  Find new parametric equations that shift this graph to the right 3 units and down 2.}
...
\mfigure{0in}{A graph of the parametric equations from \autoref{ex_pareq4}.}{fig:pareq4}{figures/figpareq4}
\begin{align*}
...
% todo Tim folium of Descartes should be in Implicit Equation section?
 ...
 \begin{tabular}{ccc}
  ...
  \begin{tikzpicture}
   \begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
                ...
                ymin=-.2,ymax=6.2,xmin=-.1,xmax=6.3]
    \addplot [thick,{\colorone}, smooth,domain=0:6.28,samples=20]
  ...
  \begin{tikzpicture}
   \begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
                axis y line=middle,axis x line=middle,name=myplot,
                %xtick={-1,1},ytick={-1,1},
                ymin=-.1,ymax=6.1,xmin=-3.1,xmax=3.1]
    \addplot [thick,{\colorone}, smooth,domain=-1.55:1.55,samples=20]
     ({2*x},{2/(1+x^2)});
   \end{axis}
   \node [right] at (myplot.right of origin) {\scriptsize $x$};
   \node [above] at (myplot.above origin) {\scriptsize $y$};
  ...
  \begin{tikzpicture}
   \begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
                axis y line=middle,axis x line=middle,name=myplot,
                %xtick={-1,1},ytick={-1,1},
                ymin=-2.1,ymax=2.1,xmin=-2.1,xmax=2.1]
    \addplot [thick,{\colorone}, smooth,domain=-40:130,samples=20]
...
\example{ex_pareq9}{Determine where a curve is not smooth}{Let a curve $C$ be defined by the parametric equations $x=t^3-12t+17$ and $y=t^2-4t+8$. Determine the points, if any, where it is not smooth.}
{We begin by taking derivatives. 
\mfigure{0in}{Graphing the curve in \autoref{ex_pareq9}; note it is not smooth at $(1,4)$.}{fig:pareq9}{figures/figpareq9}
...
\example*{ex_cycloid}{The Cycloid}{A well-known parametric curve is the cycloid.  Fix $r$, and let $x=r(t-\sin t)$, $y=r(1-\cos t)$.  This represents the path traced out by a point on a wheel of radius $r$ as starts rolling to the right.  We can think of $t$ as the angle through which the point has rotated.
 
\begin{lxfigure}
 \flushinner{%
 \begin{tikzpicture}[scale=.8]
   \foreach \x in { 0, 2.09, 4.19, 6.28, 8.38, 10.47, 12.57} {
    \draw [{\colorone}] (\x,1) circle (1);
    \draw [{\colorone},fill={\colorone}] (\x,1) circle (2pt)
     -- ({\x-sin(deg(\x))},{1-cos(deg(\x))}) circle (2pt);
   }
   \draw (0,2.5) node {$t=0$};
   \foreach \t/\n in { 2/6.28, 4/12.57} {
    \draw (\n,2.5) node {$t=\t\pi$};
   }
   \foreach \t/\n in { 2/2.09, 4/4.19, 8/8.38, 10/10.47} {
    \draw (\n,2.5) node {$t=\frac{\t\pi}3$};
   }
   \draw [{\colortwo},smooth,thick,domain=0:12.56,variable=\t]
    plot ({\t-sin(deg(\t))},{1-cos(deg(\t))});
 \end{tikzpicture}}
 \caption{A cycloid traced through two revolutions.}
 \label{fig_cycloid}
\end{lxfigure}
 
\autoref{fig_cycloid} shows a cycloid sketched out with the wheel shown at various places.  The blue dot is the point on the wheel that we're using to trace out the curve.
