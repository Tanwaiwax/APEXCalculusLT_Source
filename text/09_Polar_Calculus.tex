...
\end{align*}
%
\mtable{The limaçon in \autoref{ex_polcalc1} with its tangent line at $\theta=\pi/4$ and points of vertical and horizontal tangency.}{fig:polcalc1}{\begin{tikzpicture}[alt={Blue limacon with inner loop; red tangent at θ = pi⁄4; filled/open dots show four vertical or horizontal tangent points.}]
...
\[
\begin{tikzpicture}[alt={Circle sector with radius r and angle theta; accompanying note states its area A = 1/2 theta r^2.},x=30pt,y=30pt,thick]
...
%
\mtable[-.5in]{Computing the area of a polar region.}{fig:polararea}{%
\begin{tikzpicture}[alt={Blue r = f(θ) with dashed spokes; shaded wedge from θ = alpha to beta.}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,
...
\end{tikzpicture}
\\(a)\\
\begin{tikzpicture}[alt={Red r = f(θ) with dashed spokes; shaded wedge from θ = alpha to Beta}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,
ymin=-.1,ymax=1.1,xmin=-.1,xmax=1.1,axis equal]
\addplot
...
%
\mtable[\baselineskip]{Finding the area of the shaded region of a cardioid in \autoref{ex_polcalc4}.}{fig:polcalc4}{\begin{tikzpicture}[alt={Cardioid sector: blue curve r = f(θ) with shaded slice between θ = 0 and pi⁄6, bounded by two radii.}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
...
\mtable{Illustrating area bound between two polar curves.}{fig:polarea3}{\begin{tikzpicture}[alt={Blue outer r_2 and red inner r_1 enclose shaded region between θ = alpha and Beta, inner arc from r_1 to pole.}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,
...
\mtable{Finding the area between polar curves in \autoref{ex_polcalc5}.}{fig:polcalc5}{\begin{tikzpicture}[alt={Blue circle r = 1 intersects red cardioid r = 1 − cos theta; right‐hand lens (−pi∕2 ≤ theta ≤ pi∕2) is shaded.}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
...
\solution
%
\mtable[-10\baselineskip]{Graphing the region bounded by the functions in \autoref{ex_polcalc6}.}{fig:polcalc6}{%
\begin{tikzpicture}[alt={Blue rose r = sin 4 theta overlaps red three‐leaf r = sin 2 theta; intersection rays at theta = +/- pi∕6, +/- pi∕2.}]
...
\end{tikzpicture}
\\(a)\\[10pt]
\begin{tikzpicture}[alt={Close-up: sector aplha ≤ theta ≤ beta between the two curves is filled to illustrate the integrand region.}]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
...
\end{align*}
%
\mtable[-6\baselineskip]{The limaçon in \autoref{ex_polcalc7} whose arc length is measured.}{fig:polcalc7}{\begin{tikzpicture}[alt={Limacon r = 1 + 2 cos theta plotted; small inner loop (theta around pi) and outer loop (whole 0 ≤ θ < 2 pi) }]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
...
\begin{tikzpicture}[alt={Red and blue roses r = cos 2 theta and r = sin 2 theta share four petals, alternating every pi∕4.}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
