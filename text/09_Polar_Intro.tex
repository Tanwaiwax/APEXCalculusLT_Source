...
\begin{tikzpicture}[alt={Origin, initial ray on x-axis 0; second ray makes angle θ (counter-clockwise) marked on arc to point (r, θ)..},>=latex]
...
\fi
\begin{tikzpicture}[alt={Three concentric circles (r = 1, 2, 3); points: A at 30 deg (pi / 6), B at 210 deg (7pi / 6), C at 315 deg (7pi / 4), D at 240 deg (4pi / 3).}]
...
\mtable{Converting between rectangular and polar coordinates.}{fig:polar_intro2}{\begin{tikzpicture}[alt={Right triangle: legs x and y meet at origin, hypotenuse r to point P, angle θ at origin.
},scale=.75,>=latex]
...
\mtable{Plotting standard polar plots.}{fig:polar3}{\begin{tikzpicture}[alt={Concentric polar rings r = 0.5, 1 (bold), 1.5 with a thick ray at θ = pi⁄4 crossing r = 1.5.},>=latex]
 \draw [thick,->] (0,0) node [below] {$O$} -- (2.5,0);
 \foreach \x in {1,2}
...
\begin{tabular}{c}
 {$\begin{array}{cc}
	...
	11\pi/6 & 1+\sqrt{3}/2 \\
 \end{array}$} \\ \\
 \begin{tikzpicture}[alt={Table then polar grid; right-facing cardioid r = 1 + cos θ touches r = 0 at pi and peaks at 2 on initial ray.},scale=.95]
...
\mtable[-2in]{Polar plots from \autoref{ex_polar5}.}{fig:polar5}{%
 \begin{tikzpicture}[alt={Rectangular plot of xy=1; hyperbola branches in quadrants I and III approach both coordinate axes.},scale=1]
  \draw [dashed,gray] (-2.1,0) -- (0,0);
...
\begin{enumerate}
	...
	\end{align*}
%
\mtable{Graphing $xy=1$ from \autoref{ex_polar6}.}{fig:polar6}{\begin{tikzpicture}[alt={ALT-TEXT-TO-BE-DETERMINED}]
...
\begin{tikzpicture}[alt={ALT-TEXT-TO-BE-DETERMINED},scale=.9,>=stealth]
	\draw [<->,] (-2.1,0) -- (2.1,0);
	\draw [<->,] (0,-2.1) -- (0,2.1);
