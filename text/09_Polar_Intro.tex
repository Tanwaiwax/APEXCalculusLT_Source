...
\else
 \ifthenelse{\isodd{\pageref{ex_polar1}}}{%
...
\end{tikzpicture}}%
\ifthenelse{\boolean{latexml}}{here.}{at the bottom of this page.}%
\mfigure{0in}{Plotting polar points in \autoref{ex_polar1}.}{fig:polar1}{figures/figpolar1}
...
\mfigure{0in}{Converting between rectangular and polar coordinates.}{fig:polar_intro2}{figures/figpolarintro2}
...
\example{ex_polar2}{Converting Between Polar and Rectangular Coordinates}{\mbox{}\\[-2\baselineskip]
\begin{enumerate}
...
\example{ex_polar3}{Introduction to Graphing Polar Functions}{Describe the graphs of the following polar functions.
\begin{enumerate}
	\item $r = 1.5$
	\item $\theta = \pi/4 $
\end{enumerate}
...
\mtable{Graph of the polar function in \autoref{ex_polar4} by plotting points.}{fig:polar4}{%
\begin{tabular}{c}
 $\begin{array}{cc}
 ...
 \begin{tikzpicture}[scale=1]
  ...
  \draw [thick,{\colorone},domain=0:360,samples=60,smooth] plot
   ({cos(\x)*(1+cos(\x))},{sin(\x)*(1+cos(\x))});
  \foreach \x in { 0,30,45,60, 90,120,135,150, 180,210,225,240, 270,300,315,330 }
   { \filldraw ({\x}:{1+cos(\x)}) circle (1pt); };
\end{tikzpicture}
\end{tabular}}%
\example{ex_polar4}{Sketching Polar Functions}{Sketch the polar function $r=1+\cos \theta$ on $[0,2\pi]$ by plotting points.}
{A common question when sketching curves by plotting points is ``Which points should I plot?'' With rectangular equations, we often chose ``easy'' values --- integers, then added more if needed. When plotting polar equations, start with the ``common'' angles -- multiples of $\pi/6$ and $\pi/4$. \autoref{fig:polar4} gives a table of just a few values of $\theta$ in $[0,\pi]$. 

Consider the point $P(0,2)$ determined by the first line of the table. The angle is 0 radians -- we do not rotate from the initial ray -- then we go out 2 units from the pole. When $\theta=\pi/6$, $r = 1+\sqrt{3}/2$; so rotate by $\pi/6$ radians and go out $1+\sqrt{3}/2$ units.}
...
%\mfigure{0in}{Using  technology to graph a polar function.}{fig:polar4b}{figures/figpolar4b}\\

\example{ex_polar5}{Sketching Polar Functions}{Sketch the polar function $r=\cos (2\theta)$ on $[0,2\pi]$ by plotting points.}
...
\mtable{Polar plots from \autoref{ex_polar5}.}{fig:polar5}{%
 \begin{tikzpicture}[scale=1]
  \draw [dashed,gray] (-2.1,0) -- (0,0);
...
%  \draw [thick,{\colorone}] plot coordinates {(2.,0)(0.866,0.5)(0,0)(-0.5,-0.866)(0,-2.)(0.5,-0.866)(0,0)(-0.866,0.5)(-2.,0)(-0.866,-0.5)(0,0)(0.5,0.866)(0,2.)(-0.5,0.866)(0,0)(0.866,-0.5)(2,0)};
  ...
  \draw [thick,{\colorone},domain=0:360,samples=60,smooth]
   plot ({\x}:{2*cos(2*\x)});
\end{tikzpicture}}
...
\end{array}$
\captionof{figure}{Tables of points for plotting a polar curve.}\label{fig:polar5table}
\end{center}
\end{lxfigure}
...
\example{ex_polar6}{Converting between rectangular and polar equations.}{\mbox{}\\
...
{\begin{enumerate}
	...
	\mfigure{0in}{Graphing $xy=1$ from \autoref{ex_polar6}.}{fig:polar6}{figures/figpolar6}
...
\flushinner{%
\noindent
\rule{25pt+\marginparwidth+\textwidth}{1pt}\bigskip\\
...
\myincludegraphics[scale=.9]{figures/figpolarline1} &
\myincludegraphics[scale=.9]{figures/figpolarline2} &
\myincludegraphics[scale=.9]{figures/figpolarline4} &
\myincludegraphics[scale=.9]{figures/figpolarline3}
%
\end{tabular}}
...
\textbf{With inner loop:} & \textbf{Cardiod:} & \textbf{Dimpled:} & \textbf{Convex:} \smallskip\\
...
\example{ex_polar7}{Finding points of intersection with polar curves}{Determine where the graphs of the polar equations $r=1+3\cos\theta$ and $r=\cos \theta$ intersect.}
...
\myincludegraphics{figures/figpolar7}\\(a)\\
\myincludegraphics{figures/figpolar7b}\\(b)}
