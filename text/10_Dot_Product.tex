...
3Dc2c=.54 .61 .58,
3Dcoo=0 0 40,
3Droo=170}{width=\marginparwidth}{figures/figdotpangle3D_3D}
\\(b)}
...
\mtable{Vectors used in \autoref{ex_dotp2}.}{fig:dotp2}{\begin{tikzpicture}[>=stealth]
...
3Dc2c=.9 .12 .42,
3Dcoo=0 50 30,
3Droo=250}{width=.8\marginparwidth}{figures/figdotp4b_3D}
\\(b)
\ifthenelse{\boolean{in_threeD}}{}{%
\\[15pt]
\myincludegraphics[width=.8\marginparwidth]{figures/figdotp4c_3D} % myinc g 3D
\\(c)}% end ifthenelse
}% end mtable
...
\mtable{Illustrating the orthogonal projection.}{fig:dotpprojc}{\begin{tikzpicture}[>=stealth]
...
\example{ex_dotp6}{Orthogonally decomposing a force vector}{Consider \autoref{fig:dotp6}(a), showing a box weighing 50 lb on a ramp that rises 5 ft over a span of 20 ft. Find the components of force, and their magnitudes, acting on the box (as sketched in part (b) of the figure):
\begin{enumerate}
	\item in the direction of the ramp, and
	\item	orthogonal to the ramp.
\end{enumerate}
%
\mtable[-8\baselineskip]{Sketching the ramp and box in \autoref{ex_dotp6}. Note: \textit{The vectors are not drawn to scale.}}{fig:dotp6}{%
...
\mtable{Finding work when the force and direction of travel are given as vectors.}{fig:dotpwork}{\begin{tikzpicture}[>=stealth,scale=.8]
...
{Let $\vec F$ be a constant force that moves an object in a straight line from point $P$ to point $Q$. Let $\vec d = \vv{PQ}$. The \textbf{work} $W$ done by $\vec F$ along $\vec d$ is $W = \dotp Fd$.\index{work}}

\mtable{Computing work when sliding a box up a ramp in \autoref{ex_dotp7}.}{fig:dotp7}{\begin{tikzpicture}[>=stealth,scale=.3]
...
\example{ex_dotp7}{Computing work}{A man slides a box along a ramp that rises 3 ft over a distance of 15 ft by applying 50 lb of force as shown in \autoref{fig:dotp7}. Compute the work done.}
