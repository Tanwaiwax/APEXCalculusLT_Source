...
\example{ex_dotp1}{Evaluating dot products}{\mbox{}\\[-2\baselineskip]\begin{enumerate}
...
\mfigure{0in}{Vectors used in \autoref{ex_dotp2}.}{fig:dotp2}{figures/figdotp2}

\example{ex_dotp2}{Using the dot product to find angles}{Let $\vec u = \bracket{3,1}$, $\vec v = \bracket{-2,6}$ and $\vec w = \bracket{-4,3}$, as shown in \autoref{fig:dotp2}. Find the angles $\alpha$, $\beta$ and $\theta$.}
...
\begin{align*}
	...
	&\approx 2.1763 \approx 124.7^\circ\eoehere
\end{align*}}
...
3Dcoo=10 50 46,
3Droo=200,
3Dlights=Headlamp,add3Djscript=asylabels.js}{}{0in}{Vectors used in \autoref{ex_dotp3}.}{fig:dotp3}{figures/figdotp3}%
%\mfigure{0in}{Vectors used in \autoref{ex_dotp3}.}{fig:dotp3}{figures/figdotp3}

\example{ex_dotp3}{Using the dot product to find angles}{Let $\vec u = \bracket{1,1,1}$, $\vec v = \bracket{-1,3,-2}$ and $\vec w = \bracket{-5,1,4}$, as illustrated in \autoref{fig:dotp3}. Find the angle between each pair of vectors.}
{\begin{enumerate}
...
\example{ex_dotp4}{Computing the orthogonal projection}{\mbox{}\\[-2\baselineskip]\begin{enumerate}
...
{\mbox{}\\[-\baselineskip]\begin{enumerate}
	...
	\proj wx &= \frac{\dotp wx}{\dotp xx}\vec x \\
					...
					&=\bracket{2,2,2}.
	\end{align*}
	These vectors are sketched in \autoref{fig:dotp4}(b), and again in part (c) from a different perspective. Because of the nature of graphing these vectors, the sketch in part (b) makes it difficult  to recognize that the drawn projection has the geometric properties it should. The graph shown in part (c) illustrates these properties better.\eoehere
\end{enumerate}}

\mfigure{0in}{Illustrating the orthogonal projection.}{fig:dotpprojc}{figures/figdotpprojc}
...
\example{ex_dotp5}{Orthogonal decomposition of vectors}{\mbox{}\\[-2\baselineskip]\begin{enumerate}
...
\end{enumerate}}
{\mbox{}\\[-\baselineskip]\begin{enumerate}
	\item In \autoref{ex_dotp4}, we found that $\proj uv =\bracket{-1.5,-0.5}$. Let $$\vec z = \vec u - \proj uv =\bracket{-2,1}-\bracket{-1.5,-0.5}=\bracket{-0.5, 1.5}.$$
...
\example{ex_dotp6}{Orthogonally decomposing a force vector}{Consider \autoref{fig:dotp6}(a), showing a box weighing 50lb on a ramp that rises 5ft over a span of 20ft. Find the components of force, and their magnitudes, acting on the box (as sketched in part (b) of the figure):
\begin{enumerate}
	\item in the direction of the ramp, and
	\item	orthogonal to the ramp.
\end{enumerate}
\mtable{Sketching the ramp and box in \autoref{ex_dotp6}. Note: \textit{The vectors are not drawn to scale.}}{fig:dotp6}{%
\includegraphics{figures/figdotp6}\\
(a)\\[15pt]
\includegraphics{figures/figdotp6b}\\
(b)}}
{As the ramp rises 5ft over a horizontal distance of 20ft, we can represent the direction of the ramp with the vector $\vec r=\bracket{20,5}$. Gravity pulls down with a force of 50lb, which we represent with $\vec g =\bracket{0,-50}$. 
\begin{enumerate}
	...
	\end{align*}
	The magnitude of this force is $\norm{\vec z} \approx 48.51$lb. In physics and engineering, knowing this force is important when computing things like static frictional force. (For instance, we could easily compute if the static frictional force alone was enough to keep the box from sliding down the ramp.)\eoehere
\end{enumerate}}
...
\mfigure{0in}{Finding work when the force and direction of travel are given as vectors.}{fig:dotpwork}{figures/figdotpwork}
\begin{align*}
...
\example{ex_dotp7}{Computing work}{A man slides a box along a ramp that rises 3ft over a distance of 15ft by applying 50lb of force as shown in \autoref{fig:dotp7}. Compute the work done.}
{The figure indicates that the force applied makes a $30^\circ$ angle with the horizontal, so $\vec F = 50\bracket{\cos 30^\circ,\sin 30^\circ}\approx\bracket{43.3,25}.$ The ramp is represented by $\vec d  =\bracket{15,3}$. The work done is simply
$$\dotp Fd = 50\bracket{\cos 30^\circ,\sin 30^\circ}\cdot\bracket{15,3}\approx 724.5 \text{ft--lb}.$$

\mfigure{0in}{Computing work when sliding a box up a ramp in \autoref{ex_dotp7}.}{fig:dotp7}{figures/figdotp7}
