...
\captionof{figure}{Illustrating the relationship between the angle between vectors and the sign of their dot product.}
\label{fig:dotpsign}
\end{center}
\end{lxfigure}
...
\jmtVideo{98C7iv8OcnI}{vectors-the-dot-product}{Vectors: The Dot Product}
...
\example{ex_dotp2}{Using the dot product to find angles}{Let $\vec u = \la 3,1\ra$, $\vec v = \la -2,6\ra$ and $\vec w = \la -4,3\ra$, as shown in \autoref{fig:dotp2}. Find the angles $\alpha$, $\beta$ and $\theta$.}
...
\example{ex_dotp3}{Using the dot product to find angles}{Let $\vec u = \la 1,1,1\ra$, $\vec v = \la -1,3,-2\ra$ and $\vec w = \la -5,1,4\ra$, as illustrated in \autoref{fig:dotp3}. Find the angle between each pair of vectors.}
{\begin{enumerate}
...
\example{ex_dotp8}{Finding orthogonal vectors}{Let $\vec u = \la 3,5\ra$ and $\vec v = \la 1,2,3\ra$. 
\begin{enumerate}
...
\mtable{Graphing the vectors used in \autoref{ex_dotp4}.}{fig:dotp4}{%
...
\example{ex_dotp5}{Orthogonal decomposition of vectors}{\mbox{}\\[-2\baselineskip]\begin{enumerate}
	\item Let $\vec u = \la -2,1\ra $ and $\vec v = \la 3,1\ra$ as in \autoref{ex_dotp4}. Decompose $\vec u$ as the sum of a vector parallel to $\vec v$ and a vector orthogonal to $\vec v$.
	\item	Let $\vec w =\la 2,1,3\ra$ and $\vec x  =\la 1,1,1\ra$ as in \autoref{ex_dotp4}. Decompose $\vec w$ as the sum of a vector parallel to $\vec x$ and a vector orthogonal to $\vec x$.
\end{enumerate}}
{\mbox{}\\[-\baselineskip]\begin{enumerate}
	...
	\la 2,1,3\ra &= \underbrace{\la 2,2,2\ra}_{\parallel\ \vec x}\ +\ \underbrace{\la 0,-1,1\ra}_{\perp \ \vec x} \eoehere
	\end{align*}
\end{enumerate}}
