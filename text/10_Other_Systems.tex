...
\mtable{Cartesian Coordinates}{fig_cart_coord}{todo make the graph}
%{\includegraphics{fig1.7.1.0}}
...
\mtable{Spherical coordinates}{fig_sph_coord}{todo make the graph}
%{\includegraphics{fig1.7.3.0}}

\keyidea{idea_sph_coord}{Spherical coordinates $(\rho,\theta,\phi)$}{
 \begin{align*}
...
\example{ex_conv_coords}{Converting Between Coordinate Systems}{Convert the point $(-2,-2,1)$ from Cartesian coordinates to 1. cylindrical and 2. spherical coordinates.}{\begin{enumerate}\item $r = \sqrt{(-2)^2 + (-2)^2} = 2\sqrt{2}$, $\theta =
 \tan^{-1} \left( \frac{-2}{-2} \right) = \tan^{-1}(1) = \frac{5 \pi}{4}$, since $y = -2 < 0$.\\
 $\therefore  (r,\theta,z) = \left( 2\sqrt{2},\frac{5 \pi}{4},1 \right)$
 \item $\rho = \sqrt{(-2)^2 + (-2)^2 + 1^2} = \sqrt{9} = 3$, $\phi = \cos^{-1} \left( \frac{1}{3} \right)
 \approx 1.23$ radians.\\
 $\therefore  (\rho,\theta,\phi) = \left( 3,\frac{5 \pi}{4}, 1.23 \right)$
 \end{enumerate}}

For cylindrical coordinates $(r,\theta,z)$, and constants $\ssub{r}{0}$, $\ssub{\theta}{0}$ and $\ssub{z}{0}$, we see from \autoref{fig:cylcoordsurf} that the surface $r = \ssub{r}{0}$ is a cylinder of radius $\ssub{r}{0}$ centered along the $z$-axis, the surface $\theta = \ssub{\theta}{0}$ is a half-plane emanating from the $z$-axis, and the surface $z = \ssub{z}{0}$ is a plane parallel to the $xy$-plane.
...
\begin{lxfigure}
 ...
 \end{tikzpicture} \\
 (a) $r = \ssub{r}{0}$ & (b) $\theta = \ssub{\theta}{0}$ & (c) $z = \ssub{z}{0}$
 \end{tabular}}
 \captionof{figure}{Cylindrical coordinate surfaces}
 \label{fig:cylcoordsurf}
\end{lxfigure}

For spherical coordinates $(\rho,\theta,\phi)$, and constants $\ssub{\rho}{0}$, $\ssub{\theta}{0}$ and $\ssub{\phi}{0}$, we see from \autoref{fig:sphcoordsurf} that the surface $\rho = \ssub{\rho}{0}$ is a sphere of radius $\ssub{\rho}{0}$ centered at the origin, the surface $\theta = \ssub{\theta}{0}$ is a half-plane emanating from the $z$-axis, and the surface $\phi = \ssub{\phi}{0}$ is a circular cone whose vertex is at the origin.
...
\begin{lxfigure}
 ...
 \end{tikzpicture} \\
  (a) $\rho = \ssub{\rho}{0}$ & (b) $\theta = \ssub{\theta}{0}$ & (c) $\phi = \ssub{\phi}{0}$
  \end{tabular}}
 \captionof{figure}{Spherical coordinate surfaces}
...
\example{exmp_helicoid}{Identifying a Surface}{Describe the surface given by $\theta = z$ in cylindrical coordinates.}{This surface is called a \emph{helicoid}\index{helicoid}. As the (vertical) $z$ coordinate increases, so does the angle $\theta$, while the radius $r$ is unrestricted. So this sweeps out a (ruled!) surface shaped like a spiral staircase, where the spiral has an infinite radius. \autoref{fig:helicoid} shows a section of this surface restricted to $0 \le z \le 4\pi$ and $0 \le r \le 2$.
 \begin{lxfigure}
  \begin{center}
   todo make the graph %\input{fig176}
  \end{center}
 \captionof{figure}{Helicoid $\theta = z$}
 \label{fig:helicoid}
 \end{lxfigure}}
