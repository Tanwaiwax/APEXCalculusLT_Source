...
Any flat surface, such as a wall, table top or stiff piece of cardboard can be thought of as representing part of a plane. Consider a piece of cardboard with a point $P$ marked on it. One can take a nail and stick it into the cardboard at $P$ such that the nail is perpendicular to the cardboard; see \autoref{fig:planes_intro}.% -- that is, the line containing the nail is perpendicular to any line drawn on the cardboard through $P$.
...
3Droo=150,
3Dlights=Headlamp,add3Djscript=asylabels.js}{}{0in}{Illustrating defining a plane with a sheet of cardboard and a nail.}{fig:planes_intro}{figures/figplanes_intro}
%\mfigure{0in}{Illustrating defining a plane with a sheet of cardboard and a nail.}{fig:planes_intro}{figures/figplanes_intro}
...
% todo make a picture of this?
This gives us way of writing an equation describing the plane. Let $P=(x_0,y_0,z_0)$ be a point in the plane and let $\vec n =\bracket{a,b,c}$ be a normal vector to the plane. A point $Q = (x,y,z)$ lies in the plane defined by $P$ and $\vec n$ if, and only if, $\vv{PQ}$ is orthogonal to $\vec n$. Knowing $\vv{PQ} =\bracket{x-x_0,y-y_0,z-z_0}$, consider:

\begin{align}
\vv{PQ}\cdot\vec n &= 0 \notag\\
				\bracket{x-x_0,y-y_0,z-z_0}\cdot\bracket{a,b,c}&=0\notag\\
...
\end{align}
 \autoeqref{eq:planes3} is especially useful as many computer programs can graph functions in this form. Equations \eqref{eq:planes1} and \eqref{eq:planes2} have specific names, given next.

\definition{def:planes}{Equations of a Plane in Standard and General Forms}
{The plane passing through the point $P=(x_0,y_0,z_0)$ with normal vector $\vec n=\bracket{a,b,c}$ can be described by an equation with \textbf{standard form}
\[a(x-x_0)+b(y-y_0)+c(z-z_0) =0;\]
the equation's \textbf{general form} is 
\index{planes!equations of}
\[ax+by+cz = d;\]
the equation's \textbf{vector form} is
\[\bracket{x,y,z}\cdot\vecn=\bracket{x_0,y_0,z_0}\cdot\vecn=d.\]}
...
\youtubeVideo{0MECmEjR2WU}{Equation of a Plane Through Three Points}

\example{ex_planes1}{Finding the equation of a plane.}{Write the equation of the plane that passes through the points $P=(1,1,0)$, $Q = (1,2,-1)$ and $R = (0,1,2)$ in standard form.}
{We need a vector $\vec n$ that is orthogonal to the plane.
\mfigurethree{width=125pt,3Dmenu,activate=onclick,deactivate=onclick,
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.25,trim=2mm 0mm 2mm 2mm,clip=true}{0in}{Sketching the plane in \autoref{ex_planes1}.}{fig:planes1}{figures/figplanes1}
%\mfigure[scale=1.25,trim=2mm 0mm 2mm 2mm,clip=true]{0in}{Sketching the plane in \autoref{ex_planes1}.}{fig:planes1}{figures/figplanes1}
Since $P$, $Q$ and $R$ are in the plane, so are the vectors $\vv{PQ}$ and $\vv{PR}$; $\vv{PQ}\times\vv{PR}$ is orthogonal to $\vv{PQ}$ and $\vv{PR}$ and hence the plane itself.
...
\example{ex_planes2}{Finding the equation of a plane.}{Verify that lines $\ell_1$ and $\ell_2$, whose parametric equations are given below, intersect, then give the equation of the  plane that contains these two lines in general form.
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{}{0in}{Sketching the plane in \autoref{ex_planes2}.}{fig:planes2}{figures/figplanes2}
%\mfigure{0in}{Sketching the plane in \autoref{ex_planes2}.}{fig:planes2}{figures/figplanes2}
\begin{align*}
5(x+1) +4(y-3) -7z &= 0 \\
5x + 5 + 4y-12 -7z &= 0\\
5x+4y-7z &= 7.
...
\example{ex_planes3}{Finding the equation of a plane}{Give the equation, in standard form, of the plane that passes through the point $P=(-1,0,1)$ and is orthogonal to the line with vector equation $\vec \ell(t) =\bracket{-1,0,1}+ t\bracket{1,2,2}$.}
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.25,trim=5mm 0mm 5mm 5mm,clip=true}{0in}{The line and plane in \autoref{ex_planes3}.}{fig:planes3}{figures/figplanes3}
%\mfigure[scale=1.25,trim=5mm 0mm 5mm 5mm,clip=true]{0in}{The line and plane in \autoref{ex_planes3}.}{fig:planes3}{figures/figplanes3}
\[(x+1) +2y+2(z-1)=0.\]
The line and plane are sketched in \autoref{fig:planes3}.}

\example{ex_planes4}{Finding the intersection of two planes}{Give the parametric equations of the line that is the intersection of the planes $p_1$ and $p_2$, where:
\begin{gather*}
p_1: x-(y-2)+(z-1) =0 \\
p_2: -2(x-2)+(y+1)+(z-3)=0
\end{gather*}}
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.25,trim=5mm 0mm 5mm 5mm,clip=true}{0in}{Graphing the planes and their line of intersection in \autoref{ex_planes4}.}{fig:planes4}{figures/figplanes4}
%\mfigure[scale=1.25,trim=5mm 0mm 5mm 5mm,clip=true]{0in}{Graphing the planes and their line of intersection in \autoref{ex_planes4}.}{fig:planes4}{figures/figplanes4}
...
\example{ex_planes5}{Finding the intersection of a plane and a line}{Find the point of intersection, if any, of the line $\ell(t) =\bracket{3,-3,-1}+t\bracket{-1,2,1}$ and the plane with equation in general form $2x+y+z=4$.}
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.25,trim=2mm 0mm 2mm 0mm,clip}{0in}{Illustrating the intersection of a line and a plane in \autoref{ex_planes5}.}{fig:planes5}{figures/figplanes5}
%\mfigure[scale=1.25,trim=2mm 0mm 2mm 0mm,clip]{0in}{Illustrating the intersection of a line and a plane in \autoref{ex_planes5}.}{fig:planes5}{figures/figplanes5}
...
\end{equation}
\autoeqref{eq:plane_dist} is important as it does more than just give the distance between a point and a plane. We will see how it allows us to find several other distances as well: the distance between parallel planes and the distance from a line and a plane. Because \autoeqref{eq:plane_dist} is important, we restate it as a Key Idea.
\mfigurethree{width=125pt,3Dmenu,activate=onclick,deactivate=onclick,
...
3Droo=149.9999995928182,
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.25}{0in}{Illustrating finding the distance from a point to a plane.}{fig:planes_dist}{figures/figplanes_dist}
%\mfigure[scale=1.25]{0in}{Illustrating finding the distance from a point to a plane.}{fig:planes_dist}{figures/figplanes_dist}
...
\example{ex_planes6}{Distance between a point and a plane}{Find the distance between the point $Q = (2,1,4)$ and the plane with equation $2x-5y+6z=9$.}
