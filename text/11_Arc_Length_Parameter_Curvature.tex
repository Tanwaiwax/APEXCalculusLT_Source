...
\filldraw [draw={\colortwo}](axis cs: 0,0) circle (1.5pt)
 node [black,above right] {\scriptsize $t=0$};
\filldraw [draw={\colortwo}](axis cs: 0,2) circle (1.5pt)
 node [black, right] {\scriptsize $t=1$};
\filldraw [draw={\colortwo}](axis cs: 2,6) circle (1.5pt)
...
\filldraw [draw={\colortwo}](axis cs: -.24,.94) circle (1.5pt)
 node [black, right] {\scriptsize $s=1$};
\filldraw [draw={\colortwo}](axis cs: -.03,1.91) circle (1.5pt)
 node [black, right] {\scriptsize $s=2$};
\filldraw [draw={\colortwo}](axis cs: .33,2.84) circle (1.5pt)
 node [black, right] {\scriptsize $s=3$};
\filldraw [draw={\colortwo}](axis cs: .75,3.75) circle (1.5pt)
 node [black, right] {\scriptsize $s=4$};
\filldraw [draw={\colortwo}](axis cs: 1.21,4.63) circle (1.5pt)
 node [black, right] {\scriptsize $s=5$};
\filldraw [draw={\colortwo}](axis cs: 1.71, 5.51) circle (1.5pt)
...
\youtubeVideo{G0R-qialKlE}{Parametrize a Curve with Respect to Arc Length}

\example{ex_vvfarc1}{Finding the arc length parameter}{Let $\vec r(t) =\bracket{3t-1,4t+2}$. Parametrize $\vec r$ with the arc length parameter $s$.}
...
\filldraw (axis cs:-1,2) circle (1.5pt) node [left] {\scriptsize $t=0$}
 ...
 (axis cs: -.4,2.8) circle (1.5pt) node [right,draw={\colortwo}] {\scriptsize $s=1$}
 (axis cs: .2,3.6) circle (1.5pt) node [right,draw={\colortwo}] {\scriptsize $s=2$}
 (axis cs: .8,4.4) circle (1.5pt) node [right,draw={\colortwo}] {\scriptsize $s=3$}
 (axis cs: 1.4,5.2) circle (1.5pt) node [right,draw={\colortwo}] {\scriptsize $s=4$}
 (axis cs: 2,6) circle (1.5pt) node [right,draw={\colortwo}] {\scriptsize $s=5$};
\draw (axis cs:-1,2) node [draw={\colortwo},shift={(10pt,-5pt)}]  {\scriptsize $s=0$};
\end{axis}
\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}}
...
{In \autoref{ex_vvfarc1}, we found that the arc length parameter was defined by $s=5t$, so $\vec r(s) =\bracket{3s/5-1, 4s/5+2}$ parametrized $\vec r$ with the arc length parameter. To find $\kappa$, we need to find $\vec T\,'(s)$. 
\begin{align*}
...
\example{ex_curvature2}{Finding the curvature of a circle}{Find the curvature of a circle with radius $r$, defined by $\vec c(t) =\bracket{r\cos t,r\sin t}$.
...
\begin{align*}
\kappa &= \frac{\abs{(-r\sin t)(-r\sin t) - (-r\cos t)(r\cos t)}}{\big( (-r\sin t)^2+(r\cos t)^2\big)^{3/2}} \\
...
\example{ex_curvature3}{Finding curvature}{Find the curvature of the parabola defined by $y=x^2$ at the vertex and at $x=1$.}
...
\example{ex_curvature4}{Finding curvature}{Find where the curvature of $\vec r(t) =\bracket{t, t^2, 2t^3}$ is maximized.}
...
\example{ex_curvature5}{Curvature and road design}{The minimum radius of the curve in a highway cloverleaf is determined by the operating speed, as given in the table in \autoref{fig:curvature5}. For each curve and speed, compute $a_\text{N}$.}
...
45 & 540
\end{tabular}}
Using \autoeqref{eq:curvature_an}, we can compute the acceleration normal to the curve in each case. We start by converting each speed from ``miles per hour'' to ``feet per second'' by multiplying by $5280/3600$.
