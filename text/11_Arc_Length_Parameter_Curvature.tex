...
\mtable{Graphing $\vec r$ in \autoref{ex_vvfarc1} with parameters $t$ and $s$.}{fig:vvfarc1}{\pdftooltip{\begin{tikzpicture}[>=stealth]
...
%
\mtable{Illustrating the osculating circles for the curve seen in \autoref{fig:curvatureintro}.}{fig:curvature_osculate}{\pdftooltip{\begin{tikzpicture}[>=stealth]
\begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,
ymin=-.1,ymax=2.9,xmin=-.1,xmax=3.5]
\addplot [thick,draw={\colorone}, smooth,domain=-1:1.5] ({x^2-x+1},{x^3+1});
...
\mtable[\baselineskip]{Examining the curvature of $y=x^2$.}{fig:curvature3}{\pdftooltip{\begin{tikzpicture}[>=stealth]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,
