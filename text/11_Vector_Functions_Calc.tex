...
\example{ex_vvflimit1}{Finding limits of vector--valued functions}{Let $\ds\vec r(t) =\bracket{\frac{\sin t}{t},\, t^2-3t+3,\,\cos t}.$ Find $\ds \lim_{t\to 0}\vec r(t)$.}
...
\example{ex_vvflimit2}{Evaluating continuity of vector--valued functions}{Let $\ds\vec r(t) =\bracket{\frac{\sin t}{t},\, t^2-3t+3,\,\cos t}.$ Determine whether $\vec r$ is continuous at $t=0$ and $t=1$.}
...
\example{ex_vvflimit3}{Derivatives of vector--valued functions}{Let $\vec r(t) =\bracket{t^2,t}$. 
\mtable[-\baselineskip]{Graphing the derivative of a vector--valued function in \autoref{ex_vvflimit3}.}{fig:vvflimit3}{%
...
\mtable[-2\baselineskip]{Viewing a vector--valued function, and its derivative at one point, from two different perspectives.}{fig:vvflimit4}{%
...
\example{ex_vvflimit4}{Derivatives of vector--valued functions}{Let $\vec r(t) =\bracket{\cos t, \sin t, t}$. Compute $\vrp(t)$ and $\vrp(\pi/2)$. Sketch $\vrp(\pi/2)$ with its initial point at the origin and at $\vec r(\pi/2)$.}
{We compute $\vrp$ as $\vrp(t) =\bracket{-\sin t, \cos t, 1}$. At $t= \pi/2$, we have $\vrp(\pi/2) =\bracket{-1,0,1}$. \autoref{fig:vvflimit4} \ifthenelse{\boolean{in_threeD}}{shows a graph of $\vec r(t)$,}{shows two graphs of $\vec r(t)$, from different perspectives,} with $\vrp(\pi/2)$ plotted with its initial point at the origin and at $\vec r(\pi/2)$.}
...
\example{ex_vvfderiv1}{Finding tangent lines to curves in space}{Let $\vec r(t) =\bracket{t,t^2,t^3}$ on $[-1.5,1.5]$. Find the vector equation of the line tangent to the graph of $\vec r$ at $t=-1$. }
...
\example{ex_vvfderiv3}{Finding tangent lines to curves}{Find the equations of the lines tangent to $\vec r(t) =\bracket{t^3,t^2}$ at $t=-1$ and $t=0$.}
...
$$\ell(t) =\bracket{-1,1}+ t\bracket{3,-2}.$$ This line is graphed with $\vec r(t)$ in \autoref{fig:vvfderiv3}.
...
\end{enumerate}}
\mnote[-1in]{\textbf{Note:} Because the order is important when computing a cross product, we must maintain the correct order of the functions in rule \ref{crossprodrule}.}

\example{ex_vvfderiv2}{Using derivative properties of vector--valued functions}{Let $\vec r(t) =\bracket{t, t^2-1}$ and let $\vec u(t)$ be the unit vector that points in the direction of $\vec r(t)$.
\begin{enumerate}
...
\end{enumerate}\index{vector--valued function!integration}\index{integration!of vector--valued functions}}

\example{ex_vvfint1}{Evaluating a definite integral of a vector--valued function}{Let $\vec r(t) =\bracket{e^{2t},\sin t}$. Evaluate $\ds \int_0^1 \vec r(t) \ dt$.}
...
\example{ex_vvfint2}{Solving an initial value problem}{Let $\vrp'(t) =\bracket{2, \cos t, 12t}$. Find $\vec r(t)$ where:
\begin{itemize}
