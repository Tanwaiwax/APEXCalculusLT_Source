...
\mtable[-.5in]{Viewing a vector--valued function and its derivative at one point, from two different perspectives.}{fig:vvflimit4}{%
\myincludegraphics[width=.7\marginparwidth]{figures/figvvflimit4_3D}
\\(a)\\
% todo Tim make the graphs pull from the same source? Then they'd have the same size
\myincludegraphics[width=\marginparwidth]{figures/figvvflimit4a}
\\(b)}%
...
3Droo=149.99999927774635}{width=.8\marginparwidth}{figures/figvvfderiv1_3D}}
}{% not in three D
\mtable[-1in]{Graphing a curve in space with its tangent line.}{fig:vvfderiv1}{%
\myincludegraphics[width=.8\marginparwidth]{figures/figvvfderiv1_3D}
\\(a)\\[10pt]
% todo Tim make these pull from the same source, so they're the same size
\myincludegraphics[width=.8\marginparwidth]{figures/figvvfderiv1b}
\\(b)}% end mtable
...
\mtable{Graphing $\vec r(t)$ and its tangent line in \autoref{ex_vvfderiv3}.}{fig:vvfderiv3}{\begin{tikzpicture}[>=stealth]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,
...
\definition{thm:vvf_integration}{\parbox[t]{225pt}{Indefinite and Definite Integrals of Vector--Valued Functions}}
