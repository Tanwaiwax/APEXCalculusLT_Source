...
\example{ex_motion1}{Finding velocity and acceleration}{An object is moving with position function $\vec r(t) = \la t^2-t,t^2+t\ra$, $-3\leq t\leq 3$, where distances are measured in feet and time is measured in seconds.
\begin{enumerate}
...
\mfigure{0in}{Plotting velocity and acceleration vectors for Object 1 in \autoref{ex_motion2}.}{fig:motion2a}{figures/figmotion2a}
...
{\begin{enumerate}
	...
	\mfigure{0in}{Modeling the flight of a ball in \autoref{ex_motion3}.}{fig:motion3}{figures/figmotion3}
	...
	\end{align*}
	This is a wonderful formula. Every 1/2 second after $t=0.016$s the boy can release the string (since the ball makes 2 revolutions per second, he has two chances each second to release the ball).\eoehere
\end{enumerate}}

\example{ex_motion4}{Analyzing motion in space}{An object moves in a spiral with position function $\vrt = \la \cos t, \sin t, t\ra$, where distances are measured in meters and time is in minutes. Describe the object's speed and acceleration at time $t$.}
...
{A direct application of \autoref{idea:projectile} gives
\begin{align*}
...
%Given a position function $\vrt = \la f(t),g(t)\ra$, \autoref{thm:distance_traveled}
...
\example{ex_motion6}{Distance Traveled, Displacement, and Average Speed}{A particle moves in space with position function $\vrt = \la t,t^2,\sin (\pi t)\ra$ on $[-2,2]$, where $t$ is measured in seconds and distances are in meters. Find:
\begin{enumerate}
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{}{0in}{The path of the particle in \autoref{ex_motion6}.}{fig:motion6}{figures/figmotion6}
	}%ends in 3D
	{% now in 2D	
