...
% todo Tim check the phrasing of the proof of Directional Derivatives
\begin{proof}
...
\example{ex_direct1}{Computing directional derivatives}{Let $z= 14-x^2-y^2$ and let $P=(1,2)$. Find the directional derivative of $f$, at $P$, in the following directions:
\begin{enumerate}
...
3Droo=130.00000000067573}{width=\marginparwidth}{figures/figdirect1_3D}}
...
% todo Tim verify the phrasing of nabla as an operator
...
\example{ex_direct2}{Finding directions of maximal and minimal increase}{Let $f(x,y) = \sin x\cos y$ and let $P=(\pi/3,\pi/3)$. Find the directions of maximal/minimal increase, and find a direction where the instantaneous rate of $z$ change is 0.}
{We begin by finding the gradient. We see that $f_x = \cos x\cos y$ and $f_y = -\sin x\sin y$, thus 
...
\mtable[-6\baselineskip]{Graphing the surface and important directions in \autoref{ex_direct2}.}{fig:direct2}{%
...
3Droo=150}{width=.8\marginparwidth}{figures/figdirect2_3D}
\\(a)\\
\myincludeasythree{width=.8\marginparwidth,
3Droll=0,
...
3Droo=149.9999987284343}{width=.8\marginparwidth}{figures/figdirect2b_3D}
\\(b)}
...
\example{ex_direct9}{Understanding when $\nabla f = \vec 0$}{Let $f(x,y) = -x^2+2x-y^2+2y+1$. Find the directional derivative of $f$ in any direction at $P=(1,1)$.}
...
3Droo=200.00000572048617}{width=\marginparwidth}{figures/figdirect9_3D}}
...
\example{ex_direct3}{The flow of water downhill}{Consider the surface given by $f(x,y)= 20-x^2-2y^2$. Water is poured on the surface at $(1,1/4)$. What path does it take as it flows downhill?}
...
3Droo=150.00001038122664}{width=\marginparwidth}{figures/figdirect3_3D}
\\(a)\\
\begin{tikzpicture}
...
{Let $w=F(x,y,z)$ be differentiable on an open ball $B$ and let $\vec u $ be a unit vector in $\mathbb{R}^3$.\index{gradient}\index{directional derivative}\index{derivative!directional}\index{$\nabla$}
\begin{enumerate}
...
\example{ex_direct5}{Finding directional derivatives with functions of three variables}{If a point source $S$ is radiating energy, the intensity $I$ at a given point $P$ in space is inversely proportional to the square of the distance between $S$ and $P$. That is, when $S=(0,0,0)$,  $\ds I(x,y,z) = \frac{k}{x^2+y^2+z^2}$ for some constant $k$.
...
\nabla I(2,5,3) &=\bracket{\frac{-4}{1444},\frac{-10}{1444},\frac{-6}{1444}}%\approx\bracket{-0.003,-0.007,-0.004}
\\
