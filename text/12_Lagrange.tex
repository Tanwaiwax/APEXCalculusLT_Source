...
\theorem{thm:lagrange}{Lagrange Multipliers}{Let $f(x,y)$ and $g(x,y)$ be functions with continuous partial derivatives of all orders, and suppose that $c$ is a scalar constant such that $\nabla g(x,y) \ne \vec0$ for all $(x,y)$ that satisfy the equation $g(x,y) = c$. Then to solve the constrained optimization problem
 \[\text{Maximize (or minimize) }f(x,y)\text{ subject to }g(x,y) = c ,\]
...
% todo include a proof sketch of Theorem 120 Lagrange Multipliers?
...
% todo give a different example than example 3 so that the objective function doesn't immediately simplify because of the constraint
\example{ex_lagr_circle}{Extreme Values on a Circle}{Find the points on the circle $x^2 + y^2 = 80$ which are closest to and farthest from the point $(1,2)$.}{The distance $d$ from any point $(x,y)$ to the point $(1,2)$ is
 ...
 \text{ subject to }g(x,y) = x^2 + y^2 = 80.
 \]
...
\example{ex_lagr_3var}{Maximizing a Function of Three Variables}{Maximize (and minimize) $f(x,y,z) = x+z$ subject to $g(x,y,z) = x^2 + y^2 + z^2 = 1$.}{Solve the equation $\nabla f(x,y,z) = \lambda \nabla g(x,y,z)$:
 \begin{align*}
  1 &= 2\lambda x\\
  0 &= 2\lambda y\\
  1 &= 2\lambda z
...
% todo Tim check the phrasing of two constraints
...
\example{ex_lang_two_const}{Optimizing with Two Constraints}{The plane $x-y+z=2$ intersects the cylinder $x^2+y^2=4$ in an ellipse. Find the points on the ellipse closest to and farthest from the origin.}{We can optimize the distance $\sqrt{x^2+y^2+z^2}$ by optimizing the function $f(x,y,z)=x^2+y^2+z^2$, which has a simpler derivative.  Let $g(x,y,z)=x-y+z$ be the plane constraint, and $h(x,y,z)=x^2+y^2$ be the cylinder constraint.  We see that
\begin{align*}
