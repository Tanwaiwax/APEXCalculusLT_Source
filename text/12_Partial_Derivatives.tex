...
\mtable{By fixing $y=2$, the surface $f(x,y) = x^2+2y^2$ is a curve in space.}{fig:partialintro}{%
\myincludeasythree{
...
3Dc2c=0.6570873856544495 0.6839641332626343 0.31690558791160583,
3Dcoo=-3.0739071369171143 -0.40104401111602783 58.57058334350586,
3Droo=129.99999696026478}{.8\marginparwidth}{Same parabolic curve z=x^2 + 8 shown alone in the xz plane.}{figures/figpartialintrob_3D}
\\(b)}
...
\begin{enumerate}
	...
	%\item		If $f_x$ and $f_y$ are continuous on $S$, then $f$ is \textbf{differentiable} on $S$. (This definition of differentiability is mathematically ``weak'' but easy to understand; see \autoref{sec:total_differential} for a stronger definition that is less intuitive.)
	\end{enumerate}
	\index{partial derivative}\index{derivative!partial}
\end{definition}

\mnote{\textbf{Note:} If $f$ is a function of time, then an alternative notation for the derivative of $f$ with respect to time is $\dot f$.}

%\mnote{Alternate notations for $f_x(x,y)$ include:
...
\youtubeVideo{SbfRDBmyAMI}{Partial Derivatives}

\begin{example}[Computing partial derivatives with the limit definition]\label{ex_partial1}%
...
\begin{example}[Finding partial derivatives]\label{ex_partial2}%
...
We have shown \emph{how} to compute a partial derivative, but it may still not be clear what a partial derivative \emph{means}. Given $z=f(x,y)$, $f_x(x,y)$ measures the rate at which $z$ changes as only $x$ varies: $y$ is held constant. \index{partial derivative!meaning}
...
\begin{example}[Evaluating partial derivatives]\label{ex_partial3}%
Let $z=f(x,y)=-x^2-\frac12y^2+xy+10$. Find $f_x(2,1)$ and $f_y(2,1)$ and interpret their meaning.
\solution
%
\mtable{Illustrating the meaning of partial derivatives.}{fig:partial3}{%
\myincludeasythree{
3Droll=0.,
3Dortho=0.004999999888241291,
...
3Droo=129.9999938624484}{\marginparwidth}{Along the red line y= –2, the curve z=f(x, –2) (blue) is tangent to the red vector whose slope is f_x at the marked point.}{figures/figpartial3a_3D}
\\(a)\\[10pt]
\myincludeasythree{
3Droll=0.,
3Dortho=0.004999999888241291,
3Dc2c=0.6528097987174988 0.6841705441474915 0.3251922130584717,
3Dcoo=7.158430576324463 9.488722801208496 72.96935272216797,
3Droo=129.9999938624484}{\marginparwidth}{Along the red line x= –2, the curve z=f(–2, y) (blue) is tangent to the red vector whose slope is f_y at the same point.}{figures/figpartial3b_3D}
\\(b)}
%
...
\begin{definition}[Second Partial Derivative, Mixed Partial Derivative]\label{def:second_partial}%
Let $z=f(x,y)$ be continuous on an open set $S$.
\begin{enumerate}
	\item The \textbf{second partial derivative of $f$ with respect to $x$ then $x$} is
	\[\frac{\partial}{\partial x}\left(\frac{\partial f}{\partial x}\right) = \frac{\partial^2 f}{\partial x^2} = \bigl(\,f_x\,\bigr)_x = f_{xx}\]

\item The \textbf{second partial derivative of $f$ with respect to $x$ then $y$} is 
\[\frac{\partial}{\partial y}\left(\frac{\partial f}{\partial x}\right) = \frac{\partial^2f}{\partial y\partial x} = \bigl(\,f_x\,\bigr)_y = f_{xy}\]

%\item The \textbf{second partial derivative of $f$ with respect to $y$ then $y$} is \[\frac{\partial}{\partial y}\left(\frac{\partial f}{\py}\right) = \frac{\partial^2 f}{\py^2} = \bigl(\,f_y\,\bigr)_y= f_{yy}\]
%
%\item The \textbf{second partial derivative of $f$ with respect to $y$ then $x$} is
%\[\frac{\partial}{\partial x}\left(\frac{\partial f}{\py}\right) = \frac{\partial^2f}{\partial x\partial y} = \bigl(\,f_y\,\bigr)_x = f_{yx}\]

\end{enumerate}

Similar definitions hold for $\ds \frac{\partial^2f}{\partial y^2} = f_{yy}$ and $\ds \frac{\partial^2f}{\partial x\partial y} = f_{yx}$. \bigskip
...
\mnote{\textbf{Note:} The terms in \autoref{def:second_partial} all depend on limits, so each definition comes with the caveat ``where the limit exists.''\bigskip\\
The way to keep track of the order is to start with the variable nearest to the function.  Unfortunately, this means that while $\frac{\partial^2f}{\partial y\partial x}=f_{xy}$ both mean ``differentiate with respect to $x$ first'', the $x$ and $y$ appear in a different order.  Fortunately \autoref{thm:mixed_partial} will soon tell us that the order doesn't usually matter.}

The notation of second partial derivatives gives some insight into the notation of the second derivative of a function of a single variable. If $y=f(x)$, then $\ds \fp'(x) = \frac{\dd^2 y}{\dd x^2}$. The ``$\dd^2y$'' portion means ``take the derivative of $y$ twice,'' while ``$\dd x^2$'' means ``with respect to $x$ both times.'' When we only know of functions of a single variable, this latter phrase seems silly: there is only one variable to take the derivative with respect to. Now that we understand functions of multiple variables, we see the importance of specifying which variables we are referring to.

\begin{example}[Second partial derivatives]\label{ex_partial6}%
...
\begin{enumerate}
	...
	f_y(x,y) &= 2x^3y+6xy^2 \\
	f_{xx}(x,y) &= \frac{\partial}{\partial x}\bigl(f_x\bigr) = \frac{\partial}{\partial x}\bigl(3x^2y^2+2y^3-\sin x\bigr) = 6xy^2-\cos x \\
	f_{yy}(x,y) &= \frac{\partial}{\partial y}\bigl(f_y\bigr) = \frac{\partial}{\partial y}\bigl(2x^3y+6xy^2\bigr) = 2x^3+12xy \\
	f_{xy}(x,y) &= \frac{\partial}{\partial y}\bigl(f_x\bigr) = \frac{\partial}{\partial y}\bigl(3x^2y^2+2y^3-\sin x\bigr) = 6x^2y+6y^2 \\
	f_{yx}(x,y) &= \frac{\partial}{\partial x}\bigl(f_y\bigr) = \frac{\partial}{\partial x}\bigl(2x^3y+6xy^2\bigr) = 6x^2y+6y^2
	\end{aligned}$\hfill\null
%
	\item\hfill$\begin{aligned}[t]
	...
	f_{xx}(x,y) &= \frac{\partial}{\partial x}\bigl(f_x\bigr) = \frac{\partial}{\partial x}\bigl(\frac{3x^2}{y^2}\bigr) = \frac{6x}{y^2} \\
	f_{yy}(x,y) &= \frac{\partial}{\partial y}\bigl(f_y\bigr) = \frac{\partial}{\partial y}\bigl(-\frac{2x^3}{y^3}\bigr) = \frac{6x^3}{y^4} \\
	f_{xy}(x,y) &= \frac{\partial}{\partial y}\bigl(f_x\bigr) = \frac{\partial}{\partial y}\bigl(\frac{3x^2}{y^2}\bigr) = -\frac{6x^2}{y^3} \\
	f_{yx}(x,y) &= \frac{\partial}{\partial x}\bigl(f_y\bigr) = \frac{\partial}{\partial x}\bigl(-\frac{2x^3}{y^3}\bigr) = -\frac{6x^2}{y^3}.
	\end{aligned}$\hfill\null
%
%\item	$f(x,y) = e^x\sin(x^2y)$
...
Notice how in both of the functions in \autoref{ex_partial6}, $f_{xy} = f_{yx}$. Due to the complexity of the examples, this likely is not a coincidence. The following theorem states that it is not.

\begin{theorem}[Clairaut's Theorem: Mixed Partial Derivatives]\label{thm:mixed_partial}%
...
\begin{example}[Understanding second partial derivatives]\label{ex_partial7}%
...
\mtable{Understanding the second partial derivatives in \autoref{ex_partial7}.}{fig:partial7}{%
\myincludeasythree{
...
3Droo=129.9999903526944}{\marginparwidth}{Black curve in the plane y = 1 is z=x^2–1. Red concave up tangent shows fₓₓ > 0. Dashed blue lines in the plane x = 0 show no y‑change.}{figures/figpartial6_3D}
\\(a)\\y = 1 slice z = x²−1; 
\myincludeasythree{
3Droll=0.4263461176034552,
3Dortho=0.004999999888241291,
3Dc2c=0.358655720949173 0.8657855987548828 0.34897172451019287,
3Dcoo=14.456816673278809 24.785551071166992 12.880075454711914,
3Droo=129.9999903526944}{\marginparwidth}{Swapping: Black curve in x = 1 is z = 1 – y^2. Its concave‑down red tangent parabola shows  fₓₓ < 0. Dashed blue lines in y = 0 show no x‑change.}{figures/figpartial6b_3D}
\\(b)}
...
\begin{definition}[Partial Derivatives with Three Variables]\label{def:partial_multiple}%
...
\begin{example}[Partial derivatives of functions of three variables]\label{ex_partial8}%
...
\begin{example}[Higher order partial derivatives]\label{ex_partial9}%
...
% todo write an example where you find a function given the partial derivatives

We have seen that partial derivatives measure the instantaneous rate of change of a multivariable function with respect to one variable. %When $z=f(x,y)$, $f_x$ measures the rate at which $z$ changes as we move parallel to the $x$-axis. 
With $z=f(x,y)$, the partial derivatives $f_x$ and $f_y$ measure the instantaneous rate of change of $z$ when moving parallel to the $x$- and $y$-axes, respectively. How do we measure the rate of change at a point when we do not move parallel to one of these axes? What if we move in the direction given by the vector $\bracket{2,1}$? Can we measure that rate of change? The answer is, of course, yes, we can. This is the topic of \autoref{sec:directional_derivative}. First, we need to define what it means for a function of two variables to be \emph{differentiable.}

\printexercises{exercises/12-03-exercises}
