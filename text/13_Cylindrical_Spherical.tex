...
3Dlights=Headlamp}{width=\marginparwidth}{figures/figcylindrical1_3D}}
...
\[
M = \iiint_D\delta \dd V = \int_0^{\pi}\int_0^{\cos(3\theta)}\int_0^{1-r\cos\theta+0.1r\sin\theta} r\dd z\dd r\dd\theta \approx 0.785.
\]
...
is located at $(-0.147,0.015,0.467)$, which lies outside the bounds of the solid.
\end{example}
...
\mnote[-\baselineskip]{\textbf{Note:} The symbol $\rho$ is the Greek letter ``rho.'' Traditionally it is used in the spherical coordinate system, while $r$ is used in the polar and cylindrical coordinate systems.% In space, $r$ and $\rho$ play different roles. In spherical coordinates, $\rho$ is the dist
}
...
\mtable[-\baselineskip]{Approximating the volume of a standard region in space using spherical coordinates.}{fig:sphericalwedge}{%
\myincludeasythree{width=\marginparwidth,
3Droll=0,
...
V &= \iiint_D\dd V\\
	...
	&= \int_0^\pi\int_0^{2\pi}\left(\frac13\rho^3\sin(\varphi)\Big|_0^r\right)\dd\theta\dd\varphi\\
