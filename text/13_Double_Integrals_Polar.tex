...
\example{ex_doublepol1}{Evaluating a double integral with polar coordinates}{Find the signed volume under the plane $z= 4-x-2y$ over the circle with equation $x^2+y^2=1$.}
{The bounds of the integral are determined solely by the region $R$ over which we are integrating. In this case, it is a circle with equation $x^2+y^2=1$. We need to find polar bounds for this region. It may help to review \autoref{sec:polar}; bounds for this circle are $0\leq r\leq 1$ and $0\leq \theta\leq 2\pi$.
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.25,trim=4mm 0mm 2mm 0mm, clip}{0in}{Evaluating a double integral with polar coordinates in \autoref{ex_doublepol1}.}{fig:doublepol1}{figures/figdoublepol1}
%\mfigure[scale=1.25,trim=4mm 0mm 2mm 0mm, clip]{0in}{Evaluating a double integral with polar coordinates in \autoref{ex_doublepol1}.}{fig:doublepol1}{figures/figdoublepol1}
\begin{align*}
\iint_Rf(x,y)\ dA
 ...
 &= 4\pi \approx 12.566.
\end{align*}
The surface and region $R$ are shown in \autoref{fig:doublepol1}.}

\example{ex_doublepol2}{Evaluating a double integral with polar coordinates}{Find the volume under the paraboloid $z=4-(x-2)^2-y^2$ over the region bounded by the circles $(x-1)^2+y^2=1$ and $(x-2)^2+y^2=4$.}
...
\intertext{To integrate $\cos^4\theta$, rewrite it as $\cos^2\theta\cos^2\theta$ and employ the half--angle formula twice:}
...
\example{ex_doublepol5}{Evaluating a double integral with polar coordinates}{Find the volume under the surface $\ds f(x,y) =\frac1{x^2+y^2+1}$ over the  sector of the circle with radius $a$ centered at the origin in the first quadrant, as shown in \autoref{fig:doublepol5}.}
{\mfigurethree{width=150pt,3Dmenu,activate=onclick,deactivate=onclick,
3Droll=0,
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.2,trim=1mm 0mm 0mm 0mm,clip}{0in}{The surface and region $R$ used in \autoref{ex_doublepol5}.}{fig:doublepol5}{figures/figdoublepol5}
%\mfigure[scale=1.2,trim=1mm 0mm 0mm 0mm,clip]{0in}{The surface and region $R$ used in \autoref{ex_doublepol5}.}{fig:doublepol5}{figures/figdoublepol5}
...
\mnote{\textbf{Note:} Previous work has shown that there is finite \textit{area} under $\frac{1}{x^2+1}$ over the entire $x$-axis. However, \autoref{ex_doublepol5} shows that there is infinite \textit{volume} under $\frac{1}{x^2+y^2+1}$ over the entire $x$-$y$ plane.}}

\example{ex_doublepol3}{Finding the volume of a sphere}{Find the volume of a sphere with radius $a$.}
...
%\mtable{Finding the volume of the solid shown here from two perspectives.}{fig:doublepol4}{%
%\begin{tabular}{c}
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.25,trim=2mm 10mm 2mm 0mm,clip}{0in}{Visualizing the solid used in \autoref{ex_doublepol4}.}{fig:doublepol4}{figures/figdoublepol4} 
\example{ex_doublepol4}{Finding the volume of a solid}{A sculptor wants to make a solid bronze cast of the solid shown in \autoref{fig:doublepol4}, where the base of the solid has boundary, in polar coordinates, $r=\cos(3\theta)$, and the top is defined by the plane $z=1-x+0.1y$. Find the volume of the solid.}
