...
\myincludegraphics{figures/figdouble_intro1}\\
(a)\\
...
The volume of the rectangular solid whose base is the $i^\text{ th}$ subregion and whose height is $f(x_i,y_i)$ is $V_i=f(x_i,y_i)\dx\dy$. Such a  solid is shown in \autoref{fig:double_intro}(b). Note how this rectangular solid only approximates the true volume under the surface; part of the solid is above the surface and part is below.
...
\mnote{\textbf{Note:} Recall that the integration symbol ``$\int$'' is an ``elongated S,'' representing the word ``sum.'' We interpreted $\int_a^bf(x)\ dx$ as ``take the \textit{sum} of the areas of rectangles over the interval $[a,b]$.'' The double integral uses two integration symbols to represent a ``double sum.'' When adding up the volumes of rectangular solids over a partition of a region $R$, as done in \autoref{fig:double_intro}, one could first add up the volumes across each row (one type of sum), then add these totals together (another sum), as in
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.25,trim=2mm 0mm 0mm 2mm,clip}{0in}{Finding volume under a surface by sweeping out a cross--sectional area.}{fig:double_intro3}{figures/figdouble_intro3}
%\mfigure[scale=1.25,trim=2mm 0mm 0mm 2mm,clip]{0in}{Finding volume under a surface by sweeping out a cross--sectional area.}{fig:double_intro3}{figures/figdouble_intro3}
...
\example{ex_double1}{Evaluating a double integral}{Let $f(x,y) = xy+e^y$. Find the signed volume under $f$ on the region $R$, which is the rectangle with corners $(3,1)$ and $(4,2)$ pictured in \autoref{fig:double1}, using Fubini's Theorem and both orders of integration.}
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.25,trim= 1mm 0mm 2mm 0mm,clip}{0in}{Finding the signed volume under a surface in \autoref{ex_double1}.}{fig:double1}{figures/figdouble1}
%\mfigure[scale=1.25,trim= 1mm 0mm 2mm 0mm,clip]{0in}{Finding the signed volume under a surface in \autoref{ex_double1}.}{fig:double1}{figures/figdouble1}
...
%\example{ex_double2}{Evaluating a double integral}{Evaluate $\iint_R e^{2x+3y}\ dA$, where $R$ is the triangle bounded by $x=0$, $y=0$ and $\frac12x+y=1$, as shown in FIGURE.}
...
\example{ex_double2}{Evaluating a double integral}{Evaluate $\iint_R \big(3xy-x^2-y^2+6\big)\ dA$, where $R$ is the triangle bounded by $x=0$, $y=0$ and $x/2+y=1$, as shown in \autoref{fig:double2}.
}
{While it is not specified which order we are to use, we will evaluate the double integral using both orders to help drive home the point that it does not matter which order we use.\\
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.25,trim=1mm 0mm 2mm 0mm,clip}{0in}{Finding the signed volume under the surface in \autoref{ex_double2}.}{fig:double2}{figures/figdouble2}
%\mfigure[scale=1.25,trim=1mm 0mm 2mm 0mm,clip]{0in}{Finding the signed volume under the surface in \autoref{ex_double2}.}{fig:double2}{figures/figdouble2}
Using the order $dy\ dx$:
The bounds on $y$ go from ``curve to curve,'' i.e., $0\leq y\leq 1-x/2$, and the bounds on $x$ go from ``point to point,'' i.e., $0\leq x\leq 2$.
\begin{align*}
...
\mfigure{-2in}{$R$ is the union of two nonoverlapping regions, $R_1$ and $R_2$.}{fig:double_region}{figures/figdouble_region}

\example{ex_double3}{Evaluating a double integral}{Let $f(x,y) = \sin x\cos y$ and $R$ be the triangle with vertices $(-1,0)$, $(1,0)$ and $(0,1)$ (see \autoref{fig:double3}). Evaluate the double integral $\iint_Rf(x,y)\ dA$.
\mfigurethree{width=150pt,3Dmenu,activate=onclick,deactivate=onclick,
3Droll=0.,
3Dortho=0.0046491301618516445,
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{trim=0mm 15mm 0mm 0mm,clip}{0in}{Finding the signed volume under a surface in \autoref{ex_double3}.}{fig:double3}{figures/figdouble3}
%\mfigure[trim=0mm 15mm 0mm 0mm,clip]{0in}{Finding the signed volume under a surface in \autoref{ex_double3}.}{fig:double3}{figures/figdouble3}
}
...
\example{ex_double4}{Evaluating a double integral}{Evaluate $\iint_R (4-y)\ dA$, where $R$ is the region bounded by the parabolas $y^2=4x$ and $x^2=4y$, graphed in \autoref{fig:double4}.
\mfigurethree{width=150pt,3Dmenu,activate=onclick,deactivate=onclick,
3Droll=0,
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.25,trim=0mm 0mm 1mm 0mm,clip}{0in}{Finding the volume under the surface in \autoref{ex_double4}.}{fig:double4}{figures/figdouble4}
%\mfigure[scale=1.25,trim=0mm 0mm 1mm 0mm,clip]{0in}{Finding the volume under the surface in \autoref{ex_double4}.}{fig:double4}{figures/figdouble4}
}
...
%
%\example{ex_double5}{Changing the order of integration}{Rewrite the iterated integral $\ds \int_0^6\int_0^{x/3} \frac{x}{x^2+y^2+1}\ dy\ dx$ with the order of integration $dx\ dy$.
%}
...
%\mfigure{0in}{Sketching the region $R$ described by the iterated integral in \autoref{ex_double5}.}{fig:double5}{figures/figdouble5}
%
...
%
%\example{ex_double7}{Changing the order of integration}{%Change the order of integration of $\ds\int_0^2\int_{y^2/4}^{(y+4)/2}f(x,y)\ dx\ dy$.}
...
%\mfigure{0in}{Drawing the region determined by the bounds of integration in \autoref{ex_double7}.}{fig:double7}{figures/figdouble7}
%
...
\example{ex_double6}{Changing the order of integration}{Rewrite the iterated integral $\ds \int_0^3\int_y^3 e^{-x^2}\ dx\ dy$ with the order $dy\ dx$. Comment on the feasibility to evaluate each integral.}
...
\mfigure{0in}{Determining the region $R$ determined by the bounds of integration in \autoref{ex_double6}.}{fig:double6}{figures/figdouble6}
...
This last integral is easy to evaluate with substitution, giving a final answer of $\frac12(1-e^{-9})\approx 0.5$. \autoref{fig:double6b} shows the surface over $R$.
\mfigurethree{width=150pt,3Dmenu,activate=onclick,deactivate=onclick,
3Droll=0.,
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.25,trim = 1mm 10mm 1mm 0mm, clip}{0in}{Showing the surface $f$ defined in \autoref{ex_double6} over its region $R$.}{fig:double6b}{figures/figdouble6b}
%\mfigure[scale=1.25,trim = 1mm 10mm 1mm 0mm, clip]{0in}{Showing the surface $f$ defined in \autoref{ex_double6} over its region $R$.}{fig:double6b}{figures/figdouble6b}
...
3Dcoo=-13.985211372375488 45.56017303466797 41.332950592041016,
3Droo=201.0000081559461,
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.25,trim=0mm 0mm 1mm 0mm,clip}{0in}{Finding the average value of $f$ in \autoref{ex_double8}.}{fig:double8}{figures/figdouble4}
%\mfigure[scale=1.25,trim=0mm 0mm 1mm 0mm,clip]{0in}{Finding the average value of $f$ in \autoref{ex_double8}.}{fig:double8}{figures/figdouble4}
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.25,trim=1mm 0mm 1mm 0mm,clip}{0in}{Showing how an iterated integral used to find area also finds a certain volume.}{fig:double_summary}{figures/figdouble_summary}
%\mfigure[scale=1.25,trim=1mm 0mm 1mm 0mm,clip]{0in}{Showing how an iterated integral used to find area also finds a certain volume.}{fig:double_summary}{figures/figdouble_summary}
