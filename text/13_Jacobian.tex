...
Evaluate $\ds\iint_{R} e^{\frac{x-y}{x+y}} \,dA$, where $R= \{(x,y): x \ge 0, y \ge 0, x + y \le 1 \}$.
\solution
...
%Evaluate $\iiint_D x\,dV$ where $D$ is the region bounded by the first octant, the cone $z=\sqrt{x^2+y^2}$, and $z=3$.
%\solution
