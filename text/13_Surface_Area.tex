...
\mnote{\textbf{Note:} As before, we think of ``$\iint_R\ dS$'' as meaning ``sum up lots of little surface areas over $R$.''\\
...
\example{ex_surfacearea1}{Finding the surface area of a plane over a triangle}{Let $f(x,y) = 4-x-2y$, and let $R$ be the region in the plane bounded by $x=0$, $y=0$ and $y=2-x/2$, as shown in \autoref{fig:surfacearea1}. Find the surface area of $f$ over $R$.
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{}{0in}{Finding the area of a triangle in space in \autoref{ex_surfacearea1}.}{fig:surfacearea1}{figures/figsurfacearea1}%
}
%\mfigure{0in}{Finding the area of a triangle in space in \autoref{ex_surfacearea1}.}{fig:surfacearea1}{figures/figsurfacearea1}}
...
\example{ex_surfacearea2}{The surface area of a sphere.}{Find the surface area of the sphere with radius $a$ centered at the origin, whose top hemisphere has equation $f(x,y)=\sqrt{a^2-x^2-y^2}$. 
}
...
\end{align*}
%
\mnote{\textbf{Note:} The inner integral in \autoeqref{eq:exsurfacearea2} is an improper integral, as the integrand of $\ds \int_0^ar\sqrt{\frac{a^2}{a^2-r^2}}\ dr$ is not defined at $r=a$. To properly evaluate this integral, one must use the techniques of \autoref{sec:improper_integration}.\\

The reason this need arises is that the function $f(x,y) = \sqrt{a^2-x^2-y^2}$ fails the requirements of \autoref{def:surfacearea}, as $f_x$ and $f_y$ are not continuous on the boundary of the circle $x^2+y^2=a^2$.\\
...
\example{ex_surfacearea3}{Finding the surface area of a cone}{The general formula for a right cone with height $h$ and base radius $a$, as shown in \autoref{fig:surfacearea3}, is
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.5,trim=5mm 5mm 5mm 0mm, clip}{0in}{Finding the surface area of a cone in \autoref{ex_surfacearea3}.}{fig:surfacearea3}{figures/figsurfacearea3}
%\mfigure[scale=1.5,trim=5mm 5mm 5mm 0mm, clip]{0in}{Finding the surface area of a cone in \autoref{ex_surfacearea3}.}{fig:surfacearea3}{figures/figsurfacearea3}
...
\mnote[.2in]{\textbf{Note:} Note that once again $f_x$ and $f_y$ are not continuous on the domain of $f$, as both are undefined at $(0,0)$. (A similar problem occurred in the previous example.) Once again the resulting improper integral converges and the computation of the surface area is valid.}
\begin{align*}
...
\example{ex_surfacearea4}{Finding surface area over a region}{Find the area of the surface $f(x,y) = x^2-3y+3$ over the region $R$ bounded by $-x\leq y\leq x$, $0\leq x\leq 4$, as pictured in \autoref{fig:surfacearea4}.}
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{scale=1.2,trim=1mm 0mm 1mm 0mm, clip}{-.5in}{Graphing the surface in \autoref{ex_surfacearea4}.}{fig:surfacearea4}{figures/figsurfacearea4}
%\mfigure[scale=1.2,trim=1mm 0mm 1mm 0mm, clip]{0in}{Graphing the surface in \autoref{ex_surfacearea4}.}{fig:surfacearea4}{figures/figsurfacearea4}
...
\iint_R\sqrt{10+4x^2}\ dA &= \int_0^4\int_{-x}^x\sqrt{10+4x^2}\ dy \ dx\\
				...
				&= \frac13\big(37\sqrt{74}-5\sqrt{10}\big) %\approx 100.825
				\text{u}^2.
\end{align*}
