...
\[
V = \iint_R f(x,y)\ dA - \iint_R g(x,y)\ dA = \iint_R \bigl(f(x,y)-g(x,y)\bigr)\ dA.
\]
...
\begin{align*}
V &= \iint_R \bigl(8-3x-2y-(3x+y-4)\bigr)\ dA \\
	&= \int_0^2\int_0^{4-2x} \bigl(12-6x-3y\bigr)\ dy\ dx\\
	&= 16\text{units}^3.
\end{align*}
...
\[\ds \int_0^2\int_0^{4-2x} \bigl(8-3x-2y-(3x+y-4)\bigr)\ dy\ dx.\]
...
\[\int_0^2\int_0^{4-2x} \bigl(8-3x-2y-(3x+y-4)\bigr)\ dy\ dx = \int_0^2\int_0^{4-2x}\left(\int_{3x+y-4}^{8-3x-2y}\ dz\right)\ dy\ dx.\]
...
\begin{align*}
	...
	&= 6\ \text{units}^3.		
\end{align*}
\noindent The order $dz\ dx\ dy$:\bigskip
...
\noindent The order $dx\ dy\ dz$:\bigskip
...
We now need to collapse the space region onto the $y$-$z$ plane, as we see in \autoref{fig:trip2b}(a). (Again, we find the equation of the line $z=2-y/3$ by setting $x=0$ in the equation $x=3-y/2-3z/2$.) We need to find bounds on this region with the order $dy\ dz$. The \emph{curves} that bound $y$ are $y=0$ and $y=6-3z$; the \emph{points} that bound $z$ are 0 and 2. Thus the triple integral giving volume is:
\[
 \begin{gathered}
...
\noindent The order $dx\ dz\ dy$:\bigskip
...
\noindent The order $dy\ dz\ dx$:\bigskip
...
\noindent The order $dy\ dx\ dz$:\bigskip
...
\begin{align*}
	\int_{-1}^1\int_{-\sqrt{1-x^2}}^{0}\int_0^{-y}\ dz\ dy\ dx
	&= \int_{-1}^1\int_{-\sqrt{1-x^2}}^{0}\bigl(-y\bigr)\ dy\ dx\\
	&=\int_{-1}^1\bigl(-\frac12y^2\bigr)\Big|_{-\sqrt{1-x^2}}^{0}\ dx\\
	&= \int_{-1}^1 \frac12\bigl(1-x^2\bigr)\ dx\\
	&= \left.\left(\frac12\left(x-\frac13x^3\right)\right)\right|_{-1}^1\\
	&= \frac23\text{units}^3.
\end{align*}

\noindent With the order $dy\ dx\ dz$:\bigskip
...
\noindent With the order $dx\ dz\ dy$:\bigskip
...
All that is left is to determine bounds of $R_1$ and $R_2$, depending on whether we are integrating with order $dx\ dy$ or $dy\ dx$. We give the final integrals here, leaving it to the reader to confirm these results.\bigskip

\noindent $dz\ dy\ dx$:\\[-2\baselineskip]\parbox[t]{\linewidth}{%
\begin{gather*}
 \begin{gathered}
  0\leq z\leq 1-x/2\\
  0\leq y\leq 2x\\
...
\noindent $dz\ dx\ dy$:\\[-2\baselineskip]\parbox[t]{\linewidth}{%
\begin{gather*}
 \begin{gathered}
  0\leq z\leq 1-x/2\\
  y/2\leq x\leq 2\\
...
\noindent $dy\ dx\ dz$:\\[-\baselineskip]\parbox[t]{\linewidth}{%
\[
 \begin{gathered}
  0\leq y\leq 4-4z\\
  0\leq x\leq 2-2z\\
...
\]}
\noindent $dy\ dz\ dx$:\\[-\baselineskip]\parbox[t]{\linewidth}{%
\[
 \begin{gathered}
  0\leq y\leq 4-4z\\
  0\leq z\leq 1-x/2\\
...
\]}
\noindent $dx\ dy\ dz$:\\[-\baselineskip]\parbox[t]{\linewidth}{%
\[
 \begin{gathered}
  0\leq x\leq 2-2z\\
  0\leq y\leq 4-4z\\
...
\]}
\noindent $dx\ dz\ dy$:\\[-\baselineskip]\parbox[t]{\linewidth}{%
\[
 \begin{gathered}
  0\leq x\leq 2-2z\\
  0\leq z\leq 1-y/4\\
...
\noindent The order $dz\ dy\ dx$:\bigskip
...
$\ds \int_0^1\int_{x^2}^x\int_{x^2-y}^{2x+3y} \bigl(xy+2xz\bigr)\ dz\ dy\ dx $
\begin{align*}
			&=	\int_0^1\int_{x^2}^x\left(\int_{x^2-y}^{2x+3y} \bigl(xy+2xz\bigr)\ dz\right)\ dy\ dx\\
			&= \int_0^1\int_{x^2}^x\left(\bigl(xyz+ xz^2\bigr)\Big|_{x^2-y}^{2x+3y}\right)\ dy\ dx\\
			&= \int_0^1\int_{x^2}^x\Biggl(xy(2x+3y)+x(2x+3y)^2-\Bigl(xy(x^2-y)+x(x^2-y)^2\Bigr)\Biggr)\ dy\ dx\\
			&=\int_0^1\int_{x^2}^x\Bigl(-x^5+x^3y+4x^3+14x^2y+12xy^2\Bigr)\ dy\ dx.
			\intertext{We continue as we have in the past, showing fewer steps.}
			&= \int_0^1\Biggl(-\frac72x^7-8x^6-\frac72x^5+15x^4\Biggr)\ dx\\
...
M &= \iiint_D \delta(x,y,z)\ dV \\
  &= \int_0^3\int_0^{6-2x}\int_0^{2-y/3-2x/3} \bigl(3\bigr)\ dz\ dy\ dx\\
...
\begin{align*}
M_{xy} &= \iiint_D 3z\ dV \\
			&= \int_0^3\int_0^{6-2x}\int_0^{2-y/3-2x/3} \bigl(3z\bigr)\ dz\ dy\ dx\\
			%&= \int_0^3\int_0^{6-2x} \frac16\bigl(2x+y-6\bigr)^2\ dy\ dx \\
			&= \int_0^3\int_0^{6-2x} \frac32\bigl(2-y/3-2x/3\bigr)^2\ dy\ dx \\
			&= \int_0^3 -\frac49\bigl(x-3\bigr)^3\ dx\\
			&= 9.
\end{align*}
...
\begin{align*}
	M
	&= \iiint_D \bigl(10+x^2+5y-5z\bigr)\ dV \\
	&= \int_{-1}^1\int_{-\sqrt{1-x^2}}^0\int_0^{-y} \bigl(10+x^2+5y-5z\bigr)\ dV\\
	&= \frac{64}5-\frac{15\pi}{16} \approx 3.855.\\
	M_{yz}
	&= \iiint_D x\bigl(10+x^2+5y-5z\bigr)\ dV = 0.\\
	M_{xz}
	&= \iiint_D y\bigl(10+x^2+5y-5z\bigr)\ dV = 2-\frac{61\pi}{48}\approx -1.99.\\
	M_{xy}
	&= \iiint_D z\bigl(10+x^2+5y-5z\bigr)\ dV = \frac{61\pi}{96}-\frac{10}9\approx 0.885.
\end{align*}
Note how $M_{yz}=0$, as expected. The center of mass is
\[\bigl(\overline{x},\overline{y},\overline{z}\bigr) = \left(0,\frac{-1.99}{3.855},\frac{0.885}{3.855}\right) \approx \bigl(0,-0.516, 0.230\bigr).\]
\end{example}
