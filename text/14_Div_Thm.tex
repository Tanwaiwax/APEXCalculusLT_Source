...
\definition{defn:surfintvec}{Vector Surface Integral}{Let $\Sigma$ be a surface in $\mathbb{R}^{3}$ and let $\vecf(x,y,z) = f_1(x,y,z) \veci + f_2(x,y,z) \vecj + f_3(x,y,z) \veck$ be a vector field defined on some subset of $\mathbb{R}^{3}$ that contains $\Sigma$. The \textbf{surface integral} of \vecf\ over $\Sigma$ is
  \begin{equation}\label{eqn:surfintvec}
   ...
   \iint_{\Sigma} \Dotprod{\vecf}{\vecn}\,d\sigma ,
  \end{equation}
  where, at any point on $\Sigma$, \vecn\ is the outward unit normal vector to $\Sigma$.}

\mnote[-4\baselineskip]{Sometimes, the notation $\oiint_{\Sigma} \Dotprod{\vecf}{d\sigma}$ is used instead of $\iint_{\Sigma} \Dotprod{\vecf}{d\sigma}$ when $\Sigma$ is a closed surface.\index{$\oiint_{\Sigma}$}  Especially in physics texts, it is common to see simply $\oint_{\Sigma}$ instead of $\oiint_{\Sigma}$.}
...
Let $\Sigma$ be a closed surface in $\mathbb{R}^{3}$ which bounds a solid $S$, and let $\vecf(x,y,z) = f_1(x,y,z) \veci + f_2(x,y,z) \vecj + f_3(x,y,z) \veck$ be a vector field defined on some subset of $\mathbb{R}^{3}$ that contains $\Sigma$. Then
  \begin{equation}\label{eqn:divergence}
   \iint_{\Sigma} \Dotprod{\vecf}{d\sigma} = \iiint_{S} \Div\vecf dV ,
  \end{equation}
  where
...
\example{ex_div_thm}{Using the Divergence Theorem}{Evaluate $\iint_{\Sigma} \Dotprod{\vecf}{d\sigma}$, where $\vecf(x,y,z) = x\veci +  y\vecj + z\veck$ and $\Sigma$ is the unit sphere $x^2 + y^2 + z^2 = 1$.}{We see that $\Div\vecf = 1+1+1=3$, so
 \begin{align*}
  \iint_{\Sigma} \Dotprod{\vecf}{d\sigma} &=
...
\renewcommand{\arraystretch}{1.4}
 \begin{tabular}{|l|p{.32\specialboxlength}|p{.43\specialboxlength}|}\hline
  ...
  & $\Phi:\mathbb{R}^2\to\mathbb{R}^3$\par parameterizes a surface \\\hline
  ...
  \qquad$=\iint_D f(\Phi(u,v))\norm{\Phi_u\times\Phi_v}du dv$
  \\\hline
  $\vec F:\mathbb{R}^3\to\mathbb{R}^3$
  & vector line integral:\par
  $\int_{c(D)}\vec F\cdot d\vec s$\par
  \qquad$=\int_D\vec F(\vec c(t))\cdot\vec c\,'(t)dt$
  & vector surface integral:\par
  $\iint_{\Phi(D)}\vec F\cdot d\vec S$\par
  \qquad$=\iint_D\vec F(\Phi(u,v))\cdot(\Phi_u\times\Phi_v)dudv$
  \\\hline
 \end{tabular}}
