...
\example*{exmp_greenhole}{Green's Theorem with a Hole}{Let $\textbf{f}(x,y) = P(x,y)\,\textbf{i} + Q(x,y)\,\textbf{j}$, where
 ...
 \]
 and let $R =\lbrace\,(x,y): 0 < x^2 + y^2 \le 1\,\rbrace$. For the boundary curve $C:x^2 + y^2 = 1$, traversed counterclockwise, it was shown in Exercise \ref{contra_green}(b) in \autoref{sec:line_int_props} that $\oint_{C}\dotp{f}{dr} = 2\pi$. But
 \[
