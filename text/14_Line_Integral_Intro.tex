...
\mtable[-4\baselineskip]{Finding area under a curve in space.}{fig:lineint0}{%
...
3Dlights=Headlamp}{width=\marginparwidth}{figures/figline_integral_intro1_3D}
\\(a)\\[10pt]
\myincludeasythree{width=\marginparwidth,
3Droll=0,
3Dortho=0.005003686994314194,
...
3Dcoo=28.221464157104492 62.0896110534668 72.89360809326172,
3Droo=149.99999640034895,
3Dlights=Headlamp}{width=\marginparwidth}{figures/figline_integral_intro1b_3D}
\\(b)\\[10pt]
\myincludeasythree{width=\marginparwidth,
3Droll=0,
3Dortho=0.005003686994314194,
...
3Dcoo=28.221464157104492 62.0896110534668 72.89360809326172,
3Droo=149.99999640034895,
3Dlights=Headlamp}{width=\marginparwidth}{figures/figline_integral_intro1c_3D}
\\(c)}
...
\mnote{\textbf{Note:} \autoref{def:line_integral1} uses the term \sword{scalar field} which has not yet been defined. Its meaning is discussed in the paragraph preceding \autoref{def:line_integral2} when it is compared to a \sword{vector field}.}
...
{\begin{itemize}
	\item Let $C$ be a curve parametrized by $\vrt =\bracket{g(t),h(t)}$, $a\leq t\leq b$, where $g$ and $h$ are continuously differentiable, and let $z=f(x,y)$, where $f$ is continuous over $C$. Then\index{line integral!over scalar field}%
...
To be clear, the first point of \autoref{thm:line1} can be used to find the area under a surface $z=f(x,y)$ and above a curve $C$. We will later give an understanding of the line integral when $C$ is a curve in space.

\youtubeVideo{fjEvsinvtnw}{Line Integrals --- Evaluating a Line Integral}

Let's do an example where we actually compute an area.

\mtable[-6\baselineskip]{Finding area under a curve in \autoref{ex_linescalarfield2}.}{fig:linescalarfield2}{%
\myincludeasythree{width=\marginparwidth,
3Droll=0,
...
3Dcoo=36.11199951171875 39.69871139526367 85.62228393554688,
3Droo=149.9999948024772,
3Dlights=Headlamp}{width=\marginparwidth}{figures/figlinescalarfield3_3D}
\\(a)\\[10pt]
\myincludeasythree{width=\marginparwidth,
3Droll=0,
3Dortho=0.004519370850175619,
...
3Dcoo=36.11199951171875 39.69871139526367 85.62228393554688,
3Droo=149.9999948024772,
3Dlights=Headlamp}{width=\marginparwidth}{figures/figlinescalarfield3b_3D}
\\(b)}
...
3Dlights=Headlamp}{width=\marginparwidth}{figures/figlinescalarfield4_3D}}
...
\mtable[-5\baselineskip]{Illustrating properties of line integrals.}{fig:line_int_prop}{%
\begin{tikzpicture}
...
%The second property of note is that the orientation of the parametrization of $C$ does not affect the value of the line integral (\textit{over scalar fields}. Later, we'll see cases where orientation does matter.) In \autoref{fig:line_int_prop}, the arrows indicating orientation are extraneous. In \autoref{sec:line_int_vf}, where we study line integrals \emph{over vector fields}, the orientation of the parametrization does make a difference.
...
{A thin wire follows the path $\vrt =\bracket{1+\cos t,1+\sin t, 1+ \sin(2t)}$, $0\leq t\leq 2\pi$. The density of the wire is determined by its position in space: $\delta(x,y,z) = y+z$ gm/cm. The wire is shown in \autoref{fig:linescalarfield6}, where a light color indicates low density and a dark color represents high density. Find the mass  and center of mass of the wire.%
...
3Dlights=Headlamp}{width=\marginparwidth}{figures/figlinescalarfield6_3D}}}
