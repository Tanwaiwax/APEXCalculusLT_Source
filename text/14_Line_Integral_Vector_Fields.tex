...
%
\mtable{Paths through a vector field in \autoref{ex_livf1}.}{fig:livf1}{\begin{tikzpicture}[alt={The curves y=x and y=x^4 for 0≤x≤1, along with a field that is pointing up and right.}]
...
%
\mtable{The vector field and curve in \autoref{ex_livf3}.}{fig:livf3}{\begin{tikzpicture}[alt={The curve y=x^3 for 0≤x≤1 with an arrow pointing for an increasing x, and then the line y=x with an arrow back to the origin.  There is also a field pointing upward that starts leftward and then changes to rightward.}]
\begin{axis}[width=1.16\marginparwidth,height=1.16\marginparwidth,
tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,axis equal,
xtick={1},ytick={1},
