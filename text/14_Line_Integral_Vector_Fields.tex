...
\mnote{\textbf{Note:} When $\vec F=\bracket{g,h}$ and $\vec r=\bracket{x,y}$ parameterizes $C$, other common notations for the line integral over $\vec F$ along $C$ are $\int_C gdx+\int_C hdy$ and $\int_C gdx+hdy$.}
...
Let $\vec F$ be a vector field with continuous components defined on a smooth curve $C$, parameterized by $\vrt$, $a\leq t\leq b$, where $\vec r$ is continuously differentiable. Then\index{vector field!over vector field}
	...
	=\int_a^b \vec F\bigl(\vec r(t)\bigr) \cdot \vrp(t)\ dt.
	\]
\end{keyidea}
...
\mtable{Paths through a vector field in \autoref{ex_livf1}.}{fig:livf1}{\begin{tikzpicture}
...
\begin{align*}
	...
	&= \int_0^1 \bigl(t + 4t^4+ 4t^7\bigr)\ dt \\
...
\begin{align*}
	...
	&= \int_{-1}^1 \bigl(2t^2-1+4t^2\bigr)\ dt = 2 \text{ ft-lbs}.
\end{align*}
...
\begin{align*}
	...
	&= \int_{\pi}^{2\pi} \bigl(-\sin^2t-\sin t+\cos^2t\bigr)\ dt = 2 \text{ ft-lbs}.
\end{align*}
...
\begin{enumerate}
	...
	\[
	\ds \int_C\bigl(k_1\vec F+k_2\vec G\bigr)\cdot d\vec r
...
\mtable{The vector field and curve in \autoref{ex_livf3}.}{fig:livf3}{\begin{tikzpicture}
\begin{axis}[width=1.16\marginparwidth,height=1.16\marginparwidth,
tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,axis equal,
xtick={1},ytick={1},
...
Let $\vec F =\bracket{3(y-1/2),1}$ and let $C$ be the path that starts at $(0,0)$, goes to $(1,1)$ along the curve $y=x^3$, then returns to $(0,0)$ along the line $y=x$, as shown in \autoref{fig:livf3}. Evaluate $\oint_C \vec F\cdot d\vec r$.
\solution
...
\begin{align*}
	...
	&= \int_0^1\Bigl(3t^3+3t^2-3/2\Bigr)\ dt - \int_0^1 \Bigl(3t-1/2\Bigr)\ dt\\
...
3Dlights=Headlamp}{width=\marginparwidth}{figures/figlivf4_3D}}
...
as shown in \autoref{fig:livf4}. Evaluate $\int_C\vec F\cdot d\vec r$.
\solution
...
\begin{align*}
	...
	&= \int_0^{2\pi} \Bigl(\sin^2t+\cos^2t + \frac1{2\pi}\Bigr)dt\\
	&= 2\pi + 1 \approx 7.28.
\end{align*}
\end{example}
...
\mtable{$R_1$ is simply connected; $R_2$ is connected, but not simply connected; $R_3$ is not connected.}{fig:simply_connected_plane}{\begin{tikzpicture}
...
\mtable[2\baselineskip]{The domains in (a) are simply connected, while the domains in (b) are not.}{fig:simply_connected_space}{%
\myincludeasythree{width=\marginparwidth,
3Droll=0,
...
3Dlights=Headlamp}{width=\marginparwidth}{figures/figsimply_connected_1_3D}
\\(a)\\[10pt]
\myincludeasythree{width=\marginparwidth,
3Droll=0,
3Dortho=0.009668752551078796,
...
3Dcoo=0.47741085290908813 38.61087417602539 -23.2408390045166,
3Droo=149.999998390057,
3Dlights=Headlamp}{width=\marginparwidth}{figures/figsimply_connected_2_3D}
\\(b)}
...
\begin{itemize}
	...
	\item	The line integral $\int_C \vec F\cdot d\vec r$ is \textbf{path independent} and can be written as
...
{Let $f$ be a differentiable function defined on a  domain $D$ in the plane or in space (i.e., $z = f(x,y)$ or $w = f(x,y,z)$) and let $\vec F = \nabla f$, the gradient of $f$. Then $f$ is a \textbf{potential function} of $\vec F$.\index{potential function}\index{vector field!potential function of}
}
...
\begin{align*}
	\int_C \vec F\cdot d\vec r &= \int_C\vec F\bigl(\vrt\bigr)\cdot\vrp(t)\ dt\\
	&= \int_0^1\bracket{3(t)(t^2+2t+1)+2t, t^3+1}\cdot\bracket{t,2t+2}\ dt\\
	&= \int_0^1 \bigl(5t^4+8t^3+3t^2+4t+2\bigr)\ dt\\
