...
Note that we can also verify that the value of the line integral of \vecf along any curve $C$ going from $(0,0)$ to $(1,2)$ will always be $\frac{13}{3}$, since by \autoref{thm:lineintsuffpath}
 \[
  \int_{C}\vecf\cdot d\vecr = F(1,2) - F(0,0) = \frac{1}{3}(1)^3 + (1)(2)^2 - (0+0) = \frac{1}{3} + 4 = \frac{13}{3} .\eoehere
 \]}
...
% todo Tim do we want corollaries?
\theorem{cor:lineintsuffpath}{Closed Line Integrals of Conservative Fields}{If a vector field $\vecf$ has a potential in a region $R$, then $\ds\oint_{C}\vecf\cdot d\vecr = 0$ for any closed curve $C$ in $R$ (i.e., $\ds\oint_{C}\nabla F\cdot d\vecr = 0$ for any real-valued function $F(x,y)$).}
