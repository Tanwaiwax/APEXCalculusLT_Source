...
\theorem{thm:lineintreparam}{Line Integral is Independent of Parameterization}{Let $\textbf{f}(x,y) = P(x,y)\,\textbf{i} + Q(x,y)\,\textbf{j}$ be a vector field, and let $C$ be a smooth curve parametrized by $x=x(t)$, $y=y(t)$, $a \le t \le b$. Suppose that $t=\alpha(u)$ for $c \le u \le d$, such that $a=\alpha(c)$, $b=\alpha(d)$, and $\alpha\,'(u) > 0$ on the open interval $(c,d)$ (i.e. $\alpha(u)$ is strictly increasing on $[c,d]$). Then $\int_C \dotp{f}{dr}$ has the same value for the parametrizations $x=x(t)$, $y=y(t)$, $a \le t \le b$ and $x=\tilde{x}(u)=x(\alpha(u))$, $y=\tilde{y}(u)=y(\alpha(u))$, $c \le u \le d$.}
...
Notice that the condition $\alpha\,'(u) > 0$ in \autoref{thm:lineintreparam} means that the two parametrizations move along $C$ in the same direction. That was \emph{not} the case with the ``reverse'' parametrization for $-C$: for $u=a+b-t$ we have $t=\alpha(u)=a+b-u \Rightarrow \alpha\,'(u) = -1 <0$.
...
\begin{lxfigure}
 ...
 \end{tabular}
 \captionof{figure}{Closed vs nonclosed curves}
...
\theorem{thm:lineintpathindep}{Path Independence of Line Integrals}{In a region $R$, the line integral $\int_{C}\dotp{f}{dr}$ is independent of the path between any two points in $R$ if and only if $\oint_{C}\dotp{f}{dr} = 0$ for every closed curve $C$ which is contained in $R$.}
...
\begin{proof}
 ...
 \mtable{The idea of proving \autoref{thm:lineintpathindep}.}{fig_path_indep}{\begin{tikzpicture}
  ...
  \draw [black,line width=1.2pt] (0,-1) arc (-90:-180:2 and 1);
  \node [above] at (0,1.2) {\small $C_1$};
  \node [below] at (0,-1.2) {\small $C_2$};
  \node [left] at (-2.1,0) {\small $P_1$};
  \node [right] at (2.1,0) {\small $P_2$};
 \end{tikzpicture}}
...
%
%The proof is virtually identical to the proof of \autoref{thm:dirderiv} from Section 2.4 (which uses the Mean Value Theorem), so we omit it.\footnote{See \cite[\S\,6.5]{tm}.}
...
\theorem{thm:lineintsuffpath}{Fundamental Theorem of Line Integrals}{Let $\textbf{f}(x,y) = P(x,y)\,\textbf{i} + Q(x,y)\,\textbf{j}$ be a vector field in some region $R$, with $P$ and $Q$ continuously differentiable functions on $R$. Let $C$ be a smooth curve in $R$ parametrized by $x=x(t)$, $y=y(t)$, $a \le t \le b$. Suppose that there is a real-valued function $F(x,y)$ such that $\nabla F = \textbf{f}$ on $R$. Then
 \[
  \int_{C}\dotp{f}{dr} = F(B) - F(A) ,
 \]
...
\example{exmp_lineintexmpclosed}{Using the Fundamental Theorem of Line Integrals}{Recall from Examples \ref{exmp_lineintexmp}  and \ref{exmp_lineintexmppoly} in \autoref{sec:line_int} that the line integral $\int_C (x^2 + y^2 )\,dx + 2xy\,dy$ was found to have the value $\frac{13}{3}$ for three different curves $C$ going from the point $(0,0)$ to the point $(1,2)$. Use \autoref{thm:lineintsuffpath} to show that this line integral is indeed path independent.}{We need to find a real-valued function $F(x,y)$ such that
 ...
 \]
 Suppose that $\frac{\partial F}{\partial x} = x^2 + y^2$, Then we must have $F(x,y) = \frac{1}{3}x^3 + xy^2 + g(y)$ for some function $g(y)$. So $\frac{\partial F}{\partial y} = 2xy + g\,'(y)$ satisfies the condition $\frac{\partial F}{\partial y} = 2xy$ if $g\,'(y)=0$, i.e. $g(y)=K$, where $K$ is a constant. Since any choice for $K$ will do (why?), we pick $K=0$. Thus, a potential $F(x,y)$ for  $\textbf{f}(x,y) = ( x^2 + y^2 )\,\textbf{i} + 2xy\,\textbf{j}$ exists, namely
 \[F(x,y) = \frac{1}{3}x^3 + xy^2 .\]
 Hence the line integral $\int_C (x^2 + y^2 )\,dx + 2xy\,dy$ is path independent.\index{conservative field}
 
Note that we can also verify that the value of the line integral of \textbf{f} along any curve $C$ going from $(0,0)$ to $(1,2)$ will always be $\frac{13}{3}$, since by \autoref{thm:lineintsuffpath}
 \[
  \int_{C}\dotp{f}{dr} = F(1,2) - F(0,0) = \frac{1}{3}(1)^3 + (1)(2)^2 - (0+0) = \frac{1}{3} + 4 = \frac{13}{3} .\eoehere
 \]}

A consequence of \autoref{thm:lineintsuffpath} in the special case where $C$ is a closed curve, so that the endpoints $A$ and $B$ are the same point, is the following important corollary:

% todo do we want corollaries?
\theorem{cor:lineintsuffpath}{Closed Line Integrals of Conservative Fields}{If a vector field $\textbf{f}$ has a potential in a region $R$, then $\ds\oint_{C}\dotp{f}{dr} = 0$ for any closed curve $C$ in $R$ (i.e. $\ds\oint_{C}\dotp{\nabla F}{dr} = 0$ for any real-valued function $F(x,y)$).}

\example{ex_use_cor}{Calculating a Closed Line Integral of a Conservative Field}{Evaluate $\ds\oint_C x\,dx + y\,dy$ for $C:$ $x=2\cos t$, $y=3\sin t$, $0\le t\le 2\pi$.}{The vector field $\textbf{f}(x,y) = x\,\textbf{i} + y\,\textbf{j}$ has a potential $F(x,y)$:
 \begin{align*}
 ...
 \[\oint_C x\,dx + y\,dy = \oint_{C}\dotp{f}{dr} = 0\]
