...
\mnote{\textbf{Note:} A function is \emph{one to one} on its domain if the function never repeats an output value over the domain. In the case of $\vec r(u,v)$, $\vec r$ is one to one if $\vec r(u_1,v_1) \neq \vec r(u_2,v_2)$ for all points $(u_1,v_1) \neq (u_2,v_2)$ in the domain of $\vec r$.\index{one to one}}

\definition{def:parametrized_surface}{Parametrized Surface}
{Let $\vec r(u,v) = \bracket{f(u,v),g(u,v),h(u,v)}$ be a vector--valued function
...
\mtable{A M\"obius band, a non-orientable surface.}{fig:mobius}{%
...
\mtable{The surface parametrized in \autoref{ex_parsurf1}.}{fig:parsurf1}{%
\myincludeasythree{width=\marginparwidth,
3Droll=0,
...
3Dlights=Headlamp}{width=\marginparwidth}{figures/figparsurf1_3D}}
...
\mtable{The surface parametrized in \autoref{ex_parsurf2}.}{fig:parsurf2}{%
\myincludeasythree{width=\marginparwidth,
3Droll=0,
3Dortho=0.005000247620046139,
3Dc2c=0.6257953643798828 0.642341136932373 0.44246822595596313,
3Dcoo=-11.887701988220215 -11.637335777282715 52.40430450439453,
3Droo=399.9999800778292,
3Dlights=Headlamp}{width=\marginparwidth}{figures/figparsurf2_3D}}
...
{We can parametrize the circular boundary of $R$ with the vector--valued function $\bracket{2\cos u,2\sin u}$, where $0\leq u\leq 2\pi$. We can obtain the interior of $R$ by scaling this function by a variable amount, i.e., by multiplying by $v$: $\bracket{2v\cos u,2v\sin u}$, where $0\leq v\leq 1$.
...
In the following two examples, we parametrize the same surface over triangular regions. Each will use $v$ as a ``scaling factor'' as done in \autoref{ex_parsurf2}.
...
3Dcoo=-11.887701988220215 -11.637335777282715 52.40430450439453,
3Droo=399.9999800778292,
3Dlights=Headlamp}{width=1.16\marginparwidth}{figures/figparsurf3_3D}
\\(b)}

\example{ex_parsurf3}{Parameterizing a surface over a triangle}
{Parametrize the surface $z=x^2+2y^2$ over the triangular region $R$ enclosed by the coordinate axes and the line $y=2-2x/3$, as shown in \autoref{fig:parsurf3}(a).}
...
\[
\vec r(u,v) =\bracket{u, v(2-2u/3), u^2+2\big(v(2-2u/3)\big)^2},
...
Another common way to parametrize the surface is to begin with $y=u$, $0\leq u\leq 2$. Solving the equation of the line $y=2-2x/3$ for $x$, we have $x = 3-3y/2$, leading to using $x=v(3-3u/2)$, $0\leq v\leq 1$. With $z=x^2+2y^2$, we have $\vec r(u,v) =\bracket{v(3-3u/2),u, \big(v(3-3u/2)\big)^2+2v^2}$, $0\leq u\leq 2$, $0\leq v\leq 1$.}
...
3Dcoo=-11.887701988220215 -11.637335777282715 52.40430450439453,
3Droo=399.9999800778292,
3Dlights=Headlamp,add3Djscript=asylabels.js}{width=1.16\marginparwidth}{figures/figparsurf4_3D}
\\(b)}%
%
Parametrize the surface $z=x^2+2y^2$ over the triangular region $R$ enclosed by the  lines $y=3-2x/3$, $y=1$ and $x=0$ as shown in \autoref{fig:parsurf4}(a).}
...
We will describe the general method of proceeding following this example. For now, consider $y = 1+v(2-2u/3)$, $0\leq v\leq 1$. Note that when $v=1$, we have $y=3-2u/3$, the upper line of the boundary of $R$. Also, when $v=0$, we have $y=1$, which is the lower boundary of $R$. With $z=x^2+2y^2$, we determine $\vec r(u,v) =\bracket{u, 1+v(2-2u/3), u^2+2\big(1+v(2-2u/3)\big)^2}$, $0\leq u\leq 3$, $0\leq v\leq 1$. 

The surface is graphed in \autoref{fig:parsurf4}(b).}

Given a surface of the form $z=f(x,y)$, one can often determine a parametrization of the surface over a region $R$ in a manner similar to determining bounds of integration over a region $R$. Using the techniques of \autoref{sec:iterated_integrals}, suppose a region $R$ can be described by $a\leq x\leq b$, $g_1(x) \leq y\leq g_2(x)$, i.e., the area of $R$ can be found using the iterated integral
...
\[
\vec r(u,v)
= \bracket{u, h(u,v), f\big(u,h(u,v)\big)},\quad a\leq u\leq b,\ 0\leq v\leq 1.
\]}

\mtable{The cylinder parametrized in \autoref{ex_parsurf5}.}{fig:parsurf5}{%
\myincludeasythree{width=\marginparwidth,
3Droll=0,
...
3Dlights=Headlamp}{width=\marginparwidth}{figures/figparsurf5_3D}}

\example{ex_parsurf5}{Parameterizing a cylindrical surface}
{Find a parametrization of the cylinder $x^2 + z^2/4=1$, where $-1\leq y\leq 2$, as shown in \autoref{fig:parsurf5}.}
...
\[
\vec r(u,v)
= \bracket{\cos u, v, 2\sin u}, \quad 0\leq u\leq 2\pi,\quad -1\leq v\leq 2.\eoehere
\]}
...
3Dlights=Headlamp}{width=\marginparwidth}{figures/figparsurf6a_3D}}

\example{ex_parsurf6}{Parameterizing an elliptic cone}
{Find a parametrization of the elliptic cone $z^2 = \frac{x^2}{4}+\frac{y^2}{9}$, where $-2\leq z\leq 3$, as shown in \autoref{fig:parsurf6a}.}
...
3Dcoo=0.0026521924883127213 4.370458126068115 12.347787857055664,
3Droo=399.9999637774216,
3Dlights=Headlamp}{width=\marginparwidth}{figures/figparsurf6b_3D}}
%
\[
...
{Find a parametrization of the ellipsoid $\frac{x^2}{25}+y^2+\frac{z^2}{4}=1$ as shown in \autoref{fig:parsurf7}(a).}
...
3Dlights=Headlamp}{width=\marginparwidth}{figures/figparsurfareaA_3D}
\\(b)\\[10pt]
\myincludeasythree{width=\marginparwidth,
3Droll=0,
...
3Dlights=Headlamp}{width=\marginparwidth}{figures/figparsurfareaB_3D}
\\(c)}
...
%\mnote{\textbf{Note:} In \autoref{def:surfacearea}, the surface area differential of a surface given in the form $z=f(x,y)$ is defined to be $dS = \sqrt{1+f_x^2+f_y^2}\ dA$. We can use this differential in this and subsequent sections, though 

\example{ex_parsurfarea1}{Finding the surface area of a parametrized surface}
{Using the parametrization found in \autoref{ex_parsurf2}, find the surface area of $z=x^2+2y^2$ over the circular disk of radius 2, centered at the origin.}
{In \autoref{ex_parsurf2}, we parametrized the surface as $\vec r(u,v) =\\ \bracket{2v\cos u, 2v\sin u, 4v^2\cos^2u+8v^2\sin^2u}$, for $0\leq u\leq 2\pi$, $0\leq v\leq 1$. To find the surface area using \autoref{thm:par_surface_area}, we need $\norm{\vec r_u\times\vec r_v}$. We find:
\begin{align*}
