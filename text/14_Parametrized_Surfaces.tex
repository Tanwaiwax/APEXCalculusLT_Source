...
Let $\vec r(u,v) = \bracket{f(u,v),g(u,v),h(u,v)}$ be a vector-valued function that is continuous and one to one on the interior of its domain $R$ in the $u$-$v$ plane. The set of all terminal points of $\vec r$ (i.e., the \emph{range} of $\vec r$\,) is the \sword{surface} \surfaceS, and $\vec r$ along with its domain $R$ form a \sword{parameterization} of \surfaceS.\\
...
\end{definition}
% todo Tim check \\ in def:parameterized_surface
...
Another common way to parameterize the surface is to begin with $y=u$, $0\leq u\leq 2$. Solving the equation of the line $y=2-2x/3$ for $x$, we have $x = 3-3y/2$, leading to using $x=v(3-3u/2)$, $0\leq v\leq 1$. With $z=x^2+2y^2$, we have $\vec r(u,v) =\bracket{v(3-3u/2),u, \big(v(3-3u/2)\big)^2+2v^2}$, $0\leq u\leq 2$, $0\leq v\leq 1$.}
...
{Find a parameterization of the elliptic cone $z^2 = \frac{x^2}{4}+\frac{y^2}{9}$, where $-2\leq z\leq 3$, as shown in \autoref{fig:parsurf6a}.}
