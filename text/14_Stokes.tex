...
\definition{defn:lineintvec3}{Vector Line Integral}{For a vector field $\textbf{f}(x,y,z) = P(x,y,z)\,\textbf{i} + Q(x,y,z)\,\textbf{j} + R(x,y,z)\,\textbf{k}$ and a curve $C$ in $\mathbb{R}^{3}$ with a smooth parametrization $x=x(t)$, $y=y(t)$, $z=z(t)$, $a \le t \le b$, the \textbf{line integral of f along} $C$ is
 \begin{align*}
...
Similar to the two-variable case, if $\textbf{f}(x,y,z)$ represents the force applied to an object at a point $(x,y,z)$ then the line integral $\int_{C}{f}\cdot d\vec{r}$ represents the work done by that force in moving the object along the curve $C$ in $\mathbb{R}^{3}$.\index{work}
...
% \]
% Also, if $x=x(t_1,t_2)$, $y=y(t_1,t_2)$ and $z=z(t_1,t_2)$ are continuously differentiable function of $(t_1,t_2)$, then%\footnotemark
% \[
...
\theorem{thm:lineintsuffpath3}{Fundamental Theorem of Line Integrals in Three Dimensions}{Let $\textbf{f}(x,y,z) = P(x,y,z)\,\textbf{i} + Q(x,y,z)\,\textbf{j} + R(x,y,z)\,\textbf{k}$ be a vector field in some solid $S$, with $P$, $Q$ and $R$ continuously differentiable functions on $S$. Let $C$ be a smooth curve in $S$ parametrized by $x=x(t)$, $y=y(t)$, $z=z(t)$, $a \le t \le b$. Suppose that there is a real-valued function $F(x,y,z)$ such that $\nabla F = \textbf{f}$ on $S$. Then
 \begin{equation}\label{eqn:lineintsuffpath3}
  \int_{C}{f}\cdot d\vec{r} = F(B) - F(A) ,
 \end{equation}
 where $A=(x(a),y(a),z(a))$ and $B=(x(b),y(b),z(b))$ are the endpoints of $C$.}

\theorem{cor:lineintsuffpath3}{Zero Line Integral}{If a vector field $\textbf{f}$ has a potential in a solid $S$, then $\ds\oint_{C}{f}\cdot d\vec{r} = 0$ for any closed curve $C$ in $S$ (i.e. $\ds\oint_{C}\Dotprod{\nabla F}{d\textbf{r}} = 0$ for any real-valued function $F(x,y,z)$).}
...
\example{exmp_conhelix2}{Evaluating a Line Integral}{Let $\textbf{f}(x,y,z) = x\,\textbf{i} + y\,\textbf{j} + 2z\,\textbf{k}$ be a vector field in $\mathbb{R}^{3}$. Using the same curve $C$ from \autoref{exmp_conhelix}, evaluate $\int_{C}{f}\cdot d\vec{r}$.}{It is easy to see that $F(x,y,z)=\frac{x^2}{2}+\frac{y^2}{2}+z^2$ is a potential for $\textbf{f}(x,y,z)$ (i.e. $\nabla F = \textbf{f}$). So by \autoref{thm:lineintsuffpath3} we know that
 \begin{align*}
  \int_{C}{f}\cdot d\vec{r} &= F(B) - F(A) ,\text{ where $A=(x(0),y(0),z(0))$ and $B=(x(8\pi),y(8\pi),z(8\pi))$, so}\\
...
Let $\Sigma$ be an orientable surface in $\mathbb{R}^{3}$ whose boundary is a simple closed curve $C$, and let $\textbf{f}(x,y,z) = P(x,y,z) \textbf{i} + Q(x,y,z) \textbf{j} + R(x,y,z) \textbf{k}$ be a smooth vector field defined on some subset of $\mathbb{R}^{3}$ that contains $\Sigma$. Then
  \begin{equation}\label{eqn:stokes}
   \oint_{C}{f}\cdot d\vec{r} = \iint_{\Sigma} \Dotprod{(\text{curl}{\textbf{f}}\,)}{\textbf{n}}\,d\sigma ,
  \end{equation}
  where
...
\example{exmp_stokesparab}{Verifying Stokes' Theorem}{Verify Stokes' Theorem for $\textbf{f}(x,y,z) = z\,\textbf{i} + x\,\textbf{j} + y\,\textbf{k}$ when $\Sigma$ is the paraboloid $z=x^2 + y^2$ such that $z \le 1$ (see \autoref{fig_parab_stokes}).%
 ...
 \end{align*}
 So we see that $\oint_{C}{f}\cdot d\vec{r}=\iint_{\Sigma} \Dotprod{(\text{curl }{\textbf{f}}\,)}{\textbf{n}} \,d\sigma$, as predicted by Stokes' Theorem.}
...
\example{ex_use_stokes}{Using Stokes' Theorem}{Let $\Sigma$ be the elliptic paraboloid $z=\frac{x^2}{4}+\frac{y^2}{9}$ for $z \le 1$, and let $C$ be its boundary curve. Calculate $\oint_{C}{f}\cdot d\vec{r}$ for $\textbf{f}(x,y,z)=(9xz+2y)\textbf{i}+(2x+y^2 )\textbf{j}+(-2y^2 +2z)\textbf{k}$, where $C$ is traversed counterclockwise.}{The surface is similar to the one in \autoref{exmp_stokesparab}, except now the boundary curve $C$ is the ellipse $\frac{x^2}{4}+\frac{y^2}{9}=1$ laying in the plane $z=1$. In this case, using Stokes' Theorem is easier than computing the line integral directly. As in \autoref{exmp_stokesparab}, at each point $(x,y,z(x,y))$ on the surface $z=z(x,y)=\frac{x^2}{4}+\frac{y^2}{9}$ the vector
 ...
 \[
  \oint_{C}{f}\cdot d\vec{r} = \iint_{\Sigma} \Dotprod{(\text{curl }{\textbf{f}}\,)}{\textbf{n}}\,d\sigma =
   \iint_{\Sigma} 0\,d\sigma = 0 .\eoehere
 \]}

In physical applications, for a simple closed curve $C$ the line integral $\oint_{C}{f}\cdot d\vec{r}$ is often called the \textbf{circulation}\index{circulation} of \textbf{f} around $C$. For example, if \textbf{E} represents the electrostatic field due to a point charge, then it turns out
%\footnote{See Ch. 2 in \cite{rmc}.}
that $\text{curl }\textbf{E} = \textbf{0}$, which means that the circulation $\oint_{C}{E}\cdot d\vec{r}=0$ by Stokes' Theorem. Vector fields which have zero curl are often called \emph{irrotational}\index{irrotational} fields.

In fact, the term curl was created by the 19\textsuperscript{th} century Scottish physicist James Clerk Maxwell in his study of electromagnetism, where it is used extensively. In physics, the curl is interpreted as a measure of \emph{circulation density}. This is best seen by using another definition of curl \textbf{f} which is equivalent
%\footnote{See \cite{sch}, p. 78-81, for the derivation.}
to the definition given by \autoeqref{eqn:curl}. Namely, for a point $(x,y,z)$ in $\mathbb{R}^{3}$,
\[
 \Dotprod{\textbf{n}}{(\text{curl }\textbf{f}\,)}(x,y,z) = \lim_{S \to 0} \frac{1}{S} \oint_{C}{f}\cdot d\vec{r} ,
\]
...
Finally, by Stokes' Theorem, we know that if $C$ is a simple closed curve in some solid region $S$ in $\mathbb{R}^{3}$ and if $\textbf{f}(x,y,z)$ is a smooth vector field such that $\text{curl }\textbf{f} = \textbf{0}$ in $S$, then
\[
 \oint_{C}{f}\cdot d\vec{r} = \iint_{\Sigma} \Dotprod{(\text{curl }{\textbf{f}}\,)}{\textbf{n}}\,d\sigma =
   \iint_{\Sigma} \Dotprod{\textbf{0}}{\textbf{n}}\,d\sigma = \iint_{\Sigma} 0\,d\sigma = 0 ,
\]
...
\theorem{thm:irrot_equiv}{Irrotational Equivalences}{The following statements are equivalent for a simply connected solid region $S$ in $\mathbb{R}^{3}$:
  ...
  \item $\displaystyle{\int_{C}{f}\cdot d\vec{r}}$ is independent of the path for any curve $C$ in $S$
  \item $\displaystyle{\oint_{C}{f}\cdot d\vec{r}} = 0$ for every simple closed curve $C$ in $S$
...
\example{ex_check_irrot}{Determining Irrotation}{Determine if the vector field $\textbf{f}(x,y,z) = xyz\,\textbf{i} + xz\,\textbf{j} + xy\,\textbf{k}$ has a potential in $\mathbb{R}^{3}$.}{Since $\mathbb{R}^{3}$ is simply connected, we just need to check whether $\text{curl }\textbf{f} = \textbf{0}$ throughout $\mathbb{R}^{3}$, that is,
 \[
 ...
 \]
 Thus, $\textbf{f}(x,y,z)$ does not have a potential in $\mathbb{R}^{3}$.}
