...
\theorem{thm:lineinttangent3}{Vector Line Integral Along a Curve}{For a vector field $\vecf(x,y,z) = P(x,y,z)\,\veci + Q(x,y,z)\,\vecj + R(x,y,z)\,\veck$ and a curve $C$ with a smooth parametrization $x=x(t)$, $y=y(t)$, $z=z(t)$, $a \le t \le b$ and position vector $\vecr(t) = x(t)\,\veci + y(t)\,\vecj + z(t)\,\veck$,
 \[
...
\theorem{cor:lineintsuffpath3}{Zero Line Integral}{If a vector field $\vecf$ has a potential in a solid $S$, then $\ds\oint_{C}{f}\cdot d\vec{r} = 0$ for any closed curve $C$ in $S$ (i.e., $\ds\oint_{C}\Dotprod{\nabla F}{d\vecr} = 0$ for any real-valued function $F(x,y,z)$).}
...
\mtable{Conical helix $C$ for \autoref{exmp_conhelix}.}{fig:conhelix}{%
 \begin{tikzpicture}[scale=1.6]
  \begin{axis}[width=\marginparwidth,tick label style={font=\scriptsize},axis on top,
				axis lines=center,y dir=reverse,name=myplot,
				ymin=-30,ymax=30,xmin=-30,xmax=30,zmin=0,zmax=30]
   \addplot3[{\colorone},domain=0:8*pi,samples=100,samples y=0]
    ({x*cos(deg(x))},{x*sin(deg(x))},{x});
  \end{axis}
  \node [left] at (myplot.below origin) [shift={(-30pt,0pt)}] {$x$};
  \node [below left] at (myplot.right of origin) [shift={(-10pt,-6pt)}] {$y$};
  \node [below] at (myplot.above origin) [shift={(0,-10pt)}] {$z$};
 \end{tikzpicture}}%
...
\mtable{An orientation of the sphere.}{fig:or_sph}{\begin{tikzpicture}
 ...
 \node [right] at (1.5,1.5) {\small $\vecn$};
 \draw [dashed,line width=1.2pt,-latex] (0.98,0.98) -- (0.46,0.46);
 \node [above] at (0.4,0.58) {\small $-\vecn$};
\end{tikzpicture}}
...
Let $\Sigma$ be an orientable surface in $\mathbb{R}^{3}$ whose boundary is a simple closed curve $C$, and let $\vecf(x,y,z) = P(x,y,z) \veci + Q(x,y,z) \vecj + R(x,y,z) \veck$ be a smooth vector field defined on some subset of $\mathbb{R}^{3}$ that contains $\Sigma$. Then
  \begin{equation}\label{eqn:stokes}
   \oint_{C}{f}\cdot d\vec{r} = \iint_{\Sigma} \Dotprod{(\curl{\vecf}\,)}{\vecn}\,d\sigma ,
  \end{equation}
  where
...
\begin{proof}
 As the general case is beyond the scope of this text, we will prove the theorem only for the special case where $\Sigma$ is the graph of $z=z(x,y)$ for some smooth real-valued function $z(x,y)$, with $(x,y)$ varying over a region $D$ in $\mathbb{R}^{2}$. \mtable{A particular case of Stokes' Theorem}{fig_stokes_eg}{\begin{tikzpicture}
  ...
  \node [right] at (2.7,2.3) {\small $\vecn$};
...
\example{exmp_stokesparab}{Verifying Stokes' Theorem}{Verify Stokes' Theorem for $\vecf(x,y,z) = z\,\veci + x\,\vecj + y\,\veck$ when $\Sigma$ is the paraboloid $z=x^2 + y^2$ such that $z \le 1$ (see \autoref{fig_parab_stokes}).%
 \mtable{$z=x^2 + y^2$ for \autoref{exmp_stokesparab}.}{fig_parab_stokes}{\begin{tikzpicture}
  ...
  \pgfputat{\pgfpointxyz{0.05}{-0.2}{0}}{\pgfbox[center,center]{\small $0$}};
  \node [right] at (0.7,3.15) {\small $\vecn$};
...
In physical applications, for a simple closed curve $C$ the line integral $\oint_{C}{f}\cdot d\vec{r}$ is often called the \textbf{circulation}\index{circulation} of \vecf around $C$. For example, if \vecE represents the electrostatic field due to a point charge, then it turns out
%\footnote{See Ch. 2 in \cite{rmc}.}
that $\curl\vecE = \vec0$, which means that the circulation $\oint_{C}{E}\cdot d\vec{r}=0$ by Stokes' Theorem. Vector fields which have zero curl are often called \emph{irrotational}\index{irrotational} fields.
