...
\mnote{\textbf{Note:} the term ``outer unit normal vector'' used in \autoref{thm:divergence2} means $\vec n$ points to the outside of \surfaceS.\index{outer unit normal vector}}

\begin{theorem}[The Divergence Theorem (in space)]\label{thm:divergence2}%
...
\youtubeVideo{asyIsn59Lnc}{Divergence theorem example 1 | Divergence theorem | Multivariable Calculus | Khan Academy}

\mtable{The surfaces used in \autoref{ex_divthm_space2}.}{fig:divthm_space2}{%
\myincludeasythree{
3Droll=4.96990006464786,
3Dortho=0.004236375913023949,
3Dc2c=-0.8009352684020996 0.5406254529953003 0.2573457956314087,
3Dcoo=-4.848399639129639 2.010268211364746 61.92150115966797,
3Droo=399.9999804687495}{\marginparwidth}{A wedge where 0≤z≤2x and 0≤x≤1-y^2.}{figures/figdivthm_space2_3D}}

\begin{example}[Using the Divergence Theorem in space]\label{ex_divthm_space2}%
...
3Dlights=Headlamp}{\marginparwidth}{In the xy plane is a unit circle centered at the origin.  Above it is a dome.}{figures/figsurfflux2_3D}}

\begin{example}[Using the Divergence Theorem in space]\label{ex_divthm_space1}%
...
\begin{example}[A ``paradox'' of the Divergence Theorem and Gauss's Law]\label{ex_divthm_space3}%
...
\mtable{The cube used in \autoref{ex_divthm_space3}.}{fig:divthm_space3}{%
\myincludeasythree{
3Droll=0,
...
3Dlights=Headlamp}{\marginparwidth}{A cube centered at the origin with edges of length 2a parallel to the coordinate axes.}{figures/figdivthm_space3_3D}}
...
\begin{example}[Using the Divergence Theorem to compute flux]\label{ex_divthm_space4}%
...
\begin{theorem}[Stokes' Theorem]\label{thm:stokes_thm}%
...
\youtubeVideo{E2MRMdaPKcc}{Stokes example part 1 | Multivariable Calculus | Khan Academy}

\mtable{As given in \autoref{ex_stokes1}, the surface \surfaceS\ is the portion of the plane bounded by the curve.}{fig:stokes1}{%
\myincludeasythree{
3Droll=0,
...
3Dlights=Headlamp}{\marginparwidth}{In the xy plane is a circle of radius 1 centered at (1,1).  Above this is a plane tilted so that its normal points toward the origin.  In the plane, a copy of the circle is drawn.}{figures/figstokes1_3D}}

\begin{example}[Verifying Stokes' Theorem]\label{ex_stokes1}%
...
\begin{align*}
	...
	%\quad \text{(which simplifies to)}\\
	%&= \int_0^{2\pi}\bracket{\cos t+\sin t+2, 2\sin t+2, (\sin t+1)^2}\cdot\bracket{-\sin t,\cos t, -2\sin t+2\cos t}\dd t \\
...
One of the interesting results of Stokes' Theorem is that if two surfaces $\surfaceS_1$ and $\surfaceS_2$ share the same boundary, then $\iint_{\surfaceS_1} (\curl \vec F)\cdot \vec n\dd S = \iint_{\surfaceS_2} (\curl \vec F)\cdot \vec n\dd S$. That is, the value of these two surface integrals is somehow independent of the interior of the surface. We demonstrate this principle in the next example.

\mtable{As given in \autoref{ex_stokes2}, the surface \surfaceS\ is the portion of the plane bounded by the curve.}{fig:stokes2}
{\myincludeasythree{
3Droll=0,
...
3Droo=399.9999566444115,
3Dlights=Headlamp}{\marginparwidth}{In the xy plane is a circle of radius 1 centered at (1,1).  Above this is a curved surface, with the circle drawn on it.}{figures/figstokes2_3D}
\\(a)\\[10pt]
\myincludeasythree{
3Droll=0,
3Dortho=0.004444750025868416,
...
3Dcoo=48.335105895996094 37.655845642089844 31.75312042236328,
3Droo=399.9999566444115,
3Dlights=Headlamp}{\marginparwidth}{The same image as the previous, but with an additional surface that mostly overlaps the first surface.}{figures/figstokes3_3D}
\\(b)}

\begin{example}[Stokes' Theorem and surfaces that share a boundary]\label{ex_stokes2}%
...
%The theorems in this chapter all relate an integral over a domain to an integral over the boundary of the domain.  This means that we need to pay special attention to the orientation of the domain.
...
\begin{keyidea}[Fundamental Theorems Relating Integrals and Domains]\label{fundThms}%
\renewcommand{\arraystretch}{1.4}
\tagpdfsetup{table/header-rows={1}}
 \begin{tabular}{p{10em} l l}
  Theorem & Equation & Orientation \\\midrule
  Fundamental Theorem\par of Calculus & $\ds\left.f\right|_{\partial[a,b]}=\int_{[a,b]} \fp(x)\dd x$ &\\
  Fundamental Theorem\par of Gradient Fields & $\ds\left.\phi\right|_{\partial[P,Q]}=\int_{[P,Q]}\grad\phi\cdot\dd s$ & \\
  Green's Theorem & $\ds\oint_{\partial R}\vecF\cdot\dd\vecr=\iint_R\curl\vec F\dd A$ & $\partial D$ oriented counterclockwise \medskip\\
  Stokes' Theorem & $\ds\oint_{\partial S}\vec F\cdot\dd\vec r=\iint_S\curl\vec F\cdot \vecn\dd S$ & $\partial S$ oriented with $S$ to the left \medskip\\
  $\mathbb{R}^2$ Divergence Theorem & $\ds\oint_{\partial R}\vec F\cdot\vec n\dd s=\iint_R\divv\vec F\dd A$ & $\partial R$ oriented outwards \medskip\\
  $\mathbb{R}^3$ Divergence Theorem & $\ds\iint_{\partial D}\vec F\cdot\vec n\dd S=\iiint_D\divv\vec F\dd V$ & $\partial D$ oriented outwards
 \end{tabular}
\end{keyidea}
}

\printexercises{exercises/14-07-exercises}
