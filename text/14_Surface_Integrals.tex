...
3Dlights=Headlamp}{width=\marginparwidth}{figures/figsurfint1_3D}}
...
\setlength{\tabcolsep}{1.5em}
 \begin{tabular}{lp{13em}p{17em}}\toprule
  ...
  $\ds\int_{\vec c(D)} f ds=\int_D f(\vec c(t))\norm{\vec c\,'(t)}dt$
  & scalar surface integral:\par
  $\ds\iint_{\vec r(D)} f\ dS=\iint_D f(\vec r(u,v))\norm{\vec r_u\times\vec r_v}dA$
  \\
  $\vec F:\mathbb{R}^3\to\mathbb{R}^3$
  & vector line integral:\par
  $\ds\int_{\vec c(D)}\vec F\cdot d\vec r=\int_D\vec F(\vec c(t))\cdot\vec c\,'(t)dt$
  & vector surface integral:\par
  $\ds\iint_{\vec r(D)}\vec F\cdot\vec n\ dS
