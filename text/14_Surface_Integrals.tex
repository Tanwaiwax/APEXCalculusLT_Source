...
\begin{align*}
	...
	&= \int_0^1\int_0^1 \bigl(u^2-8uv-2u+8v+3\bigr)\bigl(2\sqrt{6}(1-u)\bigr)\ du\ dv\\
...
\[
\text{Flux} = \iint_R \vec F\bigl(\vec r(u,v)\bigr)\cdot (\vec r_u\times \vec r_v)\ dA.
\]
\end{definition}
...
Let \surfaceS\ be the surface given in \autoref{ex_surfint1}, where \surfaceS\ is parameterized by $\vec r(u,v) =\bracket{u, v(2-2u),3-2u-v(2-2u)}$ on $0\leq u\leq 1$, $0\leq v\leq 1$, and let $\vec F =\bracket{1, x,-y}$, as shown in \autoref{fig:surfflux1}. Find the flux of $\vec F$ across \surfaceS.
\solution
Using our work from the previous example, we have $\vec n = \vec r_u\times\vec r_v =\bracket{4-4u,2-2u,2-2u}$. We also need $\vec F\bigl(\vec r(u,v)\bigr) =\bracket{1, u, -v(2-2u)}$. 
...
\begin{align*}
	...
	&= \int_0^1\int_0^1 \bigl(-4u^2v-2u^2+8uv-2u-4v+4\bigr)\ du\ dv\\
	&= 5/3.
\end{align*}
...
\renewcommand{\arraystretch}{1.4}
 \begin{tabular}{|l|p{.32\linewidth}|p{.43\linewidth}|}\hline
