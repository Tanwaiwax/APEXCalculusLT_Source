...
\captionsetup{type=figure}%
 \centering
  \begin{tikzpicture}
   \draw [->] (0,0) --node[pos=1,above]{\small$x$} (6,0);
   \draw [->] (0,0) --node[pos=1,right]{\small$y$} (0,2.8);
   \node[below]at(0,0){\small$0$};
   \draw [rounded corners,->,thick,draw={\colorone}]
    (0.75,2.1) node[below]{$t=a$}node[above right]{$C$}
    .. controls (2.95,3) and (3.55,1.2) .. (4,0.75);
   \draw [rounded corners,thick,draw={\colorone}]
    (4,0.75) .. controls (4.3,0.45) and (4.9,0.3)
     .. (5.35,0.6) node[above]{$t=b$};
   % now the triangle
   \draw[draw={\colorone}] (2.2,2.3)node[above]{$t=t_{i-1}$}
    --node[above right]{$\Delta s_i\approx\sqrt{(\Delta x_i)^2 + (\Delta y_i)^2}$} (3.5,1.4)
    node[pos=1,right]{$t=t_i$}
    --node[below]{$\Delta x_i$} (2.2,1.4)
    --node[left]{$\Delta y_i$} cycle;
   % right angle marker
   \draw[draw={\colorone}] (2.2,1.6) -- (2.4,1.6) -- (2.4,1.4);
   \fill[fill={\colorone},draw={\colorone}] (0.75,2.1) circle (2pt);
   \fill[fill={\colorone},draw={\colorone}] (5.35,0.6) circle (2pt);
   \fill[fill={\colorone},draw={\colorone}] (2.2,2.32) circle (1.4pt);
   \fill[fill={\colorone},draw={\colorone}] (3.5,1.4) circle (1.4pt);
  \end{tikzpicture}
 \caption{Curve $C:x=x(t),y=y(t)$ for $t$ in $[a,b]$}
...
\captionsetup{type=figure}%
 \centering
  \begin{tikzpicture}
   \draw [->] (0,0)
    --node[pos=1,above]{\small$x$} (6,0);
   \draw [->] (0,0)
    --node[pos=1,right]{\small$y$} (0,3.5);
   \node[below]at(0,0){\small$0$};
   \filldraw [fill={\coloronefill},draw={\colorone},->] (4,0.75)
    .. controls (4.3,0.45) and (4.9,0.3)  .. (5.35,0.6) -- (5.55,1.2)
    .. controls (4.9,0.9)  and (4.3,1.1)  .. (4,1.4)
    .. controls (3.65,2)   and (2.85,3.8) .. (0.55,2.8) -- (0.75,2.1)
    .. controls (2.95,3)   and (3.55,1.2) .. cycle;
   \fill [fill={\colorone}] (1.9,2.35) -- (1.9,3.1) -- (1.7,3.1)
    --node[pos=.5,left]{$f(x,y)$} (1.7,2.35)
    --node[pos=.5,below]{$ds$} cycle;
   \fill[draw={\colorone},fill={\colorone}] (0.75,2.1) circle (1.7pt)node[below right]{$C$};
   \fill[draw={\colorone},fill={\colorone}] (5.35,0.6) circle (1.7pt);
  \end{tikzpicture}
 \caption{Area of shaded rectangle $= \text{height}\times\text{width} \approx f(x,y)\,ds$}
...
\example{exmp_lineintcyl}{Using the Line Integral}{Use a line integral to show that the lateral surface area $A$ of a right circular cylinder of radius $r$ and height $h$ is $2\pi r h$.}{%
 ...
 % todo Tim convert to asy
   ...
   \draw [black!60,line width=0.3pt,-latex] (0,0)
    --node[pos=1,above]{\small$y$} (2,0);
   \draw [black!60,line width=0.3pt,-latex] (0,0)
    --node[pos=1,right]{\small$z$} (0,3);
   \draw [black!60,line width=0.3pt,-latex] (0,0)
    --node[pos=1,right]{\small$x$} (0,0,2);
...
\end{align*}
by definition of $\vecf(x,y)$. Notice that the function $\vecf(x(t),y(t))\cdot\vecr\primeskip'(t)$ is a \emph{real-valued} function on $[a,b]$, so the last integral on the right looks somewhat similar to our earlier definition of a line integral. This leads us to the following definition:\index{line integral}\index{$\int_C$}

\definition{defn:lineintvec2}{Line Integral of a Vector Valued Function}{For a vector field $\vecf(x,y) = P(x,y)\,\veci + Q(x,y)\,\vecj$ and a curve $C$ with a smooth parametrization $x=x(t)$, $y=y(t)$, $a \le t \le b$, the \textbf{line integral of f along} $C$ is
 \begin{align}
  \int_C \vecf\cdot d\vecr
  &= \int_C P(x,y)\,dx + \int_C Q(x,y)\,dy\\
   &= \int_a^b \vecf(x(t),y(t))\cdot\vecr\primeskip'(t)\,dt ,
 \end{align}
 where $\vecr(t) = x(t)\,\veci + y(t)\,\vecj$ is the position vector for points on $C$.}

We use the notation $d\vecr = \vecr\primeskip'(t)\,dt = dx\,\veci + dy\,\vecj$\index{$d\vecr$} to denote the \textbf{differential}\index{differential} of the vector-valued function \vecr. The line integral in \autoref{defn:lineintvec2} is often called a \emph{line integral of a vector field} to distinguish it from the line integral in \autoref{defn:lineintscalar2} which is called a \emph{line integral of a scalar field}. For convenience we will often write
\[\int_C P(x,y)\,dx + \int_C Q(x,y)\,dy = \int_C P(x,y)\,dx + Q(x,y)\,dy ,\]
where it is understood that the line integral along $C$ is being applied to both $P$ and $Q$. The quantity $P(x,y)\,dx + Q(x,y)\,dy$ is known as a \textbf{differential form}.\index{differential form}
%For a real-valued function $F(x,y)$, the \textbf{differential} of $F$ is $dF = \frac{\partial F}{\partial x}\,dx + \frac{\partial F}{\partial y}\,dy$.
\mnote{\textbf{Note:} We defined total differential in \autoref{def:total_differential} in \autoref{sec:total_differential} as $dF=\frac{\partial F}{\partial x}\,dx + \frac{\partial F}{\partial y}\,dy$.}
A differential form $P(x,y)\,dx + Q(x,y)\,dy$ is called \textbf{exact}\index{exact differential form} if it equals the total differential $dF$ for some function $F(x,y)$.
...
\theorem{thm:lineinttangent}{Line Integrals and Tangent Vectors}{For a vector field $\vecf(x,y) = P(x,y)\,\veci + Q(x,y)\,\vecj$ and a curve $C$ with a smooth parametrization $x=x(t)$, $y=y(t)$, $a \le t \le b$ and position vector $\vecr(t) = x(t)\,\veci + y(t)\,\vecj$,
 \[
  \int_C \vecf\cdot d\vecr
  = \int_C \vecf\cdot\vecT\,ds ,
 \]
 where $\vecT(t) = \frac{\vecr\primeskip'(t)}{\norm{\vecr\primeskip'(t)}}$ is the unit tangent vector to $C$ at  $(x(t),y(t))$.}
...
%
 \mtable{Figure for \autoref{exmp_lineintexmp}}{fig_line_ints}{\begin{tikzpicture}
   \draw [->] (-0.5,0) --node[pos=1,below]{\small$1$}(1,0)
    --node[pos=1,above]{\small$x$} (2,0);
   \draw [->] (0,-0.5) --node[pos=1,left]{\small$2$}(0,2)
    --node[pos=1,right]{\small$y$} (0,2.5);
   \node[below left]at(0,0){\small$0$};
   \draw [draw={\colorone},thick] (0,0) parabola (1,2);
   \draw [draw={\colorone},thick,->] (0,0) parabola (0.5,0.5);
   \draw [draw={\colorone},thick] (0,0) -- (1,2)node[above right]{\small $(1,2)$};
   \draw [draw={\colorone},thick,->] (0,0) -- (0.5,1);
 ...
 \end{align*}
 So in both cases, if the vector field $\vecf(x,y) = ( x^2 + y^2 )\,\veci + 2xy\,\vecj$ represents the force moving an object from $(0,0)$ to $(1,2)$ along the given curve $C$, then the work done is $\frac{13}{3}$. This may lead you to think that work (and more generally, the line integral of a vector field) is independent of the path taken. However, as we will see in the next section, this is not always the case.
 \end{enumerate}}

We defined line integrals over a single smooth curve, but we can extend this definition to a \emph{piecewise smooth curve} as well.  We say that $C$ is piecewise smooth if\index{piecewise smooth curve}
\[C = C_1 \cup C_2 \cup \dots \cup C_n\]
is the union of smooth curves $C_1,\dotsc,C_n$ that are disjoint (or overlap at most on isolated points).  In that case, we can define
\[
 \int_C \vecf\cdot d\vecr
  = \int_{C_1} \vecf\cdot d\vecr_1 +
  \int_{C_2} \vecf\cdot d\vecr_2 + \dotsb +
  \int_{C_n} \vecf\cdot d\vecr_n
\]
where each $\vecr_i$ is the position vector of the curve $C_i$.
...
%
 \mtable{The Figure for \autoref{exmp_lineintexmppoly}}{fig_piece_line_int}{\begin{tikzpicture}
   ...
   \draw [->] (-0.5,0) --node[pos=1,below]{\small$1$}(1,0)
    --node[pos=1,above]{\small$x$} (2,0);
   \draw [->] (0,-0.5) --node[pos=1,right]{\small$y$} (0,2.5);
   \node[below left]at(0,0){\small$0$};
   \draw [draw={\colorone},thick,->] (0,0) -- (0,1)node[right]{\small$C_1$};
   \draw [draw={\colorone},thick,->] (0,1) -- (0,2)node[left]{\small$2$}
    -- (0.5,2)node[below]{\small$C_2$};
   \draw [draw={\colorone},thick] (0.5,2) -- (1,2)node[right]{\small $(1,2)$};
   \fill[draw={\colorone},fill={\colorone}] (0,0) circle (1.7pt);
   \fill[draw={\colorone},fill={\colorone}] (1,2) circle (1.7pt);
  \end{tikzpicture}}
  %
...
Line integral notation varies quite a bit. For example, in physics it is common to see the notation $\int_a^b \vecf\cdot d\vecl$, where it is understood that the limits of integration $a$ and $b$ are for the underlying parameter $t$ of the curve, and the letter \textbf{l} signifies length. Also, the formulation $\int_C \dotp{f}{T}\,ds$ from \autoref{thm:lineinttangent} is often preferred in physics since it emphasizes the idea of integrating the tangential component $\dotp{f}{T}$ of \vecf\ in the direction of \vecT\ (i.e., in the direction of $C$), which is a useful physical interpretation of line integrals.
