%\section*{\thesection\quad Chapter Prerequisites}
%\prereqIntro
%
...
%\addplot[{\colorone}, domain=-2:2, thick, smooth]{exp(x)};
%\end{axis}
%\node [right] at (myplot.right of origin) {\scriptsize $x$};
%\node [above] at (myplot.above origin) {\scriptsize $y$};
%\end{tikzpicture}
...
%\begin{tikzpicture}[scale = .3]
%\begin{axis}[axis y line=middle, axis x line=middle, xmin=-2.2, xmax=2.2, ymin=-2.2, ymax=2.2,name=myplot,axis equal]
%\addplot[{\colorone}, domain=-2:2, thick, smooth,samples=100]({x*x*x},{x});
%\end{axis}
%\node [right] at (myplot.right of origin) {\scriptsize $x$};
%\node [above] at (myplot.above origin) {\scriptsize $y$};
%\end{tikzpicture}
...
%
%\begin{center}tikzpicture\end{center}
%
%after 2
%
%\begin{tikzpicture}
...
%\end{center}
%
%\example{prereq_domain_3}{Finding a domain}{Find the domain of the function $h(x)=\dfrac1{\sqrt{x^2-4}}$}{For $h$ to be defined as a real number we must have $x^2-4>0$. This is equivalent to $(x-2)(x+2)>0$ and we create a sign chart:
%
%\begin{center}
%\begin{tikzpicture}
%  \matrix[row sep=1mm,column sep=3mm,ampersand replacement=\&]{
