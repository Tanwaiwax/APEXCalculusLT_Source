...
%where \autoeqref{eq:chainRulePfOrg} means that $\epsilon(h)\to0$ as $h\to0$.  The proof continues\ldots.
%\end{proof}
...
%\item $\ds \lim_{\norm{\Delta x}\to 0} \sum_{i=1}^n f(c_i)\Delta x_i = \int_a^b f(x)\ dx$.%, where the latter sum is any Riemann sum of $f$ on $[a,b]$, and
%\end{enumerate}
% proof
